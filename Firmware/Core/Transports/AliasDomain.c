...
static struct Map_s* _aliasDomain_aliasMemory = NULL;
...
void AliasDomain_parseAliasConfig( const char* token, char* line )
{
    ...
    Map_insert( &_aliasDomain_aliasMemory,
        Map_newElement( aliasname,
            strdup( transportdef ),
            &free_wrapper ) );
}
...
{
    Map_clear( _aliasDomain_aliasMemory );
    ...
    const char* default_target )
{
    ...
    aliasdata = ( const char* )Map_at( _aliasDomain_aliasMemory, str );
    if ( !aliasdata ) {

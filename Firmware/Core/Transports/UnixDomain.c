...
Transport_Transport* UnixDomain_transport( struct sockaddr_un* addr, int local )
{
    ...
    t->domain_length = sizeof( unixDomain_priotUnixDomain ) / sizeof( unixDomain_priotUnixDomain[ 0 ] );
    ...
    if ( local ) {
        ...
        if ( rc != 0 && errno == ENOENT && _unixDomain_createPath ) {
            rc = Directory_makeDirectoryHierarchy( addr->sun_path, _unixDomain_createMode, 1 );
            if ( rc != 0 ) {
                _UnixDomain_close( t );
                Transport_free( t );
                return NULL;
...
Transport_Transport* UnixDomain_createTstring( const char* string, int local, const char* default_target )
{
    ...
    if ( ( string != NULL && *string != '\0' ) && ( strlen( string ) < sizeof( addr.sun_path ) ) ) {

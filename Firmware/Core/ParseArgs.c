        ...
        case 'T': {
            ...
            *tmpcp++ = '\0';
            ...
            if ( !session->transportConfiguration ) {
                Container_initList();
                session->transportConfiguration = Container_find( "transport_configuration:fifo" );
                if ( !session->transportConfiguration ) {
                ...
                session->transportConfiguration->compare = ( Container_FuncCompare* )
...
#define SNMPV3_CMD_OPTIONS
    ...
    if ( session->version == API_DEFAULT_VERSION ) {
         ...
         */
        session->version = DefaultStore_getInt( DsStore_LIBRARY_ID,
            DsInt_SNMPVERSION );
    ...
    if ( Apsz ) {
        ...
        if ( session->securityAuthProto == NULL ) {
             ...
             */
            const oid* def = Usm_getDefaultAuthtype( &session->securityAuthProtoLen );
            session->securityAuthProto = Api_duplicateObjid( def, session->securityAuthProtoLen );
        }
        if ( session->securityAuthProto == NULL ) {
            /*
             * assume MD5
             */
            session->securityAuthProto = Api_duplicateObjid( usm_hMACMD5AuthProtocol,
    ...
    if ( Xpsz ) {
        ...
        if ( session->securityPrivProto == NULL ) {
             ...
             * get .conf set default
             */
            const oid* def = Usm_getDefaultPrivtype( &session->securityPrivProtoLen );
            session->securityPrivProto = Api_duplicateObjid( def, session->securityPrivProtoLen );
        }
        if ( session->securityPrivProto == NULL ) {
            /*
             * assume DES
             */
            session->securityPrivProto = Api_duplicateObjid( usm_dESPrivProtocol,

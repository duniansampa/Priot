...
static struct Parse_Tok_s _parse_tokens[] = {
...
static struct Parse_ModuleCompatability_s * _parse_moduleMapHead;
static struct Parse_ModuleCompatability_s _parse_moduleMap[] = {
    ...
    {"RFC1213-MIB", "SNMPv2-MIB", "priot", 4},
...
static struct Parse_Tok_s *_parse_buckets[PARSE_HASHSIZE];

static struct Parse_Node_s *_parse_nbuckets[PARSE_NHASHSIZE];
static struct Parse_Tree_s *_parse_tbuckets[PARSE_NHASHSIZE];
static struct Parse_Module_s *_parse_moduleHead = NULL;

static struct Parse_Node_s *  _parse_orphanNodes = NULL;

struct Parse_Tree_s * parse_treeHead = NULL;

#define	PARSE_NUMBER_OF_ROOT_NODES	3
static struct Parse_ModuleImport_s _parse_rootImports[PARSE_NUMBER_OF_ROOT_NODES];

static int      _parse_currentModule = 0;
static int      _parse_maxModule = 0;
static int      _parse_firstErrModule = 1;
static char    *_parse_lastErrModule = NULL; /* no repeats on "Cannot find module..." */


static void     _Parse_treeFromNode(struct Parse_Tree_s *tp, struct Parse_Node_s *np);
static void     _Parse_doSubtree(struct Parse_Tree_s *, struct Parse_Node_s **);
static void     _Parse_doLinkup(struct Parse_Module_s *, struct Parse_Node_s *);
static void     _Parse_dumpModuleList(void);
static int      _Parse_parseQuoteString(FILE *, char *, int);
static int      _Parse_tossObjectIdentifier(FILE *);
static int      _Parse_nameHash(const char *);
static void     _Parse_initNodeHash(struct Parse_Node_s *);
static void     _Parse_printError(const char *, const char *, int);
static void     _Parse_freeTree(struct Parse_Tree_s *);
static void     _Parse_freePartialTree(struct Parse_Tree_s *, int);
static void     _Parse_freeNode(struct Parse_Node_s *);
static void     _Parse_buildTranslationTable(void);
static void     _Parse_initTreeRoots(void);
static void     _Parse_mergeAnonChildren(struct Parse_Tree_s *, struct Parse_Tree_s *);
static void     _Parse_unlinkTbucket(struct Parse_Tree_s *);
static void     _Parse_unlinkTree(struct Parse_Tree_s *);
static int      _Parse_getoid(FILE *, struct Parse_Subid_s *, int);
static struct Parse_Node_s *_Parse_parseObjectid(FILE *, char *);
static int      _Parse_getTc(const char *, int, int *, struct Parse_EnumList_s **,
                       struct Parse_RangeList_s **, char **);
static int      _Parse_getTcIndex(const char *, int);
static struct Parse_EnumList_s *_Parse_parseEnumlist(FILE *, struct Parse_EnumList_s **);
static struct Parse_RangeList_s *_Parse_parseRanges(FILE * fp, struct Parse_RangeList_s **);
static struct Parse_Node_s *_Parse_parseAsntype(FILE *, char *, int *, char *);
static struct Parse_Node_s *_Parse_parseObjecttype(FILE *, char *);
static struct Parse_Node_s *_Parse_parseObjectgroup(FILE *, char *, int,
                                      struct Parse_Objgroup_s **);
static struct Parse_Node_s *_Parse_parseNotificationDefinition(FILE *, char *);
static struct Parse_Node_s *_Parse_parseTrapDefinition(FILE *, char *);
static struct Parse_Node_s *_Parse_parseCompliance(FILE *, char *);
static struct Parse_Node_s *_Parse_parseCapabilities(FILE *, char *);
static struct Parse_Node_s *_Parse_parseModuleIdentity(FILE *, char *);
static struct Parse_Node_s *_Parse_parseMacro(FILE *, char *);
static void     _Parse_parseImports(FILE *);
static struct Parse_Node_s * _Parse_parse(FILE *, struct Parse_Node_s *);

static int     _Parse_readModuleInternal(const char *);
static int     _Parse_readModuleReplacements(const char *);
static int     _Parse_readImportReplacements(const char *,
                                         struct Parse_ModuleImport_s *);

static void     _Parse_newModule(const char *, const char *);

static struct Parse_Node_s *_Parse_mergeParseObjectid(struct Parse_Node_s *, FILE *, char *);
static struct Parse_IndexList_s *_Parse_getIndexes(FILE * fp, struct Parse_IndexList_s **);
static struct Parse_VarbindList_s *_Parse_getVarbinds(FILE * fp, struct Parse_VarbindList_s **);
static void     _Parse_freeIndexes(struct Parse_IndexList_s **);
static void     _Parse_freeVarbinds(struct Parse_VarbindList_s **);
static void     _Parse_freeRanges(struct Parse_RangeList_s **);
static void     _Parse_freeEnums(struct Parse_EnumList_s **);
static struct Parse_RangeList_s *_Parse_copyRanges(struct Parse_RangeList_s *);
static struct Parse_EnumList_s *_Parse_copyEnums(struct Parse_EnumList_s *);

static u_int    _Parse_computeMatch(const char *search_base, const char *key);
...
static int _Parse_nameHash(const char *name)
{
    int             hash = 0;
    const char     *cp;
...
void Parse_initMibInternals(void)
{
     ...
     */
    memset(_parse_buckets, 0, sizeof(_parse_buckets));
    for (tp = _parse_tokens; tp->name; tp++) {
        tp->hash = _Parse_nameHash(tp->name);
        b = PARSE_BUCKET(tp->hash);
        if (_parse_buckets[b])
            tp->next = _parse_buckets[b];      /* BUG ??? */
        _parse_buckets[b] = tp;
    }
    ...
    max_modc = sizeof(_parse_moduleMap) / sizeof(_parse_moduleMap[0]) - 1;
    for (i = 0; i < max_modc; ++i)
        _parse_moduleMap[i].next = &(_parse_moduleMap[i + 1]);
    _parse_moduleMap[max_modc].next = NULL;
    _parse_moduleMapHead = _parse_moduleMap;

    memset(_parse_nbuckets, 0, sizeof(_parse_nbuckets));
    memset(_parse_tbuckets, 0, sizeof(_parse_tbuckets));
    memset(parse_tclist, 0, PARSE_MAXTC * sizeof(struct Parse_Tc_s));
    _Parse_buildTranslationTable();
    _Parse_initTreeRoots();          /* Set up initial roots */
    /*
...
static void _Parse_initNodeHash(struct Parse_Node_s *nodes)
{
    ...
    for (np = nodes; np;) {
        nextp = np->next;
        hash = PARSE_NBUCKET(_Parse_nameHash(np->parent));
        np->next = _parse_nbuckets[hash];
        _parse_nbuckets[hash] = np;
        np = nextp;
    }
}

static int      _parse_erroneousMibs = 0;

int Parse_getMibParseErrorCount(void)
{
    return _parse_erroneousMibs;
}


static void _Parse_printError(const char *str, const char *token, int type)
{
    _parse_erroneousMibs++;
...
static void
_Parse_printModuleNotFound(const char *cp)
{
    if (_parse_firstErrModule) {
        Logger_log(LOGGER_PRIORITY_ERR, "MIB search path: %s\n",
                           Mib_getMibDirectory());
        _parse_firstErrModule = 0;
    }
    if (!_parse_lastErrModule || strcmp(cp, _parse_lastErrModule))
        _Parse_printError("Cannot find module", cp, PARSE_CONTINUE);
    if (_parse_lastErrModule)
        free(_parse_lastErrModule);
    _parse_lastErrModule = strdup(cp);
}

static struct Parse_Node_s *
_Parse_allocNode(int modid)
{
...
static void
_Parse_unlinkTbucket(struct Parse_Tree_s *tp)
{
    int hash = PARSE_NBUCKET(_Parse_nameHash(tp->label));
    struct Parse_Tree_s    *otp = NULL, *ntp = _parse_tbuckets[hash];
    ...
    if (!ntp)
        ...
        _parse_tbuckets[hash] = tp->next;
}

static void
_Parse_unlinkTree(struct Parse_Tree_s *tp)
{
...
static void _Parse_freePartialTree(struct Parse_Tree_s *tp, int keep_label)
{
    if (!tp)
        return;
     ...
     */
    _Parse_freeEnums(&tp->enums);
    _Parse_freeRanges(&tp->ranges);
 ...
 */
static void
_Parse_freeTree(struct Parse_Tree_s *Tree)
{
    if (!Tree)
        return;

    _Parse_unlinkTbucket(Tree);
    _Parse_freePartialTree(Tree, FALSE);
    if (Tree->module_list != &Tree->modid)
        free(Tree->module_list);
    free(Tree);
}

static void _Parse_freeNode(struct Parse_Node_s *np)
{
    if (!np)
        return;

    _Parse_freeEnums(&np->enums);
    _Parse_freeRanges(&np->ranges);
    _Parse_freeIndexes(&np->indexes);
    _Parse_freeVarbinds(&np->varbinds);
    if (np->label)
        free(np->label);
    if (np->hint)
        free(np->hint);
...
static void _Parse_printRangeValue(FILE * fp, int type, struct Parse_RangeList_s * rp)
{
    switch (type) {
...
static int      _parse_translationTable[256];

static void
_Parse_buildTranslationTable(void)
{
    int             count;
    ...
    for (count = 0; count < 256; count++) {
        switch (count) {
        case PARSE_OBJID:
            _parse_translationTable[count] = PARSE_TYPE_OBJID;
            break;
        case PARSE_OCTETSTR:
            _parse_translationTable[count] = PARSE_TYPE_OCTETSTR;
            break;
        case PARSE_INTEGER:
            _parse_translationTable[count] = PARSE_TYPE_INTEGER;
            break;
        case PARSE_NETADDR:
            _parse_translationTable[count] = PARSE_TYPE_NETADDR;
            break;
        case PARSE_IPADDR:
            _parse_translationTable[count] = PARSE_TYPE_IPADDR;
            break;
        case PARSE_COUNTER:
            _parse_translationTable[count] = PARSE_TYPE_COUNTER;
            break;
        case PARSE_GAUGE:
            _parse_translationTable[count] = PARSE_TYPE_GAUGE;
            break;
        case PARSE_TIMETICKS:
            _parse_translationTable[count] = PARSE_TYPE_TIMETICKS;
            break;
        case PARSE_KW_OPAQUE:
            _parse_translationTable[count] = PARSE_TYPE_OPAQUE;
            break;
        case PARSE_NUL:
            _parse_translationTable[count] = PARSE_TYPE_NULL;
            break;
        case PARSE_COUNTER64:
            _parse_translationTable[count] = PARSE_TYPE_COUNTER64;
            break;
        case PARSE_BITSTRING:
            _parse_translationTable[count] = PARSE_TYPE_BITSTRING;
            break;
        case PARSE_NSAPADDRESS:
            _parse_translationTable[count] = PARSE_TYPE_NSAPADDRESS;
            break;
        case PARSE_INTEGER32:
            _parse_translationTable[count] = PARSE_TYPE_INTEGER32;
            break;
        case PARSE_UINTEGER32:
            _parse_translationTable[count] = PARSE_TYPE_UINTEGER;
            break;
        case PARSE_UNSIGNED32:
            _parse_translationTable[count] = PARSE_TYPE_UNSIGNED32;
            break;
        case PARSE_TRAPTYPE:
            _parse_translationTable[count] = PARSE_TYPE_TRAPTYPE;
            break;
        case PARSE_NOTIFTYPE:
            _parse_translationTable[count] = PARSE_TYPE_NOTIFTYPE;
            break;
        case PARSE_NOTIFGROUP:
            _parse_translationTable[count] = PARSE_TYPE_NOTIFGROUP;
            break;
        case PARSE_OBJGROUP:
            _parse_translationTable[count] = PARSE_TYPE_OBJGROUP;
            break;
        case PARSE_MODULEIDENTITY:
            _parse_translationTable[count] = PARSE_TYPE_MODID;
            break;
        case PARSE_OBJIDENTITY:
            _parse_translationTable[count] = PARSE_TYPE_OBJIDENTITY;
            break;
        case PARSE_AGENTCAP:
            _parse_translationTable[count] = PARSE_TYPE_AGENTCAP;
            break;
        case PARSE_COMPLIANCE:
            _parse_translationTable[count] = PARSE_TYPE_MODCOMP;
            break;
        default:
            _parse_translationTable[count] = PARSE_TYPE_OTHER;
            break;
        }
    }
}
...
_Parse_initTreeRoots(void)
{
    ...
    hash = PARSE_NBUCKET(_Parse_nameHash(tp->label));
    tp->next = _parse_tbuckets[hash];
    _parse_tbuckets[hash] = tp;
    lasttp = tp;
    _parse_rootImports[0].label = strdup(tp->label);
    _parse_rootImports[0].modid = base_modid;
    ...
    _parse_tbuckets[hash] = tp;
    lasttp = tp;
    _parse_rootImports[1].label = strdup(tp->label);
    _parse_rootImports[1].modid = base_modid;
    ...
    _parse_tbuckets[hash] = tp;
    lasttp = tp;
    _parse_rootImports[2].label = strdup(tp->label);
    _parse_rootImports[2].modid = base_modid;
...
struct Parse_Tree_s  * Parse_findTreeNode(const char *name, int modid)
{
    ...
    headtp = _parse_tbuckets[PARSE_NBUCKET(_Parse_nameHash(name))];
...
static          u_int
_Parse_computeMatch(const char *search_base, const char *key)
{
    int             rc;
    regex_t         parsetree;
    regmatch_t      pmatch;
                    ...
                    u_int * match)
{
    ...
    for (tp = tree_top; tp; tp = tp->next_peer) {
        if (!tp->reported && tp->label)
            new_match = _Parse_computeMatch(tp->label, pattrn);
        tp->reported = 1;
...
static void _Parse_mergeAnonChildren(struct Parse_Tree_s *tp1, struct Parse_Tree_s *tp2)
                /*
 ...
 */
static void _Parse_doSubtree(struct Parse_Tree_s *root, struct Parse_Node_s **nodes)
{
    ...
    tp = root;
    headp = &_parse_nbuckets[PARSE_NBUCKET(_Parse_nameHash(tp->label))];
    /*
    ...
    for (np = child_list; np; np = np->next) {
        ...
        if (tp) {
            if (!PARSE_LABEL_COMPARE(tp->label, np->label)) {
                ...
                if (DefaultStore_getBoolean(DsStorage_LIBRARY_ID,
                     ...
                     * Replace from node
                     */
                    _Parse_treeFromNode(tp, np);
                }
                /*
                 * Handle children
                 */
                _Parse_doSubtree(tp, nodes);
                continue;
            }
        ...
        if (anon_tp) {
            if (!strncmp(tp->label, PARSE_ANON, PARSE_ANON_LEN)) {
                 ...
                 */
                _Parse_mergeAnonChildren(tp, anon_tp);

                /*
                 * unlink and destroy tp
                 */
                _Parse_unlinkTree(tp);
                _Parse_freeTree(tp);
                 ...
                 */
                _Parse_mergeAnonChildren(anon_tp, tp);

                /*
                 * unlink anon_tp from the hash
                 */
                _Parse_unlinkTbucket(anon_tp);

                /*
                 * get rid of old contents of anon_tp
                 */
                _Parse_freePartialTree(anon_tp, FALSE);
                 ...
                 */
                hash = PARSE_NBUCKET(_Parse_nameHash(anon_tp->label));
                anon_tp->next = _parse_tbuckets[hash];
                _parse_tbuckets[hash] = anon_tp;

                /*
                 * unlink and destroy tp
                 */
                _Parse_unlinkTbucket(tp);
                _Parse_unlinkTree(tp);
                free(tp);
            } else {
                /*
...
static void _Parse_doLinkup(struct Parse_Module_s *mp, struct Parse_Node_s *np)
{
     ...
     */
    oldp = _parse_orphanNodes;
    do {
        for (i = 0; i < PARSE_NHASHSIZE; i++)
            for (onp = _parse_nbuckets[i]; onp; onp = onp->next) {
                struct Parse_Node_s    *op = NULL;
                int             hash = PARSE_NBUCKET(_Parse_nameHash(onp->label));
                np = _parse_nbuckets[hash];
                while (np) {
                    if (PARSE_LABEL_COMPARE(onp->label, np->parent)) {
                        ...
                        if (op)
                            ...
                            _parse_nbuckets[hash] = np->next;
                        np->next = _parse_orphanNodes;
                        _parse_orphanNodes = np;
                        op = NULL;
                        np = _parse_nbuckets[hash];
                    }
                }
            }
        newp = _parse_orphanNodes;
        more = 0;
        for (onp = _parse_orphanNodes; onp != oldp; onp = onp->next) {
            struct Parse_Node_s    *op = NULL;
            int             hash = PARSE_NBUCKET(_Parse_nameHash(onp->label));
            np = _parse_nbuckets[hash];
            while (np) {
     ...
     */
    for (np = _parse_orphanNodes; np && np->next; np = np->next);     /* find the end of the orphan list */
    for (i = 0; i < PARSE_NHASHSIZE; i++)
        if (_parse_nbuckets[i]) {
            if (_parse_orphanNodes)
                onp = np->next = _parse_nbuckets[i];
            else
                onp = _parse_orphanNodes = _parse_nbuckets[i];
            _parse_nbuckets[i] = NULL;
            while (onp) {
                Logger_log(LOGGER_PRIORITY_WARNING,
 ...
 * Returns 0 on error.
 */
static int _Parse_getoid(FILE * fp, struct Parse_Subid_s *id,   /* an array of subids */
       int length)
{                               /* the length of the array */
    ...
    for (count = 0; count < length; count++, id++) {
        ...
        if (type == PARSE_LABEL) {
            ...
            if (type == PARSE_LEFTPAREN) {
                ...
                if (type == PARSE_NUMBER) {
                    ...
                    if ((type =
                        ...
                        _Parse_printError("Expected a closing parenthesis",
                                    token, type);
                        return 0;
                    }
                } else {
                    _Parse_printError("Expected a number", token, type);
                    return 0;
                }
            } else {
                continue;
            ...
            _Parse_printError("Expected label or number", token, type);
            return 0;
        }
        type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
    }
    _Parse_printError("Too long OID", token, type);
    return 0;
}
/*
 ...
 */
static struct Parse_Node_s *
_Parse_objectid(FILE * fp, char *name)
{
    register int    count;
    ...
    if ((length = _Parse_getoid(fp, loid, 32)) == 0) {
        _Parse_printError("Bad object identifier", NULL, PARSE_CONTINUE);
        return NULL;
    }
    ...
    if (!op->label) {
        if (length == 1) {
            _Parse_printError("Attempt to define a root oid", name, PARSE_OBJECT);
            return NULL;
        }
        for (tp = parse_treeHead; tp; tp = tp->next_peer)
    ...
    for (count = 0, op = loid, nop = loid + 1; count < (length - 1);
        ...
        if (op->label && (nop->label || (nop->subid != -1))) {
            ...
            if (nop->subid != -1)
                ...
                _Parse_printError("Warning: This entry is pretty silly",
...
static int _Parse_getTc(const char *descriptor,
       int modid,
       int *tc_index,
       struct Parse_EnumList_s **ep, struct Parse_RangeList_s **rp, char **hint)
{
    ...
    i = _Parse_getTcIndex(descriptor, modid);
    if (tc_index)
        *tc_index = i;
    if (i != -1) {
        tcp = &parse_tclist[i];
        if (ep) {
            _Parse_freeEnums(ep);
            *ep = _Parse_copyEnums(tcp->enums);
        }
        if (rp) {
            _Parse_freeRanges(rp);
            *rp = _Parse_copyRanges(tcp->ranges);
        }
        if (hint) {
            if (*hint)
                free(*hint);
 ...
 */
static int
_Parse_getTcIndex(const char *descriptor, int modid)
{
    int             i;
    struct Parse_Tc_s      *tcp;
    ...
    for (mp = _parse_moduleHead; mp; mp = mp->next)
        if (mp->modid == modid)
            break;
    if (mp)
        for (i = 0, mip = mp->imports; i < mp->no_imports; ++i, ++mip) {
...
_Parse_parseEnumlist(FILE * fp, struct Parse_EnumList_s **retp)
{
    register int    type;
    char            token[PARSE_MAXTOKEN];
    struct Parse_EnumList_s *ep = NULL, **epp = &ep;

    _Parse_freeEnums(retp);
    ...
    while ((type = Parse_getToken(fp, token, PARSE_MAXTOKEN)) != PARSE_ENDOFFILE) {
        ...
        if (type == PARSE_LABEL || type == PARSE_DEPRECATED) {
            ...
            *epp =
            ...
            type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
            if (type != PARSE_NUMBER) {
                _Parse_printError("Expected integer", token, type);
                return NULL;
            }
...
_Parse_parseRanges(FILE * fp, struct Parse_RangeList_s **retp)
{
    int             low, high;
    ...
    if (nexttype == PARSE_SIZE) {
        ...
        if (nexttype != PARSE_LEFTPAREN)
            _Parse_printError("Expected \"(\" after SIZE", nexttoken, nexttype);
    }
    ...
    if (nexttype != PARSE_RIGHTPAREN)
        _Parse_printError("Expected \")\"", nexttoken, nexttype);
 ...
 */
static struct Parse_Node_s *
_Parse_parseAsntype(FILE * fp, char *name, int *ntype, char *ntoken)
{
    int             type, i;
    char            token[PARSE_MAXTOKEN];
    ...
    if (type == PARSE_SEQUENCE || type == PARSE_CHOICE) {
        ...
        *ntype = Parse_getToken(fp, ntoken, PARSE_MAXTOKEN);
        if (*ntype == PARSE_LEFTPAREN) {
            ...
            case PARSE_OCTETSTR:
                *ntype = Parse_getToken(fp, ntoken, PARSE_MAXTOKEN);
                if (*ntype != PARSE_SIZE) {
                    _Parse_printError("Expected SIZE", ntoken, *ntype);
                    return NULL;
                }
                size = 1;
                *ntype = Parse_getToken(fp, ntoken, PARSE_MAXTOKEN);
                if (*ntype != PARSE_LEFTPAREN) {
                    _Parse_printError("Expected \"(\" after SIZE", ntoken,
                                *ntype);
                    return NULL;
                }
                /*
                ...
                *ntype = Parse_getToken(fp, ntoken, PARSE_MAXTOKEN);
                do {
                    if (*ntype != PARSE_NUMBER)
                        _Parse_printError("Expected NUMBER", ntoken, *ntype);
                    *ntype = Parse_getToken(fp, ntoken, PARSE_MAXTOKEN);
                ...
                if (*ntype != PARSE_RIGHTPAREN) {
                    _Parse_printError("Expected \")\"", ntoken, *ntype);
                    return NULL;
                }
                *ntype = Parse_getToken(fp, ntoken, PARSE_MAXTOKEN);
                if (size) {
                    if (*ntype != PARSE_RIGHTPAREN) {
                        _Parse_printError("Expected \")\" to terminate SIZE",
                                    ntoken, *ntype);
                        return NULL;
                    }
                    *ntype = Parse_getToken(fp, ntoken, PARSE_MAXTOKEN);
            ...
            while (type != PARSE_SYNTAX && type != PARSE_ENDOFFILE) {
                if (type == PARSE_DISPLAYHINT) {
                    ...
                    if (type != PARSE_QUOTESTRING)
                        _Parse_printError("DISPLAY-HINT must be string", token,
                                    type);
                    else
                        hint = strdup(token);
                    ...
                    if (type != PARSE_QUOTESTRING)
                        _Parse_printError("DESCRIPTION must be string", token,
                                    type);
                    else
        ...
        if (type == PARSE_LABEL) {
            type = _Parse_getTc(token, _parse_currentModule, NULL, NULL, NULL, NULL);
        }
        ...
        tcp = &parse_tclist[i];
        tcp->modid = _parse_currentModule;
        ...
        *ntype = Parse_getToken(fp, ntoken, PARSE_MAXTOKEN);
        if (*ntype == PARSE_LEFTPAREN) {
            tcp->ranges = _Parse_parseRanges(fp, &tcp->ranges);
            *ntype = Parse_getToken(fp, ntoken, PARSE_MAXTOKEN);
        } else if (*ntype == PARSE_LEFTBRACKET) {
            /*
             * if there is an enumeration list, parse it
             */
            tcp->enums = _Parse_parseEnumlist(fp, &tcp->enums);
            *ntype = Parse_getToken(fp, ntoken, PARSE_MAXTOKEN);
        }
        return NULL;
    }
 ...
 */
static struct Parse_Node_s *
_Parse_parseObjecttype(FILE * fp, char *name)
{
    register int    type;
    char            token[PARSE_MAXTOKEN];
    char            nexttoken[PARSE_MAXTOKEN];
    ...
    if (type != PARSE_SYNTAX) {
        _Parse_printError("Bad format for OBJECT-TYPE", token, type);
        return NULL;
    }
    np = _Parse_allocNode(_parse_currentModule);
    if (np == NULL)
        return (NULL);
    type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
    if (type == PARSE_OBJECT) {
    ...
    case PARSE_LABEL:
        if (nexttype == PARSE_LEFTBRACKET) {
             ...
             * if there is an enumeration list, parse it
             */
            np->enums = _Parse_parseEnumlist(fp, &np->enums);
            nexttype = Parse_getToken(fp, nexttoken, PARSE_MAXTOKEN);
        } else if (nexttype == PARSE_LEFTPAREN) {
            /*
             * if there is a range list, parse it
             */
            np->ranges = _Parse_parseRanges(fp, &np->ranges);
            nexttype = Parse_getToken(fp, nexttoken, PARSE_MAXTOKEN);
        }
        break;
    case PARSE_OCTETSTR:
    case PARSE_KW_OPAQUE:
        ...
        if (nexttype == PARSE_LEFTPAREN) {
            ...
            _Parse_printError("Bad SIZE syntax", token, type);
            _Parse_freeNode(np);
            return NULL;
        }
        break;
    ...
    case PARSE_COUNTER64:
        break;
    default:
        _Parse_printError("Bad syntax", token, type);
        _Parse_freeNode(np);
        return NULL;
    }
    if (nexttype == PARSE_UNITS) {
        type = Parse_getToken(fp, quoted_string_buffer, PARSE_MAXQUOTESTR);
        if (type != PARSE_QUOTESTRING) {
            _Parse_printError("Bad UNITS", quoted_string_buffer, type);
            _Parse_freeNode(np);
            return NULL;
        }
    ...
    if (nexttype != PARSE_ACCESS) {
        _Parse_printError("Should be ACCESS", nexttoken, nexttype);
        _Parse_freeNode(np);
        return NULL;
    }
    type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
    if (type != PARSE_READONLY && type != PARSE_READWRITE && type != PARSE_WRITEONLY
        ...
        _Parse_printError("Bad ACCESS type", token, type);
        _Parse_freeNode(np);
        return NULL;
    }
    np->access = type;
    ...
    if (type != PARSE_STATUS) {
        _Parse_printError("Should be STATUS", token, type);
        _Parse_freeNode(np);
        return NULL;
    }
    type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
    if (type != PARSE_MANDATORY && type != PARSE_CURRENT && type != PARSE_KW_OPTIONAL &&
        ...
        _Parse_printError("Bad STATUS", token, type);
        _Parse_freeNode(np);
        return NULL;
    }
    np->status = type;
    ...
    while (type != PARSE_EQUALS && type != PARSE_ENDOFFILE) {
        ...
        case PARSE_REFERENCE:
            type = Parse_getToken(fp, quoted_string_buffer, PARSE_MAXQUOTESTR);
            if (type != PARSE_QUOTESTRING) {
                _Parse_printError("Bad REFERENCE", quoted_string_buffer, type);
                _Parse_freeNode(np);
                return NULL;
            }
        ...
        case PARSE_INDEX:
            if (np->augments) {
                _Parse_printError("Cannot have both INDEX and AUGMENTS", token,
                            type);
                _Parse_freeNode(np);
                return NULL;
            }
            np->indexes = _Parse_getIndexes(fp, &np->indexes);
            if (np->indexes == NULL) {
                _Parse_printError("Bad INDEX list", token, type);
                _Parse_freeNode(np);
                return NULL;
            }
            break;
        case PARSE_AUGMENTS:
            ...
            np->indexes = _Parse_getIndexes(fp, &np->indexes);
            if (np->indexes == NULL) {
                _Parse_printError("Bad AUGMENTS list", token, type);
                _Parse_freeNode(np);
                return NULL;
            }
        ...
        case PARSE_DEFVAL:
            ...
            if (type != PARSE_LEFTBRACKET) {
                _Parse_printError("Bad DEFAULTVALUE", quoted_string_buffer,
                            type);
                _Parse_freeNode(np);
                return NULL;
            }
        ...
        case PARSE_NUM_ENTRIES:
            if (_Parse_tossObjectIdentifier(fp) != PARSE_OBJID) {
                _Parse_printError("Bad Object Identifier", token, type);
                _Parse_freeNode(np);
                return NULL;
            }
            break;
        ...
        default:
            _Parse_printError("Bad format of optional clauses", token, type);
            _Parse_freeNode(np);
            return NULL;
    ...
    if (type != PARSE_EQUALS) {
        _Parse_printError("Bad format", token, type);
        _Parse_freeNode(np);
        return NULL;
    }
    return _Parse_mergeParseObjectid(np, fp, name);
}
 ...
 *   - WJH 10/96
 */
static struct Parse_Node_s * _Parse_parseObjectgroup(FILE * fp, char *name, int what, struct Parse_Objgroup_s **ol)
{
    int             type;
    char            token[PARSE_MAXTOKEN];
    char            quoted_string_buffer[PARSE_MAXQUOTESTR];
    ...
    if (type == what) {
        ...
        do {
            ...
            type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
            if (type != PARSE_LABEL) {
                _Parse_printError("Bad identifier", token, type);
                goto goto_skip;
            }
            o = (struct Parse_Objgroup_s *) malloc(sizeof(struct Parse_Objgroup_s));
            if (!o) {
                _Parse_printError("Resource failure", token, type);
                goto goto_skip;
            }
        ...
        if (type != PARSE_RIGHTBRACKET) {
            _Parse_printError("Expected \"}\" after list", token, type);
            goto goto_skip;
        }
        type = Parse_getToken(fp, token, type);
    }
    if (type != PARSE_STATUS) {
        _Parse_printError("Expected STATUS", token, type);
        goto goto_skip;
    }
    type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
    if (type != PARSE_CURRENT && type != PARSE_DEPRECATED && type != PARSE_OBSOLETE) {
        _Parse_printError("Bad STATUS value", token, type);
        goto goto_skip;
    }
    type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
    if (type != PARSE_DESCRIPTION) {
        _Parse_printError("Expected DESCRIPTION", token, type);
        goto goto_skip;
    }
    type = Parse_getToken(fp, quoted_string_buffer, PARSE_MAXQUOTESTR);
    if (type != PARSE_QUOTESTRING) {
 ...
 */
static struct Parse_Node_s *
_Parse_parseNotificationDefinition(FILE * fp, char *name)
{
    register int    type;
    char            token[PARSE_MAXTOKEN];
    char            quoted_string_buffer[PARSE_MAXQUOTESTR];
    ...
    while (type != PARSE_EQUALS && type != PARSE_ENDOFFILE) {
        ...
        case PARSE_REFERENCE:
            ...
            break;
        case PARSE_OBJECTS:
            np->varbinds = _Parse_getVarbinds(fp, &np->varbinds);
            if (!np->varbinds) {
                _Parse_printError("Bad OBJECTS list", token, type);
                _Parse_freeNode(np);
                return NULL;
            }
            break;
 ...
 */
static struct Parse_Node_s *
_Parse_parseTrapDefinition(FILE * fp, char *name)
{
    register int    type;
    char            token[PARSE_MAXTOKEN];
    char            quoted_string_buffer[PARSE_MAXQUOTESTR];
    ...
    while (type != PARSE_EQUALS && type != PARSE_ENDOFFILE) {
        ...
        case PARSE_ENTERPRISE:
            ...
            if (type == PARSE_LEFTBRACKET) {
                type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
                if (type != PARSE_LABEL) {
                    _Parse_printError("Bad Trap Format", token, type);
                    _Parse_freeNode(np);
                    return NULL;
                }
        ...
        case PARSE_VARIABLES:
            np->varbinds = _Parse_getVarbinds(fp, &np->varbinds);
            if (!np->varbinds) {
                _Parse_printError("Bad VARIABLES list", token, type);
                _Parse_freeNode(np);
                return NULL;
            }
            break;
    ...
    np->next = _Parse_allocNode(_parse_currentModule);
    if (np->next == NULL) {
        _Parse_freeNode(np);
        return (NULL);
    }
 ...
 */
static int
_Parse_eatSyntax(FILE * fp, char *token, int maxtoken)
{
    int             type, nexttype;
    struct Parse_Node_s    *np = _Parse_allocNode(_parse_currentModule);
    char            nexttoken[PARSE_MAXTOKEN];
...
static int
_Parse_complianceLookup(const char *name, int modid)
{
    if (modid == -1) {
...
static struct Parse_Node_s *
_Parse_parseCompliance(FILE * fp, char *name)
{
    int             type;
    char            token[PARSE_MAXTOKEN];
    char            quoted_string_buffer[PARSE_MAXQUOTESTR];
    ...
    if (type != PARSE_MODULE) {
        _Parse_printError("Expected MODULE", token, type);
        goto goto_skip;
    }
    while (type == PARSE_MODULE) {
        ...
        type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
        if (type == PARSE_LABEL
            && strcmp(token, Parse_moduleName(_parse_currentModule, modname))) {
            modid = _Parse_readModuleInternal(token);
            if (modid != PARSE_MODULE_LOADED_OK
                && modid != PARSE_MODULE_ALREADY_LOADED) {
                _Parse_printError("Unknown module", token, type);
                goto goto_skip;
            }
        ...
        if (type == PARSE_MANDATORYGROUPS) {
            ...
            do {
                type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
                if (type != PARSE_LABEL) {
                    _Parse_printError("Bad group name", token, type);
                    goto goto_skip;
                }
                if (!_Parse_complianceLookup(token, modid))
                    _Parse_printError("Unknown group", token, type);
                type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
            } while (type == PARSE_COMMA);
            if (type != PARSE_RIGHTBRACKET) {
                _Parse_printError("Expected \"}\"", token, type);
        ...
        while (type == PARSE_GROUP || type == PARSE_OBJECT) {
            if (type == PARSE_GROUP) {
                ...
                type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
                if (type != PARSE_LABEL) {
                    _Parse_printError("Bad object name", token, type);
                    goto goto_skip;
                }
                if (!_Parse_complianceLookup(token, modid))
                    _Parse_printError("Unknown group", token, type);
                type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
                if (type == PARSE_SYNTAX)
                    type = _Parse_eatSyntax(fp, token, PARSE_MAXTOKEN);
                ...
                if (type == PARSE_MINACCESS) {
                    ...
                    if (type != PARSE_NOACCESS && type != PARSE_ACCNOTIFY
                        ...
                        _Parse_printError("Bad MIN-ACCESS spec", token, type);
                        goto goto_skip;
                    }
                    type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
                }
            ...
            type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
            if (type != PARSE_QUOTESTRING) {
                _Parse_printError("Bad DESCRIPTION", token, type);
                goto goto_skip;
            }
            type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
        }
 ...
 */
static struct Parse_Node_s *
_Parse_parseCapabilities(FILE * fp, char *name)
{
    int             type;
    char            token[PARSE_MAXTOKEN];
    char            quoted_string_buffer[PARSE_MAXQUOTESTR];
    ...
    if (type != PARSE_PRODREL) {
        _Parse_printError("Expected PRODUCT-RELEASE", token, type);
        goto goto_skip;
    }
    type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
    if (type != PARSE_QUOTESTRING) {
        _Parse_printError("Expected STRING after PRODUCT-RELEASE", token, type);
        goto goto_skip;
    }
    type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
    if (type != PARSE_STATUS) {
    ...
    if (type != PARSE_CURRENT && type != PARSE_OBSOLETE) {
        _Parse_printError("STATUS should be current or obsolete", token, type);
        goto goto_skip;
    }
    type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
    if (type != PARSE_DESCRIPTION) {
    ...
    while (type == PARSE_SUPPORTS) {
        ...
        type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
        if (type != PARSE_LABEL) {
            _Parse_printError("Bad module name", token, type);
            goto goto_skip;
        }
        modid = _Parse_readModuleInternal(token);
        if (modid != PARSE_MODULE_LOADED_OK && modid != PARSE_MODULE_ALREADY_LOADED) {
            _Parse_printError("Module not found", token, type);
            goto goto_skip;
        }
        modid = Parse_whichModule(token);
        type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
        if (type != PARSE_INCLUDES) {
            _Parse_printError("Expected INCLUDES", token, type);
            goto goto_skip;
        }
        type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
        if (type != PARSE_LEFTBRACKET) {
        ...
        do {
            type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
            if (type != PARSE_LABEL) {
                _Parse_printError("Expected group name", token, type);
                goto goto_skip;
            }
            tp = Parse_findTreeNode(token, modid);
            if (!tp)
                _Parse_printError("Group not found in module", token, type);
            type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
        } while (type == PARSE_COMMA);
        if (type != PARSE_RIGHTBRACKET) {
            _Parse_printError("Expected \"}\" after group list", token, type);
            goto goto_skip;
        }
        type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
        while (type == PARSE_VARIATION) {
            ...
            tp = Parse_findTreeNode(token, modid);
            if (!tp)
                _Parse_printError("Object not found in module", token, type);
            type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
            if (type == PARSE_SYNTAX) {
                type = _Parse_eatSyntax(fp, token, PARSE_MAXTOKEN);
            }
            ...
            if (type == PARSE_ACCESS) {
                ...
                if (type != PARSE_ACCNOTIFY && type != PARSE_READONLY
                    ...
                    _Parse_printError("Bad ACCESS", token, type);
                    goto goto_skip;
                }
                type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
            }
            if (type == PARSE_CREATEREQ) {
                ...
                do {
                    type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
                    if (type != PARSE_LABEL) {
                        _Parse_printError("Bad object name in list", token,
                                    type);
                        goto goto_skip;
                    }
                    type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
            ...
            if (type == PARSE_DEFVAL) {
                ...
                type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
                if (type != PARSE_LEFTBRACKET) {
                    _Parse_printError("Expected \"{\" after DEFVAL", token,
                                type);
                    goto goto_skip;
                }
                do {
                    ...
                    if (type == PARSE_LEFTBRACKET)
                        ...
                        level--;
                         ...
                         && type != PARSE_ENDOFFILE);
                if (type != PARSE_RIGHTBRACKET) {
                    _Parse_printError("Missing \"}\" after DEFVAL", token, type);
                    goto goto_skip;
                }
                type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
            }
 ...
 */
static void
_Parse_checkUtc(const char *utc)
{
    int             len, year, month, day, hour, minute;

    len = strlen(utc);
    if (utc[len - 1] != 'Z' && utc[len - 1] != 'z') {
        _Parse_printError("Timestamp should end with Z", utc, PARSE_QUOTESTRING);
        return;
    }
    if (len == 11) {
        len =
        ...
        _Parse_printError("Bad timestamp format (11 or 13 characters)",
                    utc, PARSE_QUOTESTRING);
        return;
    }
    if (len != 5) {
        _Parse_printError("Bad timestamp format", utc, PARSE_QUOTESTRING);
        return;
    }
    if (month < 1 || month > 12)
        _Parse_printError("Bad month in timestamp", utc, PARSE_QUOTESTRING);
    if (day < 1 || day > 31)
        _Parse_printError("Bad day in timestamp", utc, PARSE_QUOTESTRING);
    if (hour < 0 || hour > 23)
        _Parse_printError("Bad hour in timestamp", utc, PARSE_QUOTESTRING);
    if (minute < 0 || minute > 59)
        _Parse_printError("Bad minute in timestamp", utc, PARSE_QUOTESTRING);
}

static struct Parse_Node_s *
_Parse_parseModuleIdentity(FILE * fp, char *name)
{
    register int    type;
    char            token[PARSE_MAXTOKEN];
    char            quoted_string_buffer[PARSE_MAXQUOTESTR];
    ...
    if (type != PARSE_LASTUPDATED) {
        _Parse_printError("Expected LAST-UPDATED", token, type);
        goto goto_skip;
    }
    type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
    if (type != PARSE_QUOTESTRING) {
        _Parse_printError("Need STRING for LAST-UPDATED", token, type);
        goto goto_skip;
    }
    _Parse_checkUtc(token);
    type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
    if (type != PARSE_ORGANIZATION) {
        _Parse_printError("Expected ORGANIZATION", token, type);
        goto goto_skip;
    }
    type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
    if (type != PARSE_QUOTESTRING) {
        _Parse_printError("Bad ORGANIZATION", token, type);
        goto goto_skip;
    }
    type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
    if (type != PARSE_CONTACTINFO) {
        _Parse_printError("Expected CONTACT-INFO", token, type);
        goto goto_skip;
    }
    type = Parse_getToken(fp, quoted_string_buffer, PARSE_MAXQUOTESTR);
    if (type != PARSE_QUOTESTRING) {
        _Parse_printError("Bad CONTACT-INFO", quoted_string_buffer, type);
        goto goto_skip;
    }
    type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
    if (type != PARSE_DESCRIPTION) {
    ...
    while (type == PARSE_REVISION) {
        type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
        if (type != PARSE_QUOTESTRING) {
            _Parse_printError("Bad REVISION", token, type);
            goto goto_skip;
        }
        _Parse_checkUtc(token);
        type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
 ...
 */
static struct Parse_Node_s *
_Parse_parseMacro(FILE * fp, char *name)
{
    register int    type;
    char            token[PARSE_MAXTOKEN];
    struct Parse_Node_s    *np;
...
#define MAX_IMPORTS	256
    ...
    while (type != PARSE_SEMI && type != PARSE_ENDOFFILE) {
        if (type == PARSE_LABEL) {
            if (import_count == MAX_IMPORTS) {
                _Parse_printError("Too many imported symbols", token, type);
                do {
                    type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
                } while (type != PARSE_SEMI && type != PARSE_ENDOFFILE);
                return;
             ...
             */
            if (_Parse_readModuleInternal(token) == PARSE_MODULE_NOT_FOUND) {
        int found = 0;
                for (; old_i < import_count; ++old_i) {
                    found += _Parse_readImportReplacements(token, &import_list[old_i]);
                }
        if (!found)
            _Parse_printModuleNotFound(token);
            }
        }
        type = Parse_getToken(fp, token, PARSE_MAXTOKEN);
    }
     ...
     */
    for (mp = _parse_moduleHead; mp; mp = mp->next)
        if (mp->modid == _parse_currentModule) {
            if (import_count == 0)
                return;
            if (mp->imports && (mp->imports != _parse_rootImports)) {
                /*
     ...
     */
    _Parse_printModuleNotFound(Parse_moduleName(_parse_currentModule, modbuf));
    return;
}
...
static void
_Parse_dumpModuleList(void)
{
    struct Parse_Module_s  *mp = _parse_moduleHead;
                       ...
                       const char *tag, int len)
{
    ...
    mcp->next = _parse_moduleMapHead;
    _parse_moduleMapHead = mcp;
}

static int
_Parse_readModuleReplacements(const char *name)
{
    struct Parse_ModuleCompatability_s *mcp;
...
static int
_Parse_readImportReplacements(const char *old_module_name,
                         struct Parse_ModuleImport_s *identifier)
{
     ...
     */
    return _Parse_readModuleReplacements(old_module_name);
}
 ...
 */
static int
_Parse_readModuleInternal(const char *name)
{
    struct Parse_Module_s  *mp;
    FILE           *fp;
    struct Parse_Node_s    *np;
    ...
    for (mp = _parse_moduleHead; mp; mp = mp->next)
        if (!PARSE_LABEL_COMPARE(mp->name, name)) {
            ...
            int             oldModule = _parse_currentModule;
            ...
            parse_file = mp->file;
            parse_mibLine = 1;
            _parse_currentModule = mp->modid;
            /*
             * Parse the file
             */
            np = _Parse_parse(fp, NULL);
            funlockfile(fp);
            fclose(fp);
            parse_file = oldFile;
            parse_mibLine = oldLine;
            _parse_currentModule = oldModule;
...
Parse_adoptOrphans(void)
{
    ...
    if (!_parse_orphanNodes)
        return;
    _Parse_initNodeHash(_parse_orphanNodes);
    _parse_orphanNodes = NULL;

    while (adopted) {
        adopted = 0;
        for (i = 0; i < PARSE_NHASHSIZE; i++)
            if (_parse_nbuckets[i]) {
                for (np = _parse_nbuckets[i]; np != NULL; np = np->next) {
            ...
            adopted = 1;
                        ...
                        if(NULL == onp) { /* not in the list */
                            np = _parse_nbuckets[i]; /* start over */
                        }
            }
        }
            }
...
{
    int status = 0;
    status = _Parse_readModuleInternal(name);

    if (status == PARSE_MODULE_NOT_FOUND) {
        if (!_Parse_readModuleReplacements(name))
            _Parse_printModuleNotFound(name);
    ...
    for (tp = tree_top; tp; tp = next) {
        ...
        if (tp->number_modules == 0) {
            ...
            if (tp->child_list == NULL) {
                ...
                _Parse_freePartialTree(tp, TRUE);
            }
        }
    }
}
...
void Parse_unloadAllMibs(void)
{
    ...
    for (mcp = _parse_moduleMapHead; mcp; mcp = _parse_moduleMapHead) {
    ...
    for (mp = _parse_moduleHead; mp; mp = _parse_moduleHead) {
    ...
    for (i = 0, ptc = parse_tclist; i < PARSE_MAXTC; i++, ptc++) {
        if (ptc->type == 0)
            continue;
        _Parse_freeEnums(&ptc->enums);
        _Parse_freeRanges(&ptc->ranges);
        free(ptc->descriptor);
        if (ptc->hint)
            free(ptc->hint);
        if (ptc->description)
...
static void
_Parse_newModule(const char *name, const char *file)
{
    struct Parse_Module_s  *mp;
    ...
    mp->modid = _parse_maxModule;
    ++_parse_maxModule;

    mp->next = _parse_moduleHead;     /* Or add to the *end* of the list? */
    _parse_moduleHead = mp;
}


static void
_Parse_scanObjlist(struct Parse_Node_s *root, struct Parse_Module_s *mp, struct Parse_Objgroup_s *list, const char *error)
{
    ...
    while (list) {
        ...
        _Parse_printError(error, gp->name, PARSE_QUOTESTRING);
        }
        }
        free(gp->name);
        free(gp);
 ...
 */
static struct Parse_Node_s *
_Parse_parse(FILE * fp, struct Parse_Node_s *root)
{
...
#define IN_MIB                2
    ...
    while (type != PARSE_ENDOFFILE) {
        ...
        case PARSE_END:
            if (state != IN_MIB) {
                _Parse_printError("Error, END before start of MIB", NULL, type);
                parse_gMibError = PARSE_MODULE_SYNTAX_ERROR;
                return NULL;
            } else {
                struct Parse_Module_s  *mp;
                ...
                for (mp = _parse_moduleHead; mp; mp = mp->next)
                    if (mp->modid == _parse_currentModule)
                        break;
                _Parse_scanObjlist(root, mp, parse_objgroups, "Undefined OBJECT-GROUP");
                _Parse_scanObjlist(root, mp, parse_objects, "Undefined OBJECT");
                _Parse_scanObjlist(root, mp, parse_notifs, "Undefined NOTIFICATION");
                parse_objgroups = oldgroups;
                parse_objects = oldobjects;
                parse_notifs = oldnotifs;
                _Parse_doLinkup(mp, root);
                np = root = NULL;
            }
            state = BETWEEN_MIBS;
        ...
        default:
            ...
            nnp = NULL;
            if (type == PARSE_MACRO) {
                nnp = _Parse_parseMacro(fp, name);
                if (nnp == NULL) {
                    _Parse_printError("Bad parse of MACRO", NULL, type);
                    parse_gMibError = PARSE_MODULE_SYNTAX_ERROR;
                    /*
                     * return NULL;
                     */
                }
                _Parse_freeNode(nnp); /* IGNORE */
                nnp = NULL;
            } else
        ...
        case PARSE_DEFINITIONS:
            if (state != BETWEEN_MIBS) {
                _Parse_printError("Error, nested MIBS", NULL, type);
                parse_gMibError = PARSE_MODULE_SYNTAX_ERROR;
                return NULL;
            }
            state = IN_MIB;
            _parse_currentModule = Parse_whichModule(name);
            ...
            parse_notifs = NULL;
            if (_parse_currentModule == -1) {
                _Parse_newModule(name, parse_file);
                _parse_currentModule = Parse_whichModule(name);
            }
            DEBUG_MSGTL(("parse-mibs", "Parsing MIB: %d %s\n",
                        _parse_currentModule, name));
            while ((type = Parse_getToken(fp, token, PARSE_MAXTOKEN)) != PARSE_ENDOFFILE)
                if (type == PARSE_BEGIN)
                    break;
            break;
        case PARSE_OBJTYPE:
            nnp = _Parse_parseObjecttype(fp, name);
            if (nnp == NULL) {
                _Parse_printError("Bad parse of OBJECT-TYPE", NULL, type);
                parse_gMibError = PARSE_MODULE_SYNTAX_ERROR;
                return NULL;
            }
            break;
        case PARSE_OBJGROUP:
            nnp = _Parse_parseObjectgroup(fp, name, PARSE_OBJECTS, &parse_objects);
            if (nnp == NULL) {
                _Parse_printError("Bad parse of OBJECT-GROUP", NULL, type);
                parse_gMibError = PARSE_MODULE_SYNTAX_ERROR;
                return NULL;
            }
            break;
        case PARSE_NOTIFGROUP:
            nnp = _Parse_parseObjectgroup(fp, name, PARSE_NOTIFICATIONS, &parse_notifs);
            if (nnp == NULL) {
                _Parse_printError("Bad parse of NOTIFICATION-GROUP", NULL, type);
                parse_gMibError = PARSE_MODULE_SYNTAX_ERROR;
                return NULL;
            }
            break;
        case PARSE_TRAPTYPE:
            nnp = _Parse_parseTrapDefinition(fp, name);
            if (nnp == NULL) {
                _Parse_printError("Bad parse of TRAP-TYPE", NULL, type);
                parse_gMibError = PARSE_MODULE_SYNTAX_ERROR;
                return NULL;
            }
            break;
        case PARSE_NOTIFTYPE:
            nnp = _Parse_parseNotificationDefinition(fp, name);
            if (nnp == NULL) {
                _Parse_printError("Bad parse of NOTIFICATION-TYPE", NULL, type);
                parse_gMibError = PARSE_MODULE_SYNTAX_ERROR;
                return NULL;
            }
            break;
        case PARSE_COMPLIANCE:
            nnp = _Parse_parseCompliance(fp, name);
            if (nnp == NULL) {
                _Parse_printError("Bad parse of MODULE-COMPLIANCE", NULL, type);
                parse_gMibError = PARSE_MODULE_SYNTAX_ERROR;
                return NULL;
            }
            break;
        case PARSE_AGENTCAP:
            nnp = _Parse_parseCapabilities(fp, name);
            if (nnp == NULL) {
                _Parse_printError("Bad parse of AGENT-CAPABILITIES", NULL, type);
                parse_gMibError = PARSE_MODULE_SYNTAX_ERROR;
                return NULL;
            }
            break;
        case PARSE_MACRO:
            ...
            break;
        case PARSE_MODULEIDENTITY:
            nnp = _Parse_parseModuleIdentity(fp, name);
            if (nnp == NULL) {
                _Parse_printError("Bad parse of MODULE-IDENTITY", NULL, type);
                parse_gMibError = PARSE_MODULE_SYNTAX_ERROR;
                return NULL;
            }
            break;
        case PARSE_OBJIDENTITY:
            nnp = _Parse_parseObjectgroup(fp, name, PARSE_OBJECTS, &parse_objects);
            if (nnp == NULL) {
                _Parse_printError("Bad parse of OBJECT-IDENTITY", NULL, type);
                parse_gMibError = PARSE_MODULE_SYNTAX_ERROR;
                return NULL;
            }
            break;
        case PARSE_OBJECT:
            ...
            nnp = _Parse_objectid(fp, name);
            if (nnp == NULL) {
                _Parse_printError("Bad parse of OBJECT IDENTIFIER", NULL, type);
                parse_gMibError = PARSE_MODULE_SYNTAX_ERROR;
                return NULL;
            }
            break;
        case PARSE_EQUALS:
            nnp = _Parse_parseAsntype(fp, name, &type, token);
            lasttype = PARSE_CONTINUE;
            break;
        case PARSE_ENDOFFILE:
            break;
        default:
            _Parse_printError("Bad operator", token, type);
            parse_gMibError = PARSE_MODULE_SYNTAX_ERROR;
            return NULL;
        }
        if (nnp) {
 ...
 */
static int _Parse_getc(FILE *stream)
{
...
Parse_getToken(FILE * fp, char *token, int maxtlen)
{
    ...
    *cp = '\0';
    ...
    case '"':
        return _Parse_parseQuoteString(fp, token, maxtlen);
    case '\'':                 /* binary or hex constant */
        seenSymbols = bdigits;
        while ((ch = _Parse_getc(fp)) != EOF && ch != '\'') {
            switch (seenSymbols) {
            case bdigits:
                if (ch == '0' || ch == '1')
                    break;
    ...
    default:
         ...
         */
        if (!_Parse_isLabelchar(ch))
            return PARSE_LABEL;
        hash += tolower(ch);
      more:
        while (_Parse_isLabelchar(ch_next = _Parse_getc(fp))) {
        ...
        *cp = '\0';
        ...
        if (too_long)
            _Parse_printError("Warning: token too long", token, PARSE_CONTINUE);
        for (tp = _parse_buckets[PARSE_BUCKET(hash)]; tp; tp = tp->next) {
        ...
        if (tp) {
            if (tp->token != PARSE_CONTINUE)
                return (tp->token);
            while (isspace((ch_next = _Parse_getc(fp))))
                if (ch_next == '\n')
                    parse_mibLine++;
            if (ch_next == EOF)
                return PARSE_ENDOFFILE;
    ...
    if (token && stat(token, &idx_stat) == 0 && stat(dirname, &dir_stat) == 0) {
        if (dir_stat.st_mtime < idx_stat.st_mtime) {
            ...
            snprintf(tmpstr1, sizeof(tmpstr1), "%s/%s", dirname, tmpstr);
                    ...
                    _Parse_newModule(token, tmpstr1);
                    count++;
                }
                fclose(ip);
                return count;
...
Parse_readMib(const char *filename)
{
    ...
    fclose(fp);
    _Parse_newModule(token, filename);
...
static int _Parse_parseQuoteString(FILE * fp, char *token, int maxtlen)
{
    register int    ch;
    int             count = 0;
    int             too_long = 0;
    ...
    for (ch = _Parse_getc(fp); ch != EOF; ch = _Parse_getc(fp)) {
        if (ch == '\r')
            continue;
        if (ch == '\n') {
            parse_mibLine++;
            ...
            *token = '\0';
            if (too_long && DefaultStore_getInt(DsStorage_LIBRARY_ID,
                ...
                *(token_start + 50) = '\0';
                _Parse_printError("Warning: string too long",
...
_Parse_getIndexes(FILE * fp, struct Parse_IndexList_s **retp)
{
    int             type;
    char            token[PARSE_MAXTOKEN];
...
_Parse_getVarbinds(FILE * fp, struct Parse_VarbindList_s **retp)
{
    int             type;
    char            token[PARSE_MAXTOKEN];
...
static void
_Parse_freeIndexes(struct Parse_IndexList_s **spp)
{
    if (spp && *spp) {
...
static void
_Parse_freeVarbinds(struct Parse_VarbindList_s **spp)
{
    if (spp && *spp) {
...
static void
_Parse_freeRanges(struct Parse_RangeList_s **spp)
{
    if (spp && *spp) {
...
static void
_Parse_freeEnums(struct Parse_EnumList_s **spp)
{
    if (spp && *spp) {
...
static struct Parse_EnumList_s *
_Parse_copyEnums(struct Parse_EnumList_s *sp)
{
...
static struct Parse_RangeList_s *
_Parse_copyRanges(struct Parse_RangeList_s *sp)
{
 ...
 */
static int _Parse_tossObjectIdentifier(FILE * fp)
{
    int             type;
    char            token[PARSE_MAXTOKEN];
    int             bracketcount = 1;
...
Parse_findModule(int mid)
{
    ...
    for (mp = _parse_moduleHead; mp != NULL; mp = mp->next) {
        if (mp->modid == mid)
            break;
    }
    return mp;
...
static char     _parse_leaveIndent[256];
static int      _parse_leaveWasSimple;

static void
_Parse_printMibLeaves(FILE * f, struct Parse_Tree_s *tp, int width)
{
    struct Parse_Tree_s    *ntp;
    char           *ip = _parse_leaveIndent + strlen(_parse_leaveIndent) - 1;
    char            last_ipch = *ip;

    *ip = '+';
    if (tp->type == PARSE_TYPE_OTHER || tp->type > PARSE_TYPE_SIMPLE_LAST) {
        fprintf(f, "%s--%s(%ld)\n", _parse_leaveIndent, tp->label, tp->subid);
        if (tp->indexes) {
            struct Parse_IndexList_s *xp = tp->indexes;
            int             first = 1, cpos = 0, len, cmax =
                width - strlen(_parse_leaveIndent) - 12;
            *ip = last_ipch;
            fprintf(f, "%s  |  Index: ", _parse_leaveIndent);
            while (xp) {
                if (first)
                    first = 0;
                else
                ...
                if (cpos > cmax) {
                    fprintf(f, "\n");
                    fprintf(f, "%s  |         ", _parse_leaveIndent);
                    cpos = len;
                }
        ...
        *ip = last_ipch;
        ...
        if (tp->enums) {
            ...
            int             cpos = 0, cmax =
                width - strlen(_parse_leaveIndent) - 16;
            fprintf(f, "%s        Values: ", _parse_leaveIndent);
            while (ep) {
                char            buf[80];
                int             bufw;
                if (ep != tp->enums)
        ...
        if (tp->ranges) {
            ...
            if (size)
                fprintf(f, "%s        Size: ", _parse_leaveIndent);
            else
                fprintf(f, "%s        Range: ", _parse_leaveIndent);
            while (rp) {
                if (rp != tp->ranges)
                    fprintf(f, " | ");
                _Parse_printRangeValue(f, tp->type, rp);
                rp = rp->next;
            }
            fprintf(f, "\n");
        }
    ...
    *ip = last_ipch;
    strcat(_parse_leaveIndent, "  |");
    _parse_leaveWasSimple = tp->type != PARSE_TYPE_OTHER;

    {
        ...
        if (count) {
            ...
            for (i = 1, lp = leaves; i <= count; i++, lp++) {
                if (!_parse_leaveWasSimple || lp->tp->type == 0)
                    fprintf(f, "%s\n", _parse_leaveIndent);
                if (i == count)
                    ip[3] = ' ';
                _Parse_printMibLeaves(f, lp->tp, width);
            }
            free(leaves);
            _parse_leaveWasSimple = 0;
        }
    }
    ip[1] = 0;
}
...
{
    _parse_leaveIndent[0] = ' ';
    _parse_leaveIndent[1] = 0;
    _parse_leaveWasSimple = 1;
    _Parse_printMibLeaves(f, tp, width);
}
 ...
 */
static struct Parse_Node_s *
_Parse_mergeParseObjectid(struct Parse_Node_s *np, FILE * fp, char *name)
{
 ...
 */
static void
_Parse_treeFromNode(struct Parse_Tree_s *tp, struct Parse_Node_s *np)
{
    _Parse_freePartialTree(tp, FALSE);
    ...
    tp->type = _parse_translationTable[np->type];
    tp->access = np->access;
    tp->status = np->status;

...
static void _Container_factoryFree( void* dat, void* context )
{
    ...
    if ( data->name != NULL ) {
        ...
        free( UTILITIES_REMOVE_CONST( void*, data->name ) ); /* MEMORY_FREE wasted on object about to be freed */
    }

    free( data ); /* MEMORY_FREE wasted on param */
}
...
int Container_registerWithCompare( const char* name, Factory_Factory* f, Container_FuncCompare* c )
{
    ...
    if ( NULL != ct ) {
        ...
        ct = MEMORY_MALLOC_TYPEDEF( Container_Type );
        if ( NULL == ct )
            return -1;
        ct->name = strdup( name );
        ct->factory = f;
...
{
    int rc, min = UTILITIES_MIN_VALUE( lhs_len, rhs_len );
...
void Container_simpleFree( void* data, void* context )
{
    ...
    free( ( void* )data ); /* MEMORY_FREE wasted on param */
}

...
static int _ContainerListSsll_insert(Container_Container *c, const void *data)
{
    ...
    newNode = MEMORY_MALLOC_TYPEDEF(ContainerListSsll_SlNode);
    if(NULL == newNode)
        return -1;
    newNode->data = UTILITIES_REMOVE_CONST(void *, data);
    ++sl->count;
    ++c->sync;
...
Container_Container * ContainerListSsll_getSsll(void)
{
     ...
     * allocate memory
     */
    ContainerListSsll_SlContainer *sl = MEMORY_MALLOC_TYPEDEF(ContainerListSsll_SlContainer);
    if (NULL==sl) {
...
static Container_Iterator * _ContainerListSsll_iteratorGet(Container_Container *c)
{
    ...
    it = MEMORY_MALLOC_TYPEDEF(ContainerListSsll_SsllIterator);
    if(NULL == it)
        return NULL;

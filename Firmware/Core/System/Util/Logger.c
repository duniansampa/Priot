...
{
    if ( pstr )
        String_copyTruncate( _logger_syslogname, pstr, sizeof( _logger_syslogname ) );
...
int Logger_stderrlogStatus( void )
{
    ...
    for ( logh = logger_logHandlerHead; logh; logh = logh->next )
        if ( logh->enabled && ( logh->type == LOGGER_LOG_TO_STDOUT || logh->type == LOGGER_LOG_TO_STDERR ) ) {
            return 1;
        }
...
void Logger_enableStderrlog( void )
{
    ...
    for ( logh = logger_logHandlerHead; logh; logh = logh->next )
        if ( logh->type == LOGGER_LOG_TO_STDOUT || logh->type == LOGGER_LOG_TO_STDERR ) {
            Logger_enableThisLoghandler( logh );
            found = 1;
        }
...
int Logger_addLoghandler( Logger_LogHandler* logh )
{
    ...
    for ( i = LOGGER_PRIORITY_EMERG; i <= logh->priority; i++ )
        if ( !logger_logHandlerPriorities[ i ] || logger_logHandlerPriorities[ i ]->priority >= logh->priority )
...
Logger_LogHandler* Logger_registerLoghandler( int type, int priority )
{
    ...
    logh = MEMORY_MALLOC_TYPEDEF( Logger_LogHandler );
    if ( !logh )
        return NULL;

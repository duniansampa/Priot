...
int Int64_c64CheckFor32bitWrap( Int64_U64* oldVal, Int64_U64* newVal, int adjust )
{
     ...
     * check for wraps
     */
    if ( ( newVal->low >= oldVal->low ) && ( newVal->high == oldVal->high ) ) {
 ...
 *   else {
 *      Int64_c64Check32AndUpdate(&current,&new,&prev,&needwrapcheck);
...
/** Convert a signed 64-bit integer from ASCII to Int64_U64. */

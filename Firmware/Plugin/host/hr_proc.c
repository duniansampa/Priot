...
int proc_status( int );
int header_hrproc( struct Variable_s*, oid*, size_t*, int,
...
void init_hr_proc( void )
{
    ...
    REGISTER_MIB( "host/hr_proc", hrproc_variables, Variable4_s,
...
int header_hrproc( struct Variable_s* vp,
    oid* name,
    size_t* length,
...
#define HRPROC_ENTRY_NAME_LENGTH 11
    ...
    DEBUG_MSGTL( ( "host/hr_proc", "var_hrproc: " ) );
    DEBUG_MSGOID( ( "host/hr_proc", name, *length ) );
    DEBUG_MSG( ( "host/hr_proc", " %d\n", exact ) );
    ...
    if ( LowIndex == -1 ) {
        DEBUG_MSGTL( ( "host/hr_proc", "... index out of range\n" ) );
        return ( MATCH_FAILED );
    }
    ...
    *var_len = sizeof( long ); /* default to 'long' results */
    ...
    DEBUG_MSGTL( ( "host/hr_proc", "... get proc stats " ) );
    DEBUG_MSGOID( ( "host/hr_proc", name, *length ) );
    DEBUG_MSG( ( "host/hr_proc", "\n" ) );
    return LowIndex;
}
...
u_char*
var_hrproc( struct Variable_s* vp,
    oid* name,
    size_t* length,
    int exact, size_t* var_len, WriteMethodFT** write_method )
{
    ...
    case HRPROC_LOAD:
        ...
        if ( !cpu || !cpu->history || !cpu->history[ 0 ].total_hist || ( cpu->history[ 0 ].total_hist == cpu->total_ticks ) )
            return NULL;

        value = ( cpu->idle_ticks - cpu->history[ 0 ].idle_hist ) * 100;
        value /= ( cpu->total_ticks - cpu->history[ 0 ].total_hist );
        vars_longReturn = 100 - value;
        if ( vars_longReturn < 0 )
            vars_longReturn = 0;
        return ( u_char* )&vars_longReturn;
    default:
        DEBUG_MSGTL( ( "host/hr_proc", "unknown sub-id %d in var_hrproc\n",
            vp->magic ) );
    }
    return NULL;
}

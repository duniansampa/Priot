...
int header_hrhfilesys( struct Variable_s*, oid*, size_t*, int,
...
{
    REGISTER_MIB( "host/hr_filesys", hrfsys_variables, Variable4_s,
...
int header_hrhfilesys( struct Variable_s* vp,
    oid* name,
    size_t* length,
...
#define HRFSYS_ENTRY_NAME_LENGTH 11
    ...
    DEBUG_MSGTL( ( "host/hr_filesys", "var_hrhfilesys: " ) );
    DEBUG_MSGOID( ( "host/hr_filesys", name, *length ) );
    DEBUG_MSG( ( "host/hr_filesys", " %d\n", exact ) );
    ...
    if ( LowIndex == -1 ) {
        DEBUG_MSGTL( ( "host/hr_filesys", "... index out of range\n" ) );
        return ( MATCH_FAILED );
    }
    ...
    *var_len = sizeof( long ); /* default to 'long' results */
    ...
    DEBUG_MSGTL( ( "host/hr_filesys", "... get filesys stats " ) );
    DEBUG_MSGOID( ( "host/hr_filesys", name, *length ) );
    DEBUG_MSG( ( "host/hr_filesys", "\n" ) );
...
u_char*
var_hrhfilesys( struct Variable_s* vp,
    oid* name,
    size_t* length,
    int exact, size_t* var_len, WriteMethodFT** write_method )
{
    ...
    case HRFSYS_TYPE:
        fsys_type_id[ fsys_type_len - 1 ] = ( HRFS_entry->type > _NETSNMP_FS_TYPE_LOCAL ? NETSNMP_FS_TYPE_OTHER : HRFS_entry->type );
        *var_len = sizeof( fsys_type_id );
        return ( u_char* )fsys_type_id;

    case HRFSYS_ACCESS:
        vars_longReturn = HRFS_entry->flags & NETSNMP_FS_FLAG_RONLY ? 2 : 1;
        return ( u_char* )&vars_longReturn;
    case HRFSYS_BOOT:
        vars_longReturn = HRFS_entry->flags & NETSNMP_FS_FLAG_BOOTABLE ? 1 : 2;
        return ( u_char* )&vars_longReturn;
    case HRFSYS_STOREIDX:
        vars_longReturn = fsys_idx + NETSNMP_MEM_TYPE_MAX;
        return ( u_char* )&vars_longReturn;
    ...
    default:
        DEBUG_MSGTL( ( "snmpd", "unknown sub-id %d in var_hrhfilesys\n",
            vp->magic ) );
    }
    return NULL;
}
...
{
    Cache* c = netsnmp_fsys_get_cache();
    CacheHandler_checkAndReload( c );

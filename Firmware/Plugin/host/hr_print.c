...
int header_hrprint( struct Variable_s*, oid*, size_t*, int,
...
void init_hr_print( void )
{
    ...
    REGISTER_MIB( "host/hr_print", hrprint_variables, Variable4_s,
 ...
 * 
 */
int header_hrprint( struct Variable_s* vp,
    oid* name,
    size_t* length,
...
#define HRPRINT_ENTRY_NAME_LENGTH 11
    ...
    DEBUG_MSGTL( ( "host/hr_print", "var_hrprint: " ) );
    DEBUG_MSGOID( ( "host/hr_print", name, *length ) );
    DEBUG_MSG( ( "host/hr_print", " %d\n", exact ) );
    ...
    for ( ;; ) {
        ...
        if ( ( !exact && ( result < 0 ) ) && ( LowIndex == -1 || print_idx < LowIndex ) ) {
            LowIndex = print_idx;
            /*
             * Save printer status information 
             */
    ...
    if ( LowIndex == -1 ) {
        DEBUG_MSGTL( ( "host/hr_print", "... index out of range\n" ) );
        return ( MATCH_FAILED );
    }
    ...
    *var_len = sizeof( long ); /* default to 'long' results */
    ...
    DEBUG_MSGTL( ( "host/hr_print", "... get print stats " ) );
    DEBUG_MSGOID( ( "host/hr_print", name, *length ) );
    DEBUG_MSG( ( "host/hr_print", "\n" ) );
    return LowIndex;
}
...
u_char*
var_hrprint( struct Variable_s* vp,
    oid* name,
    size_t* length,
    int exact, size_t* var_len, WriteMethodFT** write_method )
{
    ...
    switch ( vp->magic ) {
    case HRPRINT_STATUS:
        vars_longReturn = printer_detail_status( print_idx );
        return ( u_char* )&vars_longReturn;
    case HRPRINT_ERROR:
        return NULL;

    default:
        DEBUG_MSGTL( ( "host/hr_print", "unknown sub-id %d in var_hrprint\n",
            vp->magic ) );
    }
    return NULL;
}
...
describe_printer( int idx )
{
    ...
    DEBUG_MSGTL( ( "host/hr_print", "describe p: %d/%d %s\n", HRP_index, idx,

...
/* ---------------------------------------------------------------------
 */
int netsnmp_arch_swrun_container_load( Container_Container* container, u_int flags )
{
    DIR* procdir = NULL;
    ...
    if ( NULL == procdir ) {
        Logger_log( LOGGER_PRIORITY_ERR, "Failed to open /proc" );
        return -1;
    }
    ...
    DEBUG_MSGTL( ( "swrun:load:arch", " loaded %" NETSNMP_PRIz "d entries\n",

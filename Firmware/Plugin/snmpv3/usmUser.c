 ...
 */
oid            *
usm_generate_OID(oid * prefix, size_t prefixLen, struct Usm_User_s *uptr,
                 size_t * length)
{
    oid            *indexOid;
    int             i;
                       ...
                       u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == IMPL_RESERVE1) {
        ...
        cloneFrom =
            Usm_parseUser((oid *) var_val, var_val_len / sizeof(oid));
        ...
        if (uptr->userStatus == TC_RS_ACTIVE
            ...
            if (Api_oidCompare
                ...
                if (uptr->cloneFrom != NULL) {
                    ...
                    resetOnFail = 1;
                    uptr->authProtocol =
                        Api_duplicateObjid((oid *) var_val,
                                             var_val_len / sizeof(oid));
                    if (uptr->authProtocol == NULL) {
                        return PRIOT_ERR_RESOURCEUNAVAILABLE;
                    }
            ...
            if (Api_oidCompare(uptr->authProtocol, uptr->authProtocolLen,
                ...
                if (Api_oidCompare
                    (uptr->authProtocol, uptr->authProtocolLen,
                     usm_hMACSHA1AuthProtocol,
                     sizeof(usm_hMACSHA1AuthProtocol) / sizeof(oid)) == 0) {
            ...
            if (Api_oidCompare(uptr->authProtocol, uptr->authProtocolLen,
                ...
                if (Api_oidCompare
                 ...
                 && Api_oidCompare
                    ((oid *) var_val, var_val_len / sizeof(oid),
                     usm_dESPrivProtocol,
                     sizeof(usm_dESPrivProtocol) / sizeof(oid)) != 0
                 && Api_oidCompare
                    ((oid *) var_val, var_val_len / sizeof(oid),
                     usm_aESPrivProtocol,
                     sizeof(usm_aESPrivProtocol) / sizeof(oid)) != 0) {
                    return PRIOT_ERR_WRONGVALUE;
                }
            }
            resetOnFail = 1;
...
int
usmStatusCheck(struct Usm_User_s *uptr)
{
    if (uptr == NULL) {
        return 0;
    } else {

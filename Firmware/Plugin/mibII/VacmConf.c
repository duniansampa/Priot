 ...
 */
void VacmConf_initVacmPriotdEasyTokens( void )
{
...
void VacmConf_parseGroup( const char* token, char* param )
{
    ...
    if ( strcasecmp( model, "v1" ) == 0 )
        ...
        ReadConfig_configPerror( "bad security model \"any\" should be: v1, v2c, usm or a registered security plugin name - installing anyway" );
        imodel = PRIOT_SEC_MODEL_ANY;
    } else {
        if ( ( imodel = MapList_findValue( "priotSecmods", model ) ) == MapListErrorCode_NULL ) {
            ReadConfig_configPerror( "bad security model, should be: v1, v2c or usm or a registered security plugin name" );
            return;
        }
    }
...
int _VacmConf_parseAccessCommon( const char* token, char* param, char** st,
    ...
    *context = strtok_r( NULL, " \t\n", st );
    ...
    if ( strcasecmp( level, "noauth" ) == 0 )
        ...
        *ilevel = PRIOT_SEC_LEVEL_AUTHPRIV;
        ...
        ReadConfig_configPerror( "bad security level (noauthnopriv, authnopriv, authpriv)" );
        return PARSE_FAIL;
    }
    ...
    if ( strcmp( prefix, "exact" ) == 0 )
        ...
        *iprefix = 2;
        ...
        ReadConfig_configPerror( "bad prefix match parameter \"0\", should be: exact or prefix - installing anyway" );
        *iprefix = 1;
    } else {
        ReadConfig_configPerror( "bad prefix match parameter, should be: exact or prefix" );
        return PARSE_FAIL;
    }
...
int VacmConf_parseAuthtokens( const char* token, char** confline )
{
...
void VacmConf_parseAuthuser( const char* token, char* confline )
{
...
void VacmConf_parseAuthcommunity( const char* token, char* confline )
{
    int viewtypes = VacmConf_parseAuthtokens( token, &confline );
    if ( viewtypes != -1 )
...
void VacmConf_parseAuthaccess( const char* token, char* confline )
{
    char *group, *view, *tmp;
    const char* context;
    ...
    if ( tmp ) {
        ...
        if ( model == PRIOT_SEC_MODEL_SNMPv1 || model == PRIOT_SEC_MODEL_SNMPv2c )
            level = PRIOT_SEC_LEVEL_NOAUTH;
        else
            level = PRIOT_SEC_LEVEL_AUTHNOPRIV;
    }
...
void VacmConf_parseSetaccess( const char* token, char* param )
{
...
void VacmConf_parseAccess( const char* token, char* param )
{
...
void VacmConf_parseView( const char* token, char* param )
{
...
void VacmConf_genCom2sec( int commcount, const char* community, const char* addressname,
...
void VacmConf_parseRwuser( const char* token, char* confline )
{
    VacmConf_createSimple( token, confline, VACM_CREATE_SIMPLE_V3,
        VACM_VIEW_READ_BIT | VACM_VIEW_WRITE_BIT );
}

void VacmConf_parseRouser( const char* token, char* confline )
{
    VacmConf_createSimple( token, confline, VACM_CREATE_SIMPLE_V3,
        VACM_VIEW_READ_BIT );
}

void VacmConf_parseRocommunity( const char* token, char* confline )
{
    VacmConf_createSimple( token, confline, VACM_CREATE_SIMPLE_COMIPV4,
        VACM_VIEW_READ_BIT );
}

void VacmConf_parseRwcommunity( const char* token, char* confline )
{
    VacmConf_createSimple( token, confline, VACM_CREATE_SIMPLE_COMIPV4,
        VACM_VIEW_READ_BIT | VACM_VIEW_WRITE_BIT );
}

void VacmConf_parseRocommunity6( const char* token, char* confline )
{
    VacmConf_createSimple( token, confline, VACM_CREATE_SIMPLE_COMIPV6,
        VACM_VIEW_READ_BIT );
}

void VacmConf_parseRwcommunity6( const char* token, char* confline )
{
    VacmConf_createSimple( token, confline, VACM_CREATE_SIMPLE_COMIPV6,
        VACM_VIEW_READ_BIT | VACM_VIEW_WRITE_BIT );
}

void VacmConf_createSimple( const char* token, char* confline,
    int parsetype, int viewtypes )
{
    ...
    int commversion = PRIOT_SEC_MODEL_SNMPv1 | PRIOT_SEC_MODEL_SNMPv2c;
     ...
     */
    if ( ( viewtypes == VACM_VIEW_READ_BIT ) || ( viewtypes == ( VACM_VIEW_READ_BIT | VACM_VIEW_WRITE_BIT ) ) ) {
...
int VacmConf_standardViews( int majorID, int minorID, void* serverarg,
    void* clientarg )
{
    char line[ IMPL_SPRINT_MAX_LEN ];
...
int VacmConf_warnIfNotConfigured( int majorID, int minorID, void* serverarg,
    void* clientarg )
{
    const char* name = DefaultStore_getString( DsStore_LIBRARY_ID,
        DsStr_APPTYPE );
    const int agent_mode = DefaultStore_getBoolean( DsStore_APPLICATION_ID,
        DsAgentBoolean_ROLE );
    if ( NULL == name )
        name = "priotd";
    ...
    if ( !Vacm_isConfigured() ) {
        ...
        if ( !strcmp( name, "priottrapd" ) && !DefaultStore_getBoolean( DsStore_APPLICATION_ID, DsAgentBoolean_APP_NO_AUTHORIZATION ) ) {
            Logger_log( LOGGER_PRIORITY_WARNING,
                "Warning: no access control information configured.\n"
                "  (Config search path: %s)\n"
...
int VacmConf_inViewCallback( int majorID, int minorID, void* serverarg,
    void* clientarg )
{
    struct ViewParameters_s* view_parms = ( struct ViewParameters_s* )serverarg;
    int retval;
 ...
 */
int VacmConf_inView( Types_Pdu* pdu, oid* name, size_t namelen,
    int check_subtree )
{
    int viewtype;
 ...
 *	\<securityName\> \<groupName\> \<viewName\> \<viewType\>
 */
int VacmConf_checkView( Types_Pdu* pdu, oid* name, size_t namelen,
...
int VacmConf_checkViewContents( Types_Pdu* pdu, oid* name, size_t namelen,
...
#define CONTEXTNAMEINDEXLEN 32
    ...
    if ( !( flags & VACM_CHECK_VIEW_CONTENTS_DNE_CONTEXT_OK ) && !AgentRegistry_subtreeFindFirst( contextNameIndex ) ) {
        /*

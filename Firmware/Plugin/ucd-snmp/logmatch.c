...
updateLogmatch( int iindex )
{
    ...
    snprintf( perfilename, sizeof( perfilename ), "%s/snmpd_logmatch_%s.pos",
        ReadConfig_getPersistentDirectory(), logmatchTable[ iindex ].name );
        ...
        if ( ( perfile = fopen( perfilename, "r" ) ) ) {
            ...
            if ( fscanf( perfile, "%lu %lu %lu %s",
                ...
                if ( logmatch_update_filename( logmatchTable[ iindex ].filenamePattern,
                    ...
                    if ( ( logmatchTable[ iindex ].logfile = fopen( logmatchTable[ iindex ].filename, "r" ) ) ) {
                        ...
                        if ( !fseek( logmatchTable[ iindex ].logfile, pos, SEEK_SET ) ) {
                            ...
                            logmatchTable[ iindex ].currentMatchCounter = ccounter;
                        }
    ...
    if ( stat( logmatchTable[ iindex ].filename, &sb ) == 0 ) {
        ...
        if ( ( logmatchTable[ iindex ].logfile = fopen( logmatchTable[ iindex ].filename, "r" ) ) ) {
            ...
            logmatchTable[ iindex ].currentFilePosition = ftell( logmatchTable[ iindex ].logfile );
            fclose( logmatchTable[ iindex ].logfile );
        }
    }
...
logmatch_parse_config( const char* token, char* cptr )
{
	 ...
	 */
        if ( space_name != ' ' ) {
            Logger_log( LOGGER_PRIORITY_ERR, "logmatch_parse_config: the name scanned "
                                             "in from line %s is too large. logmatchCount = %d\n",
                cptr, logmatchCount );
            return;
        } else if ( space_path != ' ' ) {
            Logger_log( LOGGER_PRIORITY_ERR, "logmatch_parse_config: the file name "
        ...
        logmatchTable[ logmatchCount ].myRegexError = regcomp( &logmatchTable[ logmatchCount ].regexBuffer,
            logmatchTable[ logmatchCount ].regEx,
        ...
        if ( logmatchTable[ logmatchCount ].myRegexError ) {
            ...
            Logger_log( LOGGER_PRIORITY_ERR, "Could not process the logmatch regex - %s,"
...
static u_char*
var_logmatch_table( struct Variable_s* vp,
    oid* name,
    size_t* length,
    int exact,
    size_t* var_len, WriteMethodFT** write_method )
{
    ...
    default:
        DEBUG_MSGTL( ( "snmpd", "unknown sub-id %d in var_logmatch_table\n",
            vp->magic ) );
    }
...
void init_logmatch( void )
{
     ...
     */
    REGISTER_MIB( "ucd-snmp/logmatch", logmatch_info, Variable2_s,
        logmatch_info_oid );
    REGISTER_MIB( "ucd-snmp/logmatch", logmatch_table, Variable2_s,
        logmatch_variables_oid );

    AgentReadConfig_priotdRegisterConfigHandler( "logmatch", logmatch_parse_config,
        logmatch_free_config,

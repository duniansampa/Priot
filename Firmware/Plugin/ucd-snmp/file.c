...
void init_file( void )
{
    ...
    AgentReadConfig_priotdRegisterConfigHandler( "file", file_parse_config,
...
void file_parse_config( const char* token, char* cptr )
{
    ...
    if ( fileCount < MAXFILE ) {
        ...
        cp = ReadConfig_copyNword( cptr, fileTable[ fileCount ].name, FILE_NAME_MAX );

        if ( strlen( fileTable[ fileCount ].name ) >= FILE_NAME_MAX - 1 ) {
            ReadConfig_configPerror( "file name too long" );
            return;
        }
...
u_char*
var_file_table( struct Variable_s* vp,
    oid* name,
    size_t* length,
    int exact, size_t* var_len, WriteMethodFT** write_method )
{
    ...
    default:
        DEBUG_MSGTL( ( "snmpd", "unknown sub-id %d in var_file_table\n",
            vp->magic ) );
    }

...
{
    DEBUG_MSGTL( ( "disman:schedule:init", "init schedule container\n" ) );
    if ( !schedule_table ) {
        schedule_table = TableTdata_createTable( "schedTable", 0 );
        DEBUG_MSGTL( ( "disman:schedule:init",
...
void init_schedCore( void )
{
     ...
     */
    DEBUG_MSGTL( ( "disman:schedule:init", "Initializing core module\n" ) );
...
_sched_callback( unsigned int reg, void* magic )
{
    ...
    DEBUG_MSGTL( ( "disman:schedule:callback", "assignment " ) );
    DEBUG_MSGOID( ( "disman:schedule:callback", entry->schedVariable,
    ...
    ret = Client_querySet( &assign, entry->session );
    if ( ret != PRIOT_ERR_NOERROR ) {
        DEBUG_MSGTL( ( "disman:schedule:callback",
            "assignment failed (%d)\n", ret ) );
        entry->schedFailures++;
...
void sched_nextTime( struct schedTable_entry* entry )
{
    ...
    if ( !entry ) {
        DEBUG_MSGTL( ( "disman:schedule:time", "missing entry\n" ) );
        return;
    }

    if ( entry->schedCallbackID )
        Alarm_unregister( entry->schedCallbackID );

    if ( !( entry->flags & SCHEDULE_FLAG_ENABLED ) || !( entry->flags & SCHEDULE_FLAG_ACTIVE ) ) {
        DEBUG_MSGTL( ( "disman:schedule:time", "inactive entry\n" ) );
        return;
    }
    ...
    case SCHED_TYPE_PERIODIC:
        if ( !entry->schedInterval ) {
            DEBUG_MSGTL( ( "disman:schedule:time", "periodic: no interval\n" ) );
            return;
        }
        if ( entry->schedLastRun ) {
            entry->schedNextRun = entry->schedLastRun + entry->schedInterval;
        } else {
            entry->schedNextRun = now + entry->schedInterval;
        }
        DEBUG_MSGTL( ( "disman:schedule:time", "periodic: (%ld) %s",
            ( long )entry->schedNextRun, ctime( &entry->schedNextRun ) ) );
        break;

    case SCHED_TYPE_ONESHOT:
        if ( entry->schedLastRun ) {
            DEBUG_MSGTL( ( "disman:schedule:time", "one-shot: expired (%ld) %s",
                ( long )entry->schedNextRun,
                ctime( &entry->schedNextRun ) ) );
            return;
        }
        /* Fallthrough */
        DEBUG_MSGTL( ( "disman:schedule:time", "one-shot: fallthrough\n" ) );
    case SCHED_TYPE_CALENDAR:
         ...
         */
        if ( _bit_allClear( entry->schedMinute, 8 ) || _bit_allClear( entry->schedHour, 3 ) || _bit_allClear( entry->schedDay, 4 + 4 ) || _bit_allClear( entry->schedMonth, 2 ) || _bit_allClear( &entry->schedWeekDay, 1 ) ) {
            DEBUG_MSGTL( ( "disman:schedule:time", "calendar: incomplete spec\n" ) );
            return;
        }
        ...
        if ( _bit_set( entry->schedMonth, now_tm.tm_mon ) ) {
            ...
            if ( _bit_set( &entry->schedWeekDay, now_tm.tm_wday ) && ( _bit_set( entry->schedDay, now_tm.tm_mday - 1 ) || _bit_set( entry->schedDay, 31 + rev_day ) ) ) {
        ...
        DEBUG_MSGTL( ( "disman:schedule:time", "calendar: (%ld) %s",
            ( long )entry->schedNextRun, ctime( &entry->schedNextRun ) ) );
        return;

    default:
        DEBUG_MSGTL( ( "disman:schedule:time", "unknown type (%ld)\n",
            entry->schedType ) );
        return;
    }
    entry->schedCallbackID = Alarm_register(
        entry->schedNextRun - now,
        0, _sched_callback, entry );
    return;
}

void sched_nextRowTime( TdataRow* row )
{
...
schedTable_createEntry( const char* schedOwner, const char* schedName )
{
    ...
    TableTdata_addRow( schedule_table, row );
    return row;
}
 ...
 */
void schedTable_removeEntry( TdataRow* row )
{
    struct schedTable_entry* entry;

    if ( !row || !row->data ) {
        DEBUG_MSGTL( ( "disman:schedule:entry", "remove: missing entry\n" ) );
        return; /* Nothing to remove */
    }
    entry = ( struct schedTable_entry* )
        TableTdata_removeAndDeleteRow( schedule_table, row );
    if ( entry ) {

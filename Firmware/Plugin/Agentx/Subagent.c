...
int Subagent_startup( int majorID, int minorID,
...
void Subagent_freeSetVars( Types_Session* ss, Types_Pdu* pdu )
{
...
int Subagent_handleAgentxPacket( int operation, Types_Session* session, int reqid,
    Types_Pdu* pdu, void* magic )
{
    ...
    if ( pdu->command == AGENTX_MSG_GET
        ...
        smagic = ( NsSubagentMagic* )calloc( 1, sizeof( NsSubagentMagic ) );
        if ( smagic == NULL ) {
    ...
    case AGENTX_MSG_TESTSET:
        /*
         * XXXWWW we have to map this twice to both IMPL_RESERVE1 and IMPL_RESERVE2
         */
    ...
    case AGENTX_MSG_CLEANUPSET:
        ...
        if ( asi->mode == PRIOT_MSG_INTERNAL_SET_RESERVE1 || asi->mode == PRIOT_MSG_INTERNAL_SET_RESERVE2 ) {
...
_Subagent_invalidOpAndMagic( int op, NsSubagentMagic* smagic )
{
    ...
    if ( smagic && ( Api_sessPointer( smagic->session ) == NULL || op == API_CALLBACK_OP_TIMED_OUT ) ) {
...
int Subagent_handleSubagentResponse( int op, Types_Session* session, int reqid,
    Types_Pdu* pdu, void* magic )
{
    ...
    if ( pdu->command == PRIOT_MSG_INTERNAL_SET_FREE || pdu->command == PRIOT_MSG_INTERNAL_SET_UNDO || pdu->command == PRIOT_MSG_INTERNAL_SET_COMMIT ) {
...
int Subagent_handleSubagentSetResponse( int op, Types_Session* session, int reqid,
    Types_Pdu* pdu, void* magic )
{
    ...
    if ( asi->mode == PRIOT_MSG_INTERNAL_SET_RESERVE1 ) {
        /*
         * reloop for IMPL_RESERVE2 mode, an internal only agent mode
         */
        /*
        ...
        if ( !pdu->errstat ) {
            ...
            DEBUG_MSGTL( ( "agentx/subagent",
                "  going from IMPL_RESERVE1 -> IMPL_RESERVE2\n" ) );
            return 1;
        }
    } else {
        if ( asi->mode == PRIOT_MSG_INTERNAL_SET_FREE || asi->mode == PRIOT_MSG_INTERNAL_SET_UNDO || asi->mode == PRIOT_MSG_INTERNAL_SET_COMMIT ) {
...
int Subagent_registrationCallback( int majorID, int minorID, void* serverarg,
    void* clientarg )
{
    struct RegisterParameters_s* reg_parms = ( struct RegisterParameters_s* )serverarg;
...
Subagent_sysORCallback( int majorID, int minorID, void* serverarg,
    void* clientarg )
{
    const struct RegisterSysORParameters_s* reg_parms = ( const struct RegisterSysORParameters_s* )serverarg;
    Types_Session* agentx_ss = *( Types_Session** )clientarg;
...
int Subagent_openMasterSession( void )
{
    ...
    agent_mainSession = Api_addFull( &sess, t, NULL, Protocol_parse, NULL, NULL,
...
void _Subagent_reopenSession( unsigned int clientreg, void* clientarg )
{
 ...
 */
void Subagent_checkSession( unsigned int clientreg, void* clientarg )
{
    Types_Session* ss = ( Types_Session* )clientarg;
    if ( !ss ) {
        if ( clientreg )

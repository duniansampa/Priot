 ...
 */
void AgentxConfig_parseAgentxSocket( const char* token, char* cptr )
{
    DEBUG_MSGTL( ( "agentx/config", "port spec: %s\n", cptr ) );
    DefaultStore_setString( DsStore_APPLICATION_ID, DsAgentString_X_SOCKET, cptr );
}
 ...
 * Master agent
 */
void AgentxConfig_parseMaster( const char* token, char* cptr )
{
    int i = -1;
    ...
    if ( i < 0 || i > 1 ) {
        ...
        DefaultStore_setBoolean( DsStore_APPLICATION_ID, DsAgentBoolean_AGENTX_MASTER, i );
}

void AgentxConfig_parseAgentxPerms( const char* token, char* cptr )
{
     ...
     */
    if ( socket_user ) {
        uid = System_getUserUidByUserName( socket_user );
        if ( uid != 0 )
            DefaultStore_setInt( DsStore_APPLICATION_ID,
     ...
     */
    if ( socket_group ) {
        gid = System_getUserGidByUserName( socket_group );
        if ( gid != 0 )
            DefaultStore_setInt( DsStore_APPLICATION_ID,
...
void AgentxConfig_parseAgentxTimeout( const char* token, char* cptr )
{
...
void AgentxConfig_parseAgentxRetries( const char* token, char* cptr )
{
    int x = atoi( cptr );
 ...
 */
void AgentxConfig_registerConfigHandler( const char* token,
...
void AgentxConfig_unregisterConfigHandler( const char* token )
{
    ReadConfig_unregisterConfigHandler( ":agentx", token );
}

void AgentxConfig_init( void )
{
    int agent_role = DefaultStore_getBoolean( DsStore_APPLICATION_ID,
        DsAgentBoolean_ROLE );

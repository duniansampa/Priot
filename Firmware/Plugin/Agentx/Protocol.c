...
int Protocol_reallocBuildInt( u_char** buf, size_t* buf_len, size_t* out_len,
    int allow_realloc,
...
void Protocol_buildInt( u_char* bufp, u_int value, int network_byte_order )
{
...
int Protocol_reallocBuildShort( u_char** buf, size_t* buf_len,
...
int Protocol_reallocBuildOid( u_char** buf, size_t* buf_len, size_t* out_len,
    int allow_realloc,
    ...
    int network_order )
{
     ...
     */
    if ( name_len >= 5 && ( name[ 0 ] == 1 && name[ 1 ] == 3 && name[ 2 ] == 6 && name[ 3 ] == 1 && name[ 4 ] > 0 && name[ 4 ] < 256 ) ) {
        prefix = name[ 4 ];
        name += 5;
        name_len -= 5;
    }
...
int Protocol_reallocBuildString( u_char** buf, size_t* buf_len,
    size_t* out_len, int allow_realloc,
...
int Protocol_reallocBuildDouble( u_char** buf, size_t* buf_len,
    size_t* out_len, int allow_realloc,
...
int Protocol_reallocBuildFloat( u_char** buf, size_t* buf_len,
    size_t* out_len, int allow_realloc,
...
int Protocol_reallocBuildVarbind( u_char** buf, size_t* buf_len,
    size_t* out_len, int allow_realloc,
    VariableList* vp, int network_order )
{
    ...
    *( *buf + *out_len ) = 0;
    ...
    case ASN01_OPAQUE_FLOAT:
        ...
        if ( !Protocol_reallocBuildFloat( buf, buf_len, out_len, allow_realloc, *( vp->value.floatValue ),
                 network_order ) ) {
            DEBUG_INDENTLESS();
            DEBUG_INDENTLESS();
            DEBUG_INDENTLESS();
    ...
    case ASN01_OPAQUE_DOUBLE:
        ...
        if ( !Protocol_reallocBuildDouble( buf, buf_len, out_len, allow_realloc, *( vp->value.doubleValue ),
                 network_order ) ) {
            DEBUG_INDENTLESS();
            DEBUG_INDENTLESS();
            DEBUG_INDENTLESS();
    ...
    case ASN01_COUNTER64:
        if ( network_order ) {
            ...
            if ( !Protocol_reallocBuildInt( buf, buf_len, out_len, allow_realloc,
                     vp->value.counter64->high, network_order )
                || !Protocol_reallocBuildInt( buf, buf_len, out_len,
                       allow_realloc,
                       vp->value.counter64->low,
                       network_order ) ) {
                DEBUG_INDENTLESS();
                DEBUG_INDENTLESS();
                DEBUG_INDENTLESS();
            ...
            if ( !Protocol_reallocBuildInt( buf, buf_len, out_len, allow_realloc,
                     vp->value.counter64->low, network_order )
                || !Protocol_reallocBuildInt( buf, buf_len, out_len,
                       allow_realloc,
                       vp->value.counter64->high,
                       network_order ) ) {
                DEBUG_INDENTLESS();
                DEBUG_INDENTLESS();
                DEBUG_INDENTLESS();
...
int Protocol_reallocBuildHeader( u_char** buf, size_t* buf_len,
    size_t* out_len, int allow_realloc,
    Types_Pdu* pdu )
{
    size_t ilen = *out_len;
    ...
    *( *buf + *out_len ) = 0;
    ...
    if ( pdu->flags & AGENTX_MSG_FLAG_NON_DEFAULT_CONTEXT ) {
        ...
        if ( !Protocol_reallocBuildString( buf, buf_len, out_len, allow_realloc, pdu->community,
    ...
    Types_Session* session, Types_Pdu* pdu )
{
     ...
     */
    if ( !Protocol_reallocBuildHeader( buf, buf_len, out_len, allow_realloc, pdu ) ) {
        return 0;
    }
    ...
    case AGENTX_MSG_OPEN:
        ...
        *( *buf + *out_len ) = ( u_char )pdu->time;
        ...
        if ( !Protocol_reallocBuildOid( buf, buf_len, out_len, allow_realloc, 0, pdu->variables->name,
        ...
        if ( !Protocol_reallocBuildString( buf, buf_len, out_len, allow_realloc,
                 pdu->variables->value.string, pdu->variables->valueLength,
                 network_order ) ) {
            DEBUG_INDENTLESS();
            DEBUG_INDENTLESS();
            return 0;
    ...
    case AGENTX_MSG_UNREGISTER:
        ...
        *( *buf + *out_len ) = ( u_char )0;
        ...
        if ( !Protocol_reallocBuildOid( buf, buf_len, out_len, allow_realloc, 0, vp->name,
        ...
        if ( pdu->range_subid ) {
            ...
            if ( !Protocol_reallocBuildInt( buf, buf_len, out_len, allow_realloc,
                     vp->value.objectId[ pdu->range_subid - 1 ], network_order ) ) {
                DEBUG_INDENTLESS();
                DEBUG_INDENTLESS();
                return 0;
            }
    ...
    case AGENTX_MSG_GETNEXT:
        ...
        for ( vp = pdu->variables; vp != NULL; vp = vp->next ) {
            ...
            if ( !Protocol_reallocBuildOid( buf, buf_len, out_len, allow_realloc, 0, vp->value.objectId,
                     vp->valueLength / sizeof( oid ), network_order ) ) {
                DEBUG_INDENTLESS();
                DEBUG_INDENTLESS();
                return 0;
    ...
    case AGENTX_MSG_TESTSET:
        DEBUG_DUMPHEADER( "send", "Get* Variable List" );
        for ( vp = pdu->variables; vp != NULL; vp = vp->next ) {
            if ( !Protocol_reallocBuildVarbind( buf, buf_len, out_len, allow_realloc, vp,
                     network_order ) ) {
                DEBUG_INDENTLESS();
                DEBUG_INDENTLESS();
                return 0;
...
int Protocol_reallocBuild( Types_Session* session, Types_Pdu* pdu,
    u_char** buf, size_t* buf_len, size_t* out_len )
{
    if ( session == NULL || buf_len == NULL || out_len == NULL || pdu == NULL || buf == NULL ) {
        return -1;
    }
...
int Protocol_parseInt( u_char* data, u_int network_byte_order )
{
    u_int value = 0;
...
int Protocol_parseShort( u_char* data, u_int network_byte_order )
{
    u_short value = 0;
    ...
    u_int network_byte_order )
{
    ...
    int tmp;
    u_char* buf;
    u_char* const cp = Protocol_parseString( data, length,
...
int Protocol_parse( Types_Session* session, Types_Pdu* pdu, u_char* data,
    size_t len )
{
    register u_char* bufp = data;
...
#define ErrorCode_INCOMPLETE_PACKET ErrorCode_ASN_PARSE_ERR
    ...
    case AGENTX_MSG_REGISTER:
        ...
        *length -= 4;
        ...
        if ( pdu->range_subid ) {
            range_bound = Protocol_parseInt( bufp, pdu->flags & AGENTX_FLAGS_NETWORK_BYTE_ORDER );
            bufp += 4;
            *length -= 4;
    ...
    case AGENTX_MSG_GETBULK:
        ...
        pdu->non_repeaters = Protocol_parseShort( bufp, pdu->flags & AGENTX_FLAGS_NETWORK_BYTE_ORDER );
        DEBUG_INDENTLESS();
        DEBUG_DUMPHEADER( "recv", "Max-repeaters" );
        pdu->max_repetitions = Protocol_parseShort( bufp + 2, pdu->flags & AGENTX_FLAGS_NETWORK_BYTE_ORDER );
        DEBUG_INDENTLESS();
        bufp += 4;
        *length -= 4;
    /*
    ...
    case AGENTX_MSG_RESPONSE:
         ...
         * sysUpTime
         */
        pdu->time = Protocol_parseInt( bufp, pdu->flags & AGENTX_FLAGS_NETWORK_BYTE_ORDER );
        bufp += 4;
        *length -= 4;

        pdu->errstat = Protocol_parseShort( bufp, pdu->flags & AGENTX_FLAGS_NETWORK_BYTE_ORDER );
        pdu->errindex = Protocol_parseShort( bufp + 2,
            pdu->flags & AGENTX_FLAGS_NETWORK_BYTE_ORDER );
        bufp += 4;
        *length -= 4;
    /*
...
int Protocol_checkPacket( u_char* packet, size_t packet_len )
{

...
static int _PluginModules_shutdownModules(int majorID, int minorID, void *serve, void *client)
{
...
void
PluginModules_initModules(void)
{
    static int once = 0;
    ...
    if (once == 0) {
        ...
        rc = Callback_registerCallback( CALLBACK_LIBRARY,
                                      CALLBACK_SHUTDOWN,
                                     _PluginModules_shutdownModules,
                                     NULL);

        if( rc != PRIOT_ERR_NOERROR )
            Logger_log(LOGGER_PRIORITY_ERR, "error registering for SHUTDOWN callback "
                     "for mib modules\n");
    }
}

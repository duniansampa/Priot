    ...
    char* cache_expired )
{
    ...
    if ( cache_timeout != NULL )
        *cache_timeout = 5;
    if ( cache_flags != NULL )
        *cache_flags |= CacheOperation_RESET_TIMER_ON_USE | CacheOperation_DONT_FREE_BEFORE_LOAD;
    access->cache_expired = cache_expired;

    DEBUG_MSGTL( ( "access:netlink:arp", "create arp cache\n" ) );
...
int netsnmp_access_arp_load( netsnmp_arp_access* access )
{
    ...
    if ( fd == 0 ) {
        ...
        if ( fd < 0 ) {
            Logger_log( LOGGER_PRIORITY_ERR, "netsnmp_access_arp_load: netlink socket create error\n" );
            return -1;
        }
        ...
        if ( bind( fd, ( struct sockaddr* )&sa, sizeof( sa ) ) < 0 ) {
            Logger_log( LOGGER_PRIORITY_ERR, "netsnmp_access_arp_load: netlink bind failed\n" );
            return -1;
        }
    ...
    DEBUG_MSGTL( ( "access:netlink:arp", "synchronizing arp table\n" ) );
    ...
    if ( r < 0 ) {
        Logger_log( LOGGER_PRIORITY_ERR, "netsnmp_access_arp_refresh: send failed\n" );
        return -1;
    }
...
int netsnmp_access_arp_unload( netsnmp_arp_access* access )
{
    ...
    DEBUG_MSGTL( ( "access:netlink:arp", "unload arp cache\n" ) );
...
static void netsnmp_access_arp_read_netlink( int fd, void* data )
{
    ...
    do {
        ...
        if ( r < 0 ) {
            ...
            if ( errno == EAGAIN )
                return;
            Logger_log( LOGGER_PRIORITY_WARNING, "netlink buffer overrun\n" );
            access->synchronized = 0;
    ...
    for ( h = ( struct nlmsghdr* )buf; NLMSG_OK( h, len ); h = NLMSG_NEXT( h, len ) ) {
        ...
        DEBUG_MSGTL( ( "access:netlink:arp", "arp netlink notification\n" ) );
        ...
        if ( r > 0 ) {
            ...
            if ( r < 0 ) {
                LOGGER_LOGONCE( ( LOGGER_PRIORITY_ERR, "filling entry info failed\n" ) );
                DEBUG_MSGTL( ( "access:netlink:arp", "filling entry info failed\n" ) );
            }
...
fillup_entry_info( netsnmp_arp_entry* entry, struct nlmsghdr* nlmp )
{
    ...
    case RTM_GETNEIGH:
        return 0;
    default:
        DEBUG_MSGTL( ( "access:netlink:arp",
    ...
    if ( tb[ NDA_DST ] ) {
        ...
        if ( entry->arp_ipaddress_len > sizeof( entry->arp_ipaddress ) ) {
            Logger_log( LOGGER_PRIORITY_ERR, "netlink ip address length %d is too long\n",
    ...
    if ( tb[ NDA_LLADDR ] ) {
        ...
        if ( entry->arp_physaddress_len > sizeof( entry->arp_physaddress ) ) {
            Logger_log( LOGGER_PRIORITY_ERR, "netlink hw address length %d is too long\n",
    ...
    case NUD_NONE:
        ...
        break;
    default:
        Logger_log( LOGGER_PRIORITY_ERR, "Unrecognized ARP entry state %d", rtmp->ndm_state );
        break;
    }

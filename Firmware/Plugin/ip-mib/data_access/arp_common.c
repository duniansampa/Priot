    ...
    netsnmp_arp_entry* new_data )
{
    ...
    if ( entry->arp_physaddress_len != new_data->arp_physaddress_len || memcmp( entry->arp_physaddress, new_data->arp_physaddress, entry->arp_physaddress_len ) != 0 ) {
        modified = 1;
    ...
    if ( modified )
        entry->arp_last_updated = Agent_getAgentUptime();
}

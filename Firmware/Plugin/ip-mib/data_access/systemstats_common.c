 ...
 */
extern int
netsnmp_access_systemstats_container_arch_load( Container_Container* container,
    u_int load_flags );
extern void
...
netsnmp_access_systemstats_container_init( u_int flags )
{
    ...
    DEBUG_MSGTL( ( "access:systemstats:container", "init\n" ) );

    /*
     * create the containers. one indexed by ifIndex, the other
     * indexed by ifName.
     */
    container = Container_find( "accessSystemstats:tableContainer" );
    if ( NULL == container )
        return NULL;
 ...
 */
Container_Container*
netsnmp_access_systemstats_container_load( Container_Container* container, u_int load_flags )
{
    int rc;

    DEBUG_MSGTL( ( "access:systemstats:container", "load\n" ) );

    if ( NULL == container ) {
        container = netsnmp_access_systemstats_container_init( load_flags );
        if ( NULL != container )
            container->containerName = strdup( "systemstats_autocreate" );
    }
    if ( NULL == container ) {
        Logger_log( LOGGER_PRIORITY_ERR, "no container specified/found for access_systemstats\n" );
        return NULL;
    }
...
void netsnmp_access_systemstats_container_free( Container_Container* container, u_int free_flags )
{
    DEBUG_MSGTL( ( "access:systemstats:container", "free\n" ) );

    if ( NULL == container ) {
        Logger_log( LOGGER_PRIORITY_ERR, "invalid container for netsnmp_access_systemstats_free\n" );
        return;
    }
...
netsnmp_access_systemstats_entry_get_by_index( Container_Container* container, oid index )
{
    Types_Index tmp;

    DEBUG_MSGTL( ( "access:systemstats:entry", "by_index\n" ) );
...
{
    DEBUG_MSGTL( ( "access:systemstats:entry", "free\n" ) );
 ...
 * @retval <0 : error
 */
int netsnmp_access_systemstats_entry_update_stats( netsnmp_systemstats_entry* prev_vals,
    netsnmp_systemstats_entry* new_vals )
{
    DEBUG_MSGTL( ( "access:systemstats", "check_wrap\n" ) );

    /*
     * sanity checks
     */
    if ( ( NULL == prev_vals ) || ( NULL == new_vals ) || ( prev_vals->index[ 0 ] != new_vals->index[ 0 ] )
    ...
    if ( NULL == prev_vals->old_stats ) {
        ...
        if ( new_vals->stats.columnAvail[ IPSYSTEMSTATSTABLE_HCINNOROUTES ] )
            if ( 0 != Int64_c64Check32AndUpdate(
                          ...
                          &need_wrap_check ) )
                LOGGER_LOGONCE( ( LOGGER_PRIORITY_ERR,
...
{
    DEBUG_MSGTL( ( "access:systemstats", "copy\n" ) );

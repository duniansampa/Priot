...
{
    DEBUG_MSGTL( ( "verbose:ipAddressTable:ipAddressTable_init_data",
        "called\n" ) );
 ...
 */
void ipAddressTable_container_init( Container_Container** container_ptr_ptr,
    Cache* cache )
{
    DEBUG_MSGTL( ( "verbose:ipAddressTable:ipAddressTable_container_init",
        "called\n" ) );
     ...
     * assigned...
     */
    *container_ptr_ptr = Container_find( "ipAddressTable:tableContainer" );
    if ( NULL != *container_ptr_ptr ) {
        ( *container_ptr_ptr )->containerName = strdup( "ipAddressTable" );
        ipAddressTable_container_load( *container_ptr_ptr );
        CONTAINER_FOR_EACH( *container_ptr_ptr,
            ( Container_FuncObjFunc* )_clear_times,
            NULL );
    }
    ...
    cache->flags |= ( CacheOperation_DONT_AUTO_RELEASE | CacheOperation_DONT_FREE_EXPIRED
        | CacheOperation_DONT_FREE_BEFORE_LOAD | CacheOperation_AUTO_RELOAD
        | CacheOperation_DONT_INVALIDATE_ON_SET );
 ...
 */
void ipAddressTable_container_shutdown( Container_Container* container_ptr )
{
    DEBUG_MSGTL( ( "verbose:ipAddressTable:ipAddressTable_container_shutdown",
        "called\n" ) );
...
_check_entry_for_updates( ipAddressTable_rowreq_ctx* rowreq_ctx,
    void** magic )
{
    Container_Container* ipaddress_container = ( Container_Container* )magic[ 0 ];
    Container_Container* to_delete = ( Container_Container* )magic[ 1 ];
    ...
    if ( NULL == ipaddress_entry ) {
        DEBUG_MSGTL( ( "ipAddressTable:access", "removing missing entry\n" ) );

        if ( NULL == to_delete ) {
            magic[ 1 ] = to_delete = Container_find( "lifo" );
            if ( NULL == to_delete )
                Logger_log( LOGGER_PRIORITY_ERR, "couldn't create delete container\n" );
        }
        if ( NULL != to_delete )
            CONTAINER_INSERT( to_delete, rowreq_ctx );
    } else {
        DEBUG_MSGTL( ( "ipAddressTable:access", "updating existing entry\n" ) );
    ...
    Container_Container* container )
{
    ...
    DEBUG_MSGTL( ( "ipAddressTable:access", "creating new entry\n" ) );

    Assert_assert( NULL != ipaddress_entry );
    ...
    if ( ( NULL != rowreq_ctx ) && ( MFD_SUCCESS == ipAddressTable_indexes_set( rowreq_ctx,
                                                        ipaddress_entry->ia_address_len,
                                                        ipaddress_entry->ia_address,
                                                        ipaddress_entry->ia_address_len ) ) ) {
        if ( CONTAINER_INSERT( container, rowreq_ctx ) < 0 ) {
            DEBUG_MSGTL( ( "ipAddressTable:access", "container insert failed for new entry\n" ) );
            ipAddressTable_release_rowreq_ctx( rowreq_ctx );
            return;
        }
        rowreq_ctx->ipAddressLastChanged = rowreq_ctx->ipAddressCreated = Agent_getAgentUptime();
    } else {
        if ( NULL != rowreq_ctx ) {
 ...
 *
 */
int ipAddressTable_container_load( Container_Container* container )
{
    Container_Container* ipaddress_container;
    void* tmp_ptr[ 2 ];

    DEBUG_MSGTL( ( "verbose:ipAddressTable:ipAddressTable_cache_load",
        "called\n" ) );
     ...
     * the container.
     */
    ipaddress_container = netsnmp_access_ipaddress_container_load( NULL,
    ...
    CONTAINER_FOR_EACH( ipaddress_container,
        ( Container_FuncObjFunc* )_add_new_entry,
        container );
     ...
     */
    if ( NULL != tmp_ptr[ 1 ] ) {
        Container_Container* tmp_container = ( Container_Container* )tmp_ptr[ 1 ];
    ...
    DEBUG_MSGT( ( "verbose:ipAddressTable:ipAddressTable_cache_load",
 ...
 *
 */
void ipAddressTable_container_free( Container_Container* container )
{
    DEBUG_MSGTL( ( "verbose:ipAddressTable:ipAddressTable_container_free",
        "called\n" ) );
...
{
    DEBUG_MSGTL( ( "verbose:ipAddressTable:ipAddressTable_row_prep",
        "called\n" ) );
...
{
    DEBUG_MSGTL( ( "verbose:ipAddressTable:ipAddressAddrType_check_index",
        "called\n" ) );
    ...
    default:
        DEBUG_MSGT( ( "ipAddressTable", "illegal addr type\n" ) );
        return MFD_ERROR;
    }
...
{
    DEBUG_MSGTL( ( "verbose:ipAddressTable:ipAddressAddr_check_index",
        "called\n" ) );
    ...
    case INETADDRESSTYPE_IPV4:
        if ( 4 != rowreq_ctx->tbl_idx.ipAddressAddr_len ) {
            DEBUG_MSGT( ( "ipAddressTable", "bad addr len\n" ) );
            return MFD_ERROR;
        }
        break;
 ...
 */
int ipAddressTable_validate_index( ipAddressTable_registration*
                                       ipAddressTable_reg,
    ipAddressTable_rowreq_ctx* rowreq_ctx )
{
    int rc = MFD_SUCCESS;

    DEBUG_MSGTL( ( "verbose:ipAddressTable:ipAddressTable_validate_index",
        "called\n" ) );
    ...
    if ( ( 4 != rowreq_ctx->tbl_idx.ipAddressAddr_len )
        ...
        Logger_log( LOGGER_PRIORITY_WARNING, "invalid index for a new row in the "

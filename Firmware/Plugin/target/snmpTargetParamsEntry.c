    ...
    if ( ( prev_struct = curr_struct = *listPtr ) == NULL ) {
        *listPtr = newEntry;
        ...
        while ( curr_struct != NULL ) {
            ...
            i = Api_oidCompare( newOID, newOIDLen, currOID, currOIDLen );
...
{
    if ( ( entry->mpModel < 0 ) || ( entry->secModel < 0 ) || ( entry->secLevel < 0 ) || ( entry->secName == NULL ) )
        return 0;
    else
        return 1;
...
void init_snmpTargetParamsEntry( void )
{
    ...
    REGISTER_MIB( "target/snmpTargetParamsEntry",
        snmpTargetParamsEntry_variables, Variable2_s,
        snmpTargetParamsEntry_variables_oid );

    AgentReadConfig_priotdRegisterConfigHandler( "targetParams",
    ...
    char* cptr )
{
     ...
     */
        ...
        DEBUG_MSG( ( "snmpTargetParamsEntry",
    ...
    char* cptr )
{
     ...
     */
        ...
        DEBUG_MSG( ( "snmpTargetParamsEntry",
            ...
            PRIOT_ROW_ACTIVE, PRIOT_ROW_NOTINSERVICE,
            PRIOT_ROW_NOTREADY ) );
...
void snmpd_parse_config_targetParams( const char* token, char* char_ptr )
{
    ...
    cptr = ReadConfig_copyNword( cptr, buff, sizeof( buff ) );
    ...
    void* clientarg )
{
    ...
    if ( ( curr_struct = aPTable ) != NULL ) {
        while ( curr_struct != NULL ) {
            if ( ( curr_struct->storageType == PRIOT_STORAGE_NONVOLATILE || curr_struct->storageType == PRIOT_STORAGE_PERMANENT )
                && ( curr_struct->rowStatus == PRIOT_ROW_ACTIVE || curr_struct->rowStatus == PRIOT_ROW_NOTINSERVICE ) ) {
...
u_char*
var_snmpTargetParamsEntry( struct Variable_s* vp,
    oid* name,
    size_t* length,
    int exact,
 ...
 */
int write_snmpTargetParamsMPModel( int action,
    u_char* var_val,
    u_char var_val_type,
    size_t var_val_len,
    u_char* statP, oid* name, size_t name_len )
{
    ...
    if ( action == IMPL_RESERVE1 ) {
        ...
        if ( params == NULL ) {
             ...
             */
            if ( params->rowStatus == PRIOT_ROW_ACTIVE ) {
                DEBUG_MSGTL( ( "snmpTargetParamsEntry",
            ...
            if ( params->rowStatus == PRIOT_ROW_NOTREADY && snmpTargetParams_rowStatusCheck( params ) ) {
                params->rowStatus = PRIOT_ROW_NOTINSERVICE;
            }
        }
 ...
 */
int write_snmpTargetParamsSecModel( int action,
    u_char* var_val,
    u_char var_val_type,
    size_t var_val_len,
    u_char* statP, oid* name, size_t name_len )
 ...
 */
int write_snmpTargetParamsSecLevel( int action,
    u_char* var_val,
    u_char var_val_type,
    size_t var_val_len,
    u_char* statP, oid* name, size_t name_len )
 ...
 */
int write_snmpTargetParamsStorageType( int action,
    u_char* var_val,
    u_char var_val_type,
    size_t var_val_len,
    u_char* statP,
    oid* name, size_t name_len )
{
    ...
    if ( action == IMPL_RESERVE1 ) {
        ...
        if ( params == NULL ) {
             ...
             * Row exists, check if it is changeable.  
             */
            if ( params->storageType == PRIOT_STORAGE_READONLY || params->storageType == PRIOT_STORAGE_PERMANENT ) {
                DEBUG_MSGTL( ( "snmpTargetParamsEntry",
        ...
        if ( ( params = search_snmpTargetParamsTable( snmpTargetParamsOID,
            ...
            != NULL ) {
            if ( params->storageType != PRIOT_STORAGE_READONLY
                && params->storageType != PRIOT_STORAGE_PERMANENT ) {
 ...
 */
int write_snmpTargetParamsSecName( int action,
    u_char* var_val,
    u_char var_val_type,
    size_t var_val_len,
    u_char* statP, oid* name, size_t name_len )
 ...
 */
int write_snmpTargetParamsRowStatus( int action,
    u_char* var_val,
    u_char var_val_type,
    size_t var_val_len,
    u_char* statP,
    oid* name, size_t name_len )
{
    ...
    if ( action == IMPL_RESERVE1 ) {
        ...
        value = *( ( long* )var_val );
        if ( value == PRIOT_ROW_NOTREADY || value < 1 || value > 6 ) {
            return PRIOT_ERR_WRONGVALUE;
        }
        ...
        if ( name_len < snmpTargetParamsOIDLen + 1 || name_len > snmpTargetParamsOIDLen + 32 ) {
            DEBUG_MSGTL( ( "snmpTargetParamsEntry", "bad index length %d\n",
        ...
        params = search_snmpTargetParamsTable( snmpTargetParamsOID,
            ...
            name, &name_len, 1 );
        if ( params != NULL ) {
            if ( value == PRIOT_ROW_CREATEANDGO
                || value == PRIOT_ROW_CREATEANDWAIT ) {
                value = PRIOT_ROW_NOTREADY;
                return PRIOT_ERR_INCONSISTENTVALUE;
            }
            if ( params->storageType == PRIOT_STORAGE_READONLY ) {
                DEBUG_MSGTL( ( "snmpTargetParamsEntry",
            ...
            if ( params->storageType == PRIOT_STORAGE_PERMANENT ) {
                if ( value == PRIOT_ROW_DESTROY ) {
                    DEBUG_MSGTL( ( "snmpTargetParamsEntry",
            ...
            if ( value == PRIOT_ROW_ACTIVE || value == PRIOT_ROW_NOTINSERVICE ) {
                return PRIOT_ERR_INCONSISTENTVALUE;
            }
            if ( value == PRIOT_ROW_CREATEANDGO
                || value == PRIOT_ROW_CREATEANDWAIT ) {
        ...
        if ( params != NULL ) {
            ...
            } else if ( value == PRIOT_ROW_CREATEANDWAIT ) {
                /*
                 * Check whether all the required objects have been set.  
                 */
                if ( snmpTargetParams_rowStatusCheck( params ) ) {
            ...
            } else if ( value == PRIOT_ROW_ACTIVE ) {
                if ( params->rowStatus == PRIOT_ROW_NOTINSERVICE ) {
                    params->rowStatus = PRIOT_ROW_ACTIVE;
                } else if ( params->rowStatus == PRIOT_ROW_NOTREADY ) {
                    return PRIOT_ERR_INCONSISTENTVALUE;
                }
            } else if ( value == PRIOT_ROW_NOTINSERVICE ) {
                if ( params->rowStatus == PRIOT_ROW_ACTIVE ) {
                    params->rowStatus = PRIOT_ROW_NOTINSERVICE;
                } else if ( params->rowStatus == PRIOT_ROW_NOTREADY ) {
                    return PRIOT_ERR_INCONSISTENTVALUE;

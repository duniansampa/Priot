 ...
 *
 */
int header_simple_table( struct Variable_s* vp, oid* name, size_t* length,
    int exact, size_t* var_len,
    WriteMethodFT** write_method, int max )
{
    ...
    if ( ( ( int )*length ) <= ( int )vp->namelen || rtest == -1 ) {
        ...
        *length = vp->namelen + 1;
        ...
        if ( name[ *length - 1 ] < TYPES_OID_MAX_SUBID ) {
    ...
    if ( ( max >= 0 && ( ( int )newname[ *length - 1 ] > max ) ) || ( 0 == newname[ *length - 1 ] ) ) {
        if ( var_len )
            *var_len = 0;
        return MATCH_FAILED;
    }

...
int get_exec_output( struct extensible* ex )
{
    ...
    DEBUG_MSGTL( ( "exec:get_exec_output", "calling %s\n", ex->command ) );

    sprintf( cachefile, "%s/%s", ReadConfig_getPersistentDirectory(), NETSNMP_CACHEFILE );
    curtime = time( NULL );
    if ( curtime > ( cachetime + NETSNMP_EXCACHETIME ) || strcmp( ex->command, lastcmd ) != 0 ) {
        ...
        ex->result = ExecuteCmd_runExecCommand( ex->command, NULL, cache, &cachebytes );
        ...
        if ( ( cfd = open( cachefile, O_WRONLY | O_TRUNC | O_CREAT, 0600 ) ) < 0 ) {
            Logger_log( LOGGER_PRIORITY_ERR, "can not create cache file\n" );
            Logger_logPerror( cachefile );
            cachetime = 0;
            return -1;
        }
        if ( cachebytes > 0 )
    ...
    DEBUG_MSGTL( ( "exec:get_exec_output", "using cached value\n" ) );
    if ( ( cfd = open( cachefile, O_RDONLY ) ) < 0 ) {
        Logger_log( LOGGER_PRIORITY_ERR, "can not open cache file\n" );
        Logger_logPerror( cachefile );
        return -1;
    }
    return ( cfd );
...
int get_exec_pipes( char* cmd, int* fdIn, int* fdOut, Types_PidT* pid )
{
...
*/
    ...
    if ( pipe( fd[ 0 ] ) || pipe( fd[ 1 ] ) ) {
        Logger_logPerror( "pipe" );
        return 0;
    }
    if ( ( *pid = fork() ) == 0 ) { /* First handle for the child */
        close( 0 );
        if ( dup( fd[ 0 ][ 0 ] ) != 0 ) {
            Logger_logPerror( "dup 0" );
            return 0;
        }
        close( 1 );
        if ( dup( fd[ 1 ][ 1 ] ) != 1 ) {
            Logger_logPerror( "dup 1" );
            return 0;
        }
        ...
        for ( cnt = 1, cptr1 = cmd, cptr2 = argvs; *cptr1 != 0;
            ...
            *cptr2 = *cptr1;
            if ( *cptr1 == ' ' ) {
                *( cptr2++ ) = 0;
                if ( ( cptr1 = ReadConfig_skipWhite( cptr1 ) ) == NULL )
                    break;
                *cptr2 = *cptr1;
                if ( *cptr1 != 0 )
                    cnt++;
        ...
        *( aptr++ ) = argvs;
        ...
        while ( *cptr2 != 0 )
            cptr2++;
        *( aptr++ ) = NULL;
        ReadConfig_copyNword( cmd, ctmp, sizeof( ctmp ) );
        execv( ctmp, argv );
        perror( ctmp );
        exit( 1 );
    } else {
...
void print_mib_oid( oid name[], size_t len )
{
    ...
    if ( !buffer ) {
        Logger_log( LOGGER_PRIORITY_ERR, "Malloc failed - out of memory?" );
        return;
    }
    sprint_mib_oid( buffer, name, len );
    Logger_log( LOGGER_PRIORITY_NOTICE, "Mib: %s\n", buffer );
    free( buffer );
}
 ...
 */
int checkmib( struct Variable_s* vp, oid* name, size_t* length,
    int exact, size_t* var_len, WriteMethodFT** write_method, int max )
{
    /*
...
char* find_field( char* ptr, int field )
{
    ...
    if ( field == NETSNMP_LASTFIELD ) {
        ...
        if ( ( ptr = ReadConfig_skipWhite( ptr ) ) == NULL )
            return ( NULL );
        for ( i = 1; *ptr != 0 && i != field; i++ ) {
            if ( ( ptr = ReadConfig_skipNotWhite( ptr ) ) == NULL )
                return ( NULL );
            if ( ( ptr = ReadConfig_skipWhite( ptr ) ) == NULL )
                return ( NULL );
        }
    ...
    prefix_cbx* node_to_find )
{
    ...
    if ( iret < 0 ) {
        DEBUG_MSGTL( ( "util_funcs:prefix", "Unable to search the list\n" ) );
        return -1;
    } else if ( !iret ) {
        DEBUG_MSGTL( ( "util_funcs:prefix", "Could not find prefix info\n" ) );
        return -1;
    } else
        return 0;
}
    ...
    prefix_cbx* node_to_update )
{
    ...
    if ( iret < 0 ) {
        DEBUG_MSGTL( ( "util_funcs:prefix", "Unable to update prefix info\n" ) );
        return -1;
    } else if ( !iret ) {
        DEBUG_MSGTL( ( "util_funcs:prefix", "Unable to find the node to update\n" ) );
        return -1;
    } else
        return 0;
}
    ...
    int functionality )
{
    ...
    * 1 - Need to do only search. Provide the node_to_use with in6p value filled                    */
    ...
    Assert_assert( NULL != node_to_use );

...
Instance_numFileInstanceRef(NumFileInstance *nfi)
{
    nfi->refcnt++;
    return nfi;
}
...
static void
Instance_numFileInstanceDeref(NumFileInstance *nfi)
{
    if (--nfi->refcnt == 0) {
    free(nfi->file_name);
    free(nfi);
...
static
HandlerRegistration *
Instance_getReg(const char *name,
        const char *ourname,
        ...
        const char *contextName)
{
    ...
    if (subhandler) {
        ...
        myhandler->myvoid = it;
    myhandler->data_clone = (void*(*)(void*))Instance_numFileInstanceRef;
    myhandler->data_free = (void(*)(void*))Instance_numFileInstanceDeref;
    ...
    myreg->handler->data_clone
        = (void *(*)(void *))Instance_numFileInstanceRef;
    myreg->handler->data_free
        = (void (*)(void *))Instance_numFileInstanceDeref;
    }
    if (contextName)
                                   ...
                                   const char *contextName)
{
    ...
    nfi->refcnt = 1;
    myreg = Instance_getReg(name, "file_num_handler", reg_oid, reg_oid_len, nfi,
                    mode, Instance_numFileHandler,
                    subhandler, contextName);
    if (NULL == myreg) {
        Instance_numFileInstanceDeref(nfi);
        return MIB_REGISTRATION_FAILED;
    }

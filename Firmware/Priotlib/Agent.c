...
#include "Enum.h"
#include "../Modlib/Agentx/Master.h"
...
void
Agent_addListData(AgentRequestInfo *ari,
...
int
Agent_removeListData(AgentRequestInfo *ari,
                              const char * name)
{
...
void*
Agent_getListData(AgentRequestInfo *ari,
                           const char *name)
{
   if (ari) {
...
void
Agent_freeAgentDataSet(AgentRequestInfo *ari)
{
   if (ari) {
...
void
Agent_freeAgentDataSets(AgentRequestInfo *ari)
{
   if (ari) {
...
void
Agent_freeAgentRequestInfo(AgentRequestInfo *ari)
{
   if (ari) {
       if (ari->agent_data) {
           DataList_freeAll(ari->agent_data);
...
static struct AddrCache_s agent_addrCache[SNMP_ADDRCACHE_SIZE];
...
static AgentNsap *agent_agentNsapList = NULL;
static AgentSession *agent_sessionList = NULL;
AgentSession *agent_processingSet = NULL;
AgentSession *agent_delegated_list = NULL;
AgentSession *agent_agentQueuedList = NULL;

static int      agent_currentGlobalid = 0;

int      agent_running = 1;

int
Agent_allocateGlobalcacheid(void)
{
   return ++agent_currentGlobalid;
}

int
Agent_getLocalCachid(Cachemap *cache_store, int globalid)
{
...
void
Agent_freeCachemap(Cachemap *cache_store)
{
   Cachemap *tmpp;
   while (cache_store) {
       tmpp = cache_store;
...
static AgentSetCache *agent_sets = NULL;
...
int
Agent_getSetCache(AgentSession *asp)
{
   AgentSetCache *ptr, *prev = NULL;

   for (ptr = agent_sets; ptr != NULL; ptr = ptr->next) {
...
*/
 void
Agent_reorderGetbulk(AgentSession *asp)
{
   int             i, n = 0, r = 0;
   ...
   for (i = 0; i < r - 1; i++) {
       for (j = 0; j < repeats; j++) {
           asp->bulkcache[i * repeats + j]->nextVariable =
   ...
   asp->bulkcache[(r - 1) * repeats + j]->nextVariable =
       asp->bulkcache[j + 1];
   }
...
*/
 void
Agent_fixEndofmibview(AgentSession *asp)
{
...
*/
int
Agent_checkAndProcess(int block)
{
   int             numfds;
   fd_set          fdset;
...
*/
void
Agent_addrcacheInitialise(void)
{
   int             i = 0;

   for (i = 0; i < SNMP_ADDRCACHE_SIZE; i++) {
       agent_addrCache[i].addr = NULL;
       agent_addrCache[i].status = SNMP_ADDRCACHE_UNUSED;
   }
}
...
void Agent_addrcacheDestroy(void)
{
   ...
   for (i = 0; i < SNMP_ADDRCACHE_SIZE; i++) {
       if (agent_addrCache[i].status == SNMP_ADDRCACHE_USED) {
           free(agent_addrCache[i].addr);
           agent_addrCache[i].status = SNMP_ADDRCACHE_UNUSED;
       }
   }
}
...
*/
int
Agent_addrcacheAdd(const char *addr)
{
    ...
    */
   for(i = 0; i < SNMP_ADDRCACHE_SIZE; i++) {
       if (agent_addrCache[i].status == SNMP_ADDRCACHE_UNUSED) { /* If unused */
           /*
           ...
           if ((NULL != addr) && (strcmp(agent_addrCache[i].addr, addr) == 0)) {
               /*
                * found a match
                */
               agent_addrCache[i].lastHitM = now;
               if (timercmp(&agent_addrCache[i].lastHitM, &aged, <))
           ...
           rc = 0; /* not expired, so is existing entry */
               ...
               if (timercmp(&agent_addrCache[i].lastHitM, &aged, <)) {
                    ...
                    * Stale, reuse
                    */
                   TOOLS_FREE(agent_addrCache[i].addr);
                   agent_addrCache[i].status = SNMP_ADDRCACHE_UNUSED;
                   /*
                    * remember this location, in case addr isn't in the cache
                    */
           if (unused < 0)
                   ...
                   if (oldest < 0)
                       oldest = i;
                   else if (timercmp(&agent_addrCache[i].lastHitM,
                                     &agent_addrCache[oldest].lastHitM, <))
                       oldest = i;
               } /* fresh */
           } /* used, no match */
       } /* used */
   ...
   if ((-1 == rc) && (NULL != addr)) {
       ...
       if (unused >= 0) {
            ...
            */
           agent_addrCache[unused].addr = strdup(addr);
           agent_addrCache[unused].status = SNMP_ADDRCACHE_USED;
           ...
           free(agent_addrCache[oldest].addr);
           agent_addrCache[oldest].addr = strdup(addr);
           agent_addrCache[oldest].lastHitM = now;
       }
       rc = 1;
   }
...
int
Agent_checkPacket( Types_Session * session,
...
int
Agent_checkParse(Types_Session * session, Types_Pdu *pdu,
                         int result)
{
   if (result == 0) {
       ...
       DefaultStore_getBoolean(DsStorage_APPLICATION_ID,
           ...
           case PRIOT_MSG_INTERNAL_SET_RESERVE1:
               Logger_log(LOGGER_PRIORITY_DEBUG, "  INTERNAL RESERVE1 message\n");
               break;

           case PRIOT_MSG_INTERNAL_SET_RESERVE2:
               Logger_log(LOGGER_PRIORITY_DEBUG, "  INTERNAL RESERVE2 message\n");
               break;

           case PRIOT_MSG_INTERNAL_SET_ACTION:
               Logger_log(LOGGER_PRIORITY_DEBUG, "  INTERNAL ACTION message\n");
               break;

           case PRIOT_MSG_INTERNAL_SET_COMMIT:
               Logger_log(LOGGER_PRIORITY_DEBUG, "  INTERNAL COMMIT message\n");
               break;

           case PRIOT_MSG_INTERNAL_SET_FREE:
               Logger_log(LOGGER_PRIORITY_DEBUG, "  INTERNAL FREE message\n");
               break;

           case PRIOT_MSG_INTERNAL_SET_UNDO:
               Logger_log(LOGGER_PRIORITY_DEBUG, "  INTERNAL UNDO message\n");
               break;
...
int
Agent_registerAgentNsap(Transport_Transport *t)
{
   Types_Session *s, *sp = NULL;
   AgentNsap     *a = NULL, *n = NULL, **prevNext = &agent_agentNsapList;
   int             handle = 0;
   void           *isp = NULL;
   ...
   for (a = agent_agentNsapList; a != NULL && handle + 1 >= a->handle;
        a = a->next) {
       handle = a->handle;
       prevNext = &(a->next);
   }
...
void
Agent_deregisterAgentNsap(int handle)
{
   AgentNsap     *a = NULL, **prevNext = &agent_agentNsapList;
   int             main_session_deregistered = 0;

   DEBUG_MSGTL(("Agent_deregisterAgentNsap", "handle %d\n", handle));

   for (a = agent_agentNsapList; a != NULL && a->handle < handle; a = a->next) {
       prevNext = &(a->next);
   }
   ...
   if (main_session_deregistered) {
       if (agent_agentNsapList != NULL) {
           DEBUG_MSGTL(("snmp_agent",
           "WARNING: agent_mainSession ptr changed from %p to %p\n",
                       agent_mainSession, Api_sessSession(agent_agentNsapList->s)));
           agent_mainSession = Api_sessSession(agent_agentNsapList->s);
       } else {
           DEBUG_MSGTL(("snmp_agent",
...
Agent_initMasterAgent(void)
{
   ...
   if (st)
       *st++ = '\0';
       ...
       transport = Transport_openServer("snmp", cptr);
...
void
Agent_clearNsapList(void)
{
   DEBUG_MSGTL(("Agent_clearNsapList", "clear the nsap list\n"));

   while (agent_agentNsapList != NULL)
   Agent_deregisterAgentNsap(agent_agentNsapList->handle);
}

void
Agent_shutdownMasterAgent(void)
{
   Agent_clearNsapList();
}
...
void
Agent_freeAgentPriotSession(AgentSession *asp)
{
   if (!asp)
       return;
...
int
Agent_checkForDelegated(AgentSession *asp)
{
   int             i;
   RequestInfo *request;
...
int
Agent_checkDelegatedChainFor(AgentSession *asp)
{
   AgentSession *asptmp;
...
int
Agent_checkForDelegatedAndAdd(AgentSession *asp)
{
...
int
Agent_removeFromDelegated(AgentSession *asp)
{
...
*/
int
Agent_removeDelegatedRequestsForSession(Types_Session *sess)
{
   AgentSession *asp;
   int count = 0;
...
int
Agent_checkQueuedChainFor(AgentSession *asp)
{
   AgentSession *asptmp;
...
int
Agent_addQueued(AgentSession *asp)
{
...
int
Agent_wrapUpRequest(AgentSession *asp, int status)
{
   /*
...
#define INCLUDE_V2ERRORS_IN_V1STATS
   ...
   if ((status == PRIOT_ERR_NOERROR) && (asp->pdu)) {
       Api_incrementStatisticBy((asp->pdu->command == PRIOT_MSG_SET ?
                                    API_STAT_SNMPINTOTALSETVARS :
                                    API_STAT_SNMPINTOTALREQVARS),
   ...
   if (asp->pdu) {
       ...
       if (!Api_send(asp->session, asp->pdu) &&
            asp->session->s_snmp_errno != ErrorCode_SUCCESS) {
           Types_VariableList *var_ptr;
...
void
Agent_dumpSessList(void)
{
   AgentSession *a;

   DEBUG_MSGTL(("snmp_agent", "DUMP agent_sess_list -> "));
   for (a = agent_sessionList; a != NULL; a = a->next) {
...
void
Agent_removeAndFreeAgentPriotSession(AgentSession *asp)
{
   AgentSession *a, **prevNext = &agent_sessionList;

   DEBUG_MSGTL(("snmp_agent", "REMOVE session == %8p\n", asp));

   for (a = agent_sessionList; a != NULL; a = *prevNext) {
       if (a == asp) {
           *prevNext = a->next;
           a->next = NULL;
...
void
Agent_freeAgentPriotSessionBySession(Types_Session * sess,
                                          void (*free_request)
                                          (Api_RequestList *))
{
   AgentSession *a, *next, **prevNext = &agent_sessionList;

   DEBUG_MSGTL(("snmp_agent", "REMOVE session == %8p\n", sess));

   for (a = agent_sessionList; a != NULL; a = next) {
       if (a->session == sess) {
           *prevNext = a->next;
           next = a->next;
           Agent_freeAgentPriotSession(a);
...
int
Agent_handlePriotPacket(int op, Types_Session * session, int reqid,
                  Types_Pdu *pdu, void *magic)
{
    ...
    * might.
    */
   if (pdu->command == PRIOT_MSG_TRAP || pdu->command == PRIOT_MSG_INFORM ||
   ...
   if (pdu->version  == PRIOT_VERSION_3 &&
       session->s_snmp_errno == ErrorCode_USM_AUTHENTICATIONFAILURE) {
                            ...
                            Subtree *tp)
{
   ...
   if (tp &&
       (asp->pdu->command == PRIOT_MSG_GETNEXT ||
        asp->pdu->command == PRIOT_MSG_GETBULK)) {
       int result;
       int prefix_len;
       ...
       if (prefix_len < 1) {
           ...
           result =
               AgentRegistry_acmCheckSubtree(asp->pdu, tp->start_a, prefix_len);
       }
       ...
       while (result == VACM_NOTINVIEW) {
           ...
           if (tp) {
               ...
               if (prefix_len < 1) {
                   ...
                   result =
                       AgentRegistry_acmCheckSubtree(asp->pdu,
   ...
   if (tp == NULL) {
       ...
       if (tp->global_cacheid) {
           ...
           if (asp->cache_store && -1 !=
               (cacheid = Agent_getLocalCachid(asp->cache_store,
                                                   tp->global_cacheid))) {
           } else {
...
#define CACHE_GROW_SIZE 16
               ...
               asp->treecache =
                   (TreeCache *)realloc(asp->treecache,
                           sizeof(TreeCache) *
                           asp->treecache_len);
        ...
        */
       if (asp->pdu->command == PRIOT_MSG_GETNEXT ||
           asp->pdu->command == PRIOT_MSG_GETBULK) {
...
*/
int
Agent_checkAcm(AgentSession *asp, u_char type)
{
   int             view;
   int             i, j, k;
   RequestInfo *request;
   ...
   for (i = 0; i <= asp->treecache_num; i++) {
       for (request = asp->treecache[i].requests_begin;
           ...
           for(j = request->repeat, vb = request->requestvb_start;
               ...
               if (vb->type != ASN01_NULL &&
                   vb->type != ASN01_PRIV_RETRY) { /* not yet processed */
                   view =
                       AgentRegistry_inAView(vb->name, &vb->nameLength,
                                 asp->pdu, vb->type);
                   ...
                   if (view != VACM_SUCCESS) {
                       ...
                       if (request->repeat < request->orig_repeat) {
                           ...
                           if (j > -1 && vb->nextVariable &&
                               vb->nextVariable->type != ASN01_NULL &&
                               vb->nextVariable->type != ASN01_PRIV_RETRY) {
...
int
Agent_createSubtreeCache(AgentSession *asp)
{
   Subtree *tp;
   ...
   if (asp->treecache == NULL && asp->treecache_len == 0) {
       ...
       asp->treecache =
           (TreeCache *)calloc(asp->treecache_len, sizeof(TreeCache));
       if (asp->treecache == NULL)
           return PRIOT_ERR_GENERR;
   }
   asp->treecache_num = -1;
           ...
           if (n > 0) {
               ...
               if (asp->pdu->errindex > 0) {
                   ...
                   for (i = 1; i < asp->pdu->errindex; i++) {
                       vbptr->nextVariable =
                           TOOLS_MALLOC_STRUCT(Types_VariableList_s);
                       /*
...
*/
int
Agent_reassignRequests(AgentSession *asp)
{
   /*
    ...
    */
   asp->treecache =
       (TreeCache *) calloc(asp->treecache_len,
                                     sizeof(TreeCache));
...
void
Agent_deleteRequestInfos(RequestInfo *reqlist)
{
   while (reqlist) {
...
void
Agent_deleteSubtreeCache(AgentSession *asp)
{
   while (asp->treecache_num >= 0) {
       /*
        * don't delete subtrees
        */
       Agent_deleteRequestInfos(asp->treecache[asp->treecache_num].
                                    requests_begin);
       asp->treecache_num--;
   }
}
...
*/
int
Agent_checkAllRequestsError(AgentSession *asp,
                                int look_for_specific)
{
    ...
    */
   for( i = 0; i < asp->vbcount; ++i ) {
       if ((PRIOT_ERR_NOERROR != asp->requests[i].status) &&
           (!look_for_specific ||
...
int
Agent_checkRequestsError(RequestInfo *requests)
{
   /*
    * find any errors marked in the requests
    */
...
int
Agent_checkRequestsStatus(AgentSession *asp,
                             RequestInfo *requests,
                             int look_for_specific)
{
   /*
...
int
Agent_checkAllRequestsStatus(AgentSession *asp,
                                 int look_for_specific)
{
   int             i;
   for (i = 0; i <= asp->treecache_num; i++) {
...
int
Agent_handleVarRequests(AgentSession *asp)
{
...
*/
void
Agent_checkOutstandingAgentRequests(void)
{
   ...
   while (agent_agentQueuedList) {
        ...
        */
       Assert_assert((!agent_processingSet) ||
        ...
        */
       if ((agent_agentQueuedList->pdu->command == PRIOT_MSG_SET) &&
...
int
Agent_checkTransactionId(int transaction_id)
{
   AgentSession *asp;
...
int
Agent_checkDelayedRequest(AgentSession *asp)
{
   ...
   case PRIOT_MSG_GETNEXT:
       ...
       if (Agent_checkForDelegated(asp) &&
           Agent_checkTransactionId(asp->pdu->transid) !=
           ErrorCode_SUCCESS) {
           /*
            * add to delegated request chain
            */
...
int
Agent_checkGetnextResults(AgentSession *asp)
{
   /*
    * get old info
    */
   ...
   for (i = 0; i <= old_treecache_num; i++) {
       for (request = old_treecache[i].requests_begin; request;
           ...
           if (special) {
               ...
               } else if (request->requestvb->type == ASN01_NULL ||
                          request->requestvb->type == PRIOT_NOSUCHINSTANCE ||
                          request->requestvb->type == PRIOT_NOSUCHOBJECT) {
                   /*
...
int
Agent_handleGetnextLoop(AgentSession *asp)
{
   int             status;
   Types_VariableList *var_ptr;
...
int
Agent_handleSet(AgentSession *asp)
{
   int             status;
   /*
   ...
   if (asp->mode != IMPL_FINISHED_SUCCESS && asp->mode != IMPL_FINISHED_FAILURE) {
       DEBUG_MSGTL(("agent_set", "doing set mode = %d (%s)\n", asp->mode,
                   Enum_seFindLabelInSlist("agent_mode", asp->mode)));
       status = Agent_handleVarRequests(asp);
       DEBUG_MSGTL(("agent_set", "did set mode = %d, status = %d\n",
                   asp->mode, status));
       if ((status != PRIOT_ERR_NOERROR && asp->status == PRIOT_ERR_NOERROR) ||
       status == PRIOT_ERR_COMMITFAILED ||
       status == PRIOT_ERR_UNDOFAILED) {
           asp->status = status;
       }
   }
   return asp->status;
...
int
Agent_handleSetLoop(AgentSession *asp)
{
...
int
Agent_handleRequest(AgentSession *asp, int status)
{
   /*
    ...
    */
   if ((0 == Agent_checkDelegatedChainFor(asp)) &&
       (asp != agent_processingSet)) {
       /*
...
int
Agent_handlePdu(AgentSession *asp)
{
   ...
   case PRIOT_MSG_GETBULK:
       for (v = asp->pdu->variables; v != NULL; v = v->nextVariable) {
           if (v->type == ASN01_PRIV_INCL_RANGE) {
                ...
                * ASN_NULL in Agent_addVarbindToCache,
...
*/
 int
Agent_requestSetError2(RequestInfo *request, int mode, int error_value)
{
   if (!request)
...
*/
int
Agent_requestSetError(RequestInfo *request, int error_value)
{
   if (!request || !request->agent_req_info)
       return ErrorCode_NO_VARS;

   return Agent_requestSetError2(request, request->agent_req_info->mode,
                             error_value);
}
...
*/
int
Agent_requestSetErrorIdx(RequestInfo *request,
                             int error_value, int idx)
{
   ...
   return Agent_requestSetError2(req, request->agent_req_info->mode,
                             error_value);
}
...
*/
int
Agent_requestSetErrorAll( RequestInfo *requests, int error)
{
   ...
   for(; requests ; requests = requests->next) {
        ...
        */
       if((rc = Agent_requestSetError2(requests, mode, error))) {
...
static struct timeval agent_starttimeM;
...
Agent_getAgentRuntime(void)
{
   ...
   Tools_getMonotonicClock(&now);
   TOOLS_TIMERSUB(&now, &agent_starttimeM, &delta);
...
*/
void
Agent_setAgentStarttime(markerT s)
{
   if (s) {
       ...
       TOOLS_TIMERSUB(&agent_starttime, &nowA, &agent_starttimeM);
       TOOLS_TIMERADD(&agent_starttimeM, &nowM, &agent_starttimeM);
   } else {
       gettimeofday(&agent_starttime, NULL);
       Tools_getMonotonicClock(&agent_starttimeM);
   }
}
...
*/
void
Agent_setAgentUptime(u_long hsec)
{
   struct timeval  nowA, nowM;
   ...
   TOOLS_TIMERSUB(&nowM, &new_uptime, &agent_starttimeM);
}
                         ...
                         RequestInfo *request, int error_value)
{
   ...
   Agent_requestSetError2(request, reqinfo->mode, error_value);
...
*/
int
Agent_setModeRequestError(int mode, RequestInfo *request,
                              int error_value)
{
   Agent_requestSetError2(request, mode, error_value);

   return error_value;
}

/** set error for all request
* \deprecated use Agent_requestSetErrorAll
...
*/
int
Agent_setAllRequestsError(AgentRequestInfo *reqinfo,
                              RequestInfo *requests,
                              int error_value)
{
   Agent_requestSetErrorAll(requests, error_value);
   return error_value;
}

 ...
 */
void
_TableTdata_generateIndexOid(TdataRow *row)
{
...
TableTdata_createTable(const char *name, long flags)
{
    ...
    if (!(table->flags & TDATA_FLAG_NO_CONTAINER)) {
        ...
        if (!table->container)
            table->container = Container_find( "table_container" );
        if (table->container)
                      ...
                      TdataRow *row)
{
    ...
    if (row->indexes)
        _TableTdata_generateIndexOid(row);
...
NodeHandlerFT _TableTdata_helperHandler;
 ...
 */
int
_TableTdata_helperHandler(MibHandler *handler,

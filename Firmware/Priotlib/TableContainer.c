...
static int
TableContainer_handler(MibHandler *handler,
                         HandlerRegistration *reginfo,
                         AgentRequestInfo *agtreq_info,
                         RequestInfo *requests);

static void *
TableContainer_findNextRow(Container_Container *c,
                                 ...
                                 Container_Container *container, long flags )
{
    ...
    if (container)
        ...
        table->table = Container_find("table_container");
        if (!table->table) {
            TOOLS_FREE(table);
            return NULL;
        }
...
TableContainer_dataClone(ContainerTableData *tad)
{
    ++tad->refcnt;
    return tad;
}
...
static void
TableContainer_dataFree(ContainerTableData *tad)
{
    if (--tad->refcnt == 0)
    free(tad);
}
                                    ...
                                    Container_Container *container, char key_type)
{
    ...
    handler = AgentHandler_createHandler("table_container",
                                     TableContainer_handler);
    ...
    if(NULL == container)
        container = Container_find("table_container");
    tad->table = container;
    ...
    handler->data_clone = (void *(*)(void *))TableContainer_dataClone;
    handler->data_free = (void (*)(void *))TableContainer_dataFree;
...
TableContainer_unregister(HandlerRegistration *reginfo)
{
    ...
    tad = (ContainerTableData *)
        AgentHandler_findHandlerDataByName(reginfo, "table_container");
    if (tad) {
 ...
 **********************************************************************/
static inline void
TableContainer_setKey( ContainerTableData * tad, RequestInfo *request,
...
static inline void
TableContainer_dataLookup(HandlerRegistration *reginfo,
            AgentRequestInfo *agtreq_info,
            RequestInfo *request, ContainerTableData * tad)
{
    ...
    DEBUG_IF("table_container") {
        DEBUG_MSGTL(("table_container", "  data_lookup oid:"));
        DEBUG_MSGOID(("table_container", var->name, var->nameLength));
        DEBUG_MSG(("table_container", "\n"));
    }
    ...
    if ((agtreq_info->mode == MODE_GETNEXT) ||
         ...
         * column, if necessary.
         */
        TableContainer_setKey( tad, request, tblreq_info, &key, &index );
        row = (Types_Index*)TableContainer_findNextRow(tad->table, tblreq_info, key);
        if (row) {
            /*
            ...
            DEBUG_MSGTL(("table_container", "no row found\n"));
        }
    } /** GETNEXT/GETBULK */
    else {
                         ...
                         RequestInfo *requests)
{
    ...
    DEBUG_MSGTL(("table_container", "Mode %s, Got request:\n",
                Enum_seFindLabelInSlist("agent_mode",agtreq_info->mode)));
       ...
       || (MODE_SET_RESERVE1 == oldmode)
        ...
        for (curr_request = requests; curr_request; curr_request = curr_request->next) {
            ...
            if (curr_request->processed != 0) {
                DEBUG_MSGTL(("table_container", "already processed\n"));
                continue;
            }
             ...
             */
            TableContainer_dataLookup(reginfo, agtreq_info, curr_request, tad);
...
TableContainer_indexFindNextRow(Container_Container *c,
                                  TableRequestInfo *tblreq)
{
    return (Types_Index*)TableContainer_findNextRow(c, tblreq, NULL );
}

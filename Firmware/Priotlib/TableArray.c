...
static const char * tableArray_modeName[] = {
    "Reserve 1",
    "Reserve 2",
    "Action",
    "Commit",
                             ...
                             int group_rows)
{
    ...
    if (NULL==container) {
        tad->table = Container_find("table_array");
...
static void
TableArray_releaseRequestGroup(Types_Index *g, void *v)
{
...
static void
TableArray_releaseRequestGroups(void *vp)
{
    Container_Container *c = (Container_Container*)vp;
    CONTAINER_FOR_EACH(c, (Container_FuncObjFunc*)
                       TableArray_releaseRequestGroup, NULL);
    CONTAINER_FREE(c);
}

static void
TableArray_processSetGroup(Types_Index *o, void *c)
{
                     ...
                     TableContainerData * tad, char *handler_name)
{
    ...
    CONTAINER_FOR_EACH(request_group,
                       (Container_FuncObjFunc*)TableArray_processSetGroup,
                       &context);
                                   ...
                                   RequestInfo *requests)
{
    ...
    if (agtreq_info->mode < 0 || agtreq_info->mode > 5) {
        DEBUG_MSGTL(("table_array", "Mode %d, Got request:\n",
                    agtreq_info->mode));
    } else {
        DEBUG_MSGTL(("table_array", "Mode %s, Got request:\n",
                    tableArray_modeName[agtreq_info->mode]));
    }
    ...
    if (rc != PRIOT_ERR_NOERROR) {
        DEBUG_MSGTL(("table_array", "processing returned rc %d\n", rc));
    }
    ...
    if (handler->next) {
        ...
        if (rc != PRIOT_ERR_NOERROR) {
            DEBUG_MSGTL(("table_array", "next handler returned rc %d\n", rc));
        }
    }

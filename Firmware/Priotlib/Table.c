...
static void     Table_helperCleanup(AgentRequestInfo *reqinfo,
                                     RequestInfo *request,
                                     int status);
static void     Table_dataFreeFunc(void *data);
static int
Table_sparseTableHelperHandler(MibHandler *handler,
...
int
Table_helperHandler(MibHandler *handler,
                     HandlerRegistration *reginfo,
                     AgentRequestInfo *reqinfo,
                     RequestInfo *requests)
{
...
/** XXX-rks: memory leak. add cleanup handler? */
    ...
    for (request = requests; request; request = request->next) {
         ...
         * table.1 (table.ENTRY).
         */
        else if ((var->nameLength > reginfo->rootoid_len) &&
        ...
        if (out_of_range) {
             ...
             */
            if (reqinfo->mode == MODE_SET_RESERVE1)
                Table_helperCleanup(reqinfo, request,
        ...
        if (var->nameLength > oid_column_pos) {
             ...
             */
                ...
                tbl_req_info->colnum =
                    Table_closestColumn(var->name[oid_column_pos],
             ...
             */
            if (reqinfo->mode == MODE_GET ) {
                Table_helperCleanup(reqinfo, request, PRIOT_NOSUCHOBJECT);
            } else if (reqinfo->mode == MODE_SET_RESERVE1 ) {
                Table_helperCleanup(reqinfo, request, PRIOT_ERR_NOTWRITABLE);
            }
            continue;
        }
        ...
        if ((reqinfo->mode != MODE_GETNEXT) &&
            ...
            if ( MODE_IS_SET(reqinfo->mode) ) {
                ...
                /** set actual error */
                Table_helperCleanup(reqinfo, request, PRIOT_ERR_NOCREATION);
                need_processing = 0; /* don't call next handler */
                break;
            }
            Table_helperCleanup(reqinfo, request, PRIOT_NOSUCHINSTANCE);
            continue;
        }
        Assert_assert(request->status == PRIOT_ERR_NOERROR);
     ...
     */
    if (reqinfo->mode == MODE_GETNEXT)
        Table_sparseTableHelperHandler( handler, reginfo, reqinfo, requests );
                     ...
                     RequestInfo *requests)
{
    ...
    if (reqinfo->mode == MODE_GETNEXT) {
        for(request = requests ; request; request = request->next) {
            if ((request->requestvb->type == ASN01_NULL && request->processed) ||
                request->delegated)
                continue;
            ...
            if (request->requestvb->type == PRIOT_NOSUCHOBJECT ||
                request->requestvb->type == PRIOT_ENDOFMIBVIEW) {
                /*
...
Table_sparseTableHandlerGet(void)
{
    return AgentHandler_createHandler(SPARSE_TABLE_HANDLER_NAME,
                                  Table_sparseTableHelperHandler);
}
                           ...
                           u_char type, u_char * result, size_t result_len)
{
    ...
    if (Table_buildOid(reginfo, reqinfo, table_info) !=
        ErrorCode_SUCCESS)
        return ErrorCode_GENERR;
...
int
Table_updateVariableListFromIndex(TableRequestInfo *tri)
{
    if (!tri)
        return ErrorCode_GENERR;
 ...
 */
int
Table_checkGetnextReply(RequestInfo *request,
                            oid * prefix,
                            size_t prefix_len,
                            ...
                            Types_VariableList ** outvar)
{
     ...
     */
    if ((!(*outvar) || Api_oidCompare(myname + prefix_len,
        ...
        if (Api_oidCompare(myname, myname_len,
                             ...
                             request->requestvb->nameLength) > 0) {
            /*
...
void
Table_registrationInfoFree(TableRegistrationInfo *tri)
{
    if (NULL == tri)
        return;
...
static void
Table_helperCleanup(AgentRequestInfo *reqinfo,
...
static void
Table_rowStashDataListFree(void *ptr) {

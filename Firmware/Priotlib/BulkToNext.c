...
{
    MibHandler* handler =
        AgentHandler_createHandler("bulk_to_next",
                               BulkToNext_helper);
                            ...
                            RequestInfo *requests)
{
    ...
    if(MODE_GETBULK == reqinfo->mode) {
        ...
        DEBUG_IF("bulk_to_next") {
            RequestInfo *req = requests;
            while(req) {
                DEBUG_MSGTL(("bulk_to_next", "Got request: "));
                DEBUG_MSGOID(("bulk_to_next", req->requestvb->name,
                             req->requestvb->nameLength));
                DEBUG_MSG(("bulk_to_next", "\n"));
                req = req->next;
            }
        }
...
{
    AgentHandler_registerHandlerByName("bulk_to_next",

...
TableIterator_createTable( FirstDataPointFT *firstDP,
                               NextDataPoint  *nextDP,
                               FirstDataPointFT *getidx,
                               Types_VariableList    *indexes)
{
    IteratorInfo *iinfo =
        TOOLS_MALLOC_TYPEDEF(IteratorInfo);
...
void
TableIterator_deleteTable( IteratorInfo *iinfo )
{
    if (!iinfo)
        return;
...
static void
TableIterator_deref(IteratorInfo *iinfo)
{
    if (--iinfo->refcnt == 0)
...
void TableIterator_handlerOwnsIteratorInfo(MibHandler *h)
{
    ...
    h->data_free  = (void(*)(void *))TableIterator_deref;
}
...
TableIterator_getTableIteratorHandler(IteratorInfo *iinfo)
{
    ...
    me =
        AgentHandler_createHandler(TABLE_ITERATOR_NAME,
 ...
 */
int
TableIterator_registerTableIterator(HandlerRegistration *reginfo,
                                IteratorInfo *iinfo)
{
 ...
 */
 void    *
TableIterator_extractIteratorContext(RequestInfo *request)
{
 ...
 *  created row into a request */
 void
TableIterator_insertIteratorContext(RequestInfo *request, void *data)
{
    RequestInfo       *req;
...
static TiCacheInfo *
TableIterator_remember(RequestInfo *request,
                          oid *oid_to_save,
...
int
TableIterator_helperHandler(MibHandler *handler,
                                      ...
                                      RequestInfo *requests)
{
                ...
                for(request = requests ; request; request = request->next) {
                    ...
                    case MODE_SET_RESERVE1:
                        ...
                        if (Api_oidCompare(myname, myname_len,
                             ...
                             * keep this
                             */
                            if (TableIterator_remember(request,
                                                          myname,
                                                          myname_len,
                                                          callback_data_context,
                    ...
                    case MODE_GET_STASH:
                        ...
                        if (reqtmp)
                            ldata =
                                DataList_getNode(reqtmp->parent_data,
                                                      TABLE_ITERATOR_NAME);
                        if (!ldata) {
                            AgentHandler_requestAddListData(reqtmp,
                                                          DataList_create
    ...
    if (reqinfo->mode == MODE_GET ||
        ...
        for(request = requests ; request; request = request->next) {
            ...
            ti_info = (TiCacheInfo*)
                AgentHandler_requestGetListData(request, TI_REQUEST_CACHE);
            table_info =
                Table_extractTableInfo(request);
    ...
    if (reqinfo->mode != MODE_GET_STASH) {
        DEBUG_MSGTL(("table_iterator", "call subhandler for mode: %s\n",
                    Enum_seFindLabelInSlist("agent_mode", oldmode)));
        ret =
...
void *
TableIterator_rowFirst( IteratorInfo *iinfo ) {
...
void *
TableIterator_rowGet( IteratorInfo *iinfo, void *row )
{
    Types_VariableList *vp1, *vp2;
    void *ctx1, *ctx2;
...
void *
TableIterator_rowNext( IteratorInfo *iinfo, void *row )
{
    Types_VariableList *vp1, *vp2;
    void *ctx1, *ctx2;
...
void *
TableIterator_rowGetByidx(  IteratorInfo *iinfo,
                                 Types_VariableList *indexes )
{
...
void *
TableIterator_rowNextByidx( IteratorInfo *iinfo,
                                 Types_VariableList *indexes )
{
    oid    dummy[] = {0,0};
    oid    instance[ASN01_MAX_OID_LEN];
...
void *
TableIterator_rowGetByoid(  IteratorInfo *iinfo,
...
void *
TableIterator_rowNextByoid( IteratorInfo *iinfo,
                                 oid *instance, size_t len )
{
    oid    dummy[] = {0,0};
    oid    this_inst[ ASN01_MAX_OID_LEN];
...
int
TableIterator_rowCount( IteratorInfo *iinfo )
{
    Types_VariableList *vp1, *vp2;
    void *ctx1, *ctx2;
    int   i=0;

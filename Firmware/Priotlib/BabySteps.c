...
static u_short babySteps_getModeMap[BABY_STEPS_PER_MODE_MAX] = {
                MODE_BSTEP_PRE_REQUEST, MODE_BSTEP_OBJECT_LOOKUP,
                BSTEP_USE_ORIGINAL, MODE_BSTEP_POST_REQUEST };

static u_short babySteps_setModeMap[PRIOT_MSG_INTERNAL_SET_MAX][BABY_STEPS_PER_MODE_MAX] = {
    /*R1*/
...
static int
BabySteps_helper(  MibHandler *handler,
                   HandlerRegistration *reginfo,
                   AgentRequestInfo *reqinfo,
                   RequestInfo *requests);
static int
BabySteps_accessMultiplexer(   MibHandler *handler,
                               HandlerRegistration *reginfo,
                               AgentRequestInfo *reqinfo,
                               RequestInfo *requests);
...
BabySteps_modesRef(BabyStepsModes *md)
{
    md->refcnt++;
    return md;
}
...
static void
BabySteps_modesDeref(BabyStepsModes *md)
{
    if (--md->refcnt == 0)
    free(md);
}
...
BabySteps_handlerGet(u_long modes)
{
    ...
    mh = AgentHandler_createHandler("baby_steps", BabySteps_helper);
    if(!mh)
        return NULL;
    ...
    md->refcnt = 1;
        mh->myvoid = md;
    mh->data_clone = (void *(*)(void *))BabySteps_modesRef;
    mh->data_free = (void (*)(void *))BabySteps_modesDeref;
        if (0 == modes)
                         ...
                         RequestInfo *requests)
{
    ...
    DEBUG_MSGTL(("baby_steps", "Got request, mode %s\n",
    ...
    case MODE_SET_FREE:
    case MODE_SET_UNDO:
        mode_map_ptr = babySteps_setModeMap[reqinfo->mode];
        break;
    ...
    default:
        ...
        mode_map_ptr = babySteps_getModeMap;
    }
    ...
    for(i = 0; i < BABY_STEPS_PER_MODE_MAX; ++i ) {
        ...
        DEBUG_MSGTL(("baby_steps", " baby step mode %s\n",
                    Enum_seFindLabelInSlist("babystep_mode",mode_map_ptr[i])));
         ...
         */
        if(rc) {
            DEBUG_MSGTL(("baby_steps", "   ERROR:handler error\n"));
            break;
        }
        ...
        if (MODE_IS_GET(save_mode)
            ...
            if(rc) {
                DEBUG_MSGTL(("baby_steps", "   ERROR:request error\n"));
                break;
            }
        }
    }
...
{
    AgentHandler_registerHandlerByName("baby_steps",
...
BabySteps_accessMultiplexerGet(BabyStepsAccessMethods *am)
{
    ...
    mh = AgentHandler_createHandler("baby_steps_mux",
                                BabySteps_accessMultiplexer);
    if(!mh)
        return NULL;
                               ...
                               RequestInfo *requests)
{
    ...
    DEBUG_MSGT(("baby_steps_mux", "mode %s\n",
               Enum_seFindLabelInSlist("babystep_mode",reqinfo->mode)));

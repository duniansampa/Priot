...
static LookupCacheContext *agentRegistry_thecontextcache = NULL;
...
AgentRegistry_getContextLookupCache(const char *context) {
    LookupCacheContext *ptr;
    if (!context)
        context = "";

    for(ptr = agentRegistry_thecontextcache; ptr; ptr = ptr->next) {
    ...
    if (!ptr) {
        if (AgentRegistry_subtreeFindFirst(context)) {
            ...
            ptr->next = agentRegistry_thecontextcache;
            ptr->context = strdup(context);
            agentRegistry_thecontextcache = ptr;
        } else {
            return NULL;
        }
    }
 ...
 */
static inline void
AgentRegistry_lookupCacheAdd(const char *context,
                 Subtree *next, Subtree *previous) {
    LookupCacheContext *cptr;

    if ((cptr = AgentRegistry_getContextLookupCache(context)) == NULL)
        return;
 ...
 */
static inline void
AgentRegistry_lookupCacheReplace(LookupCache *ptr,
                     Subtree *next, Subtree *previous) {
...
AgentRegistry_lookupCacheFind(const char *context, const oid *name, size_t name_len,
                  int *retcmp) {
    LookupCacheContext *cptr;
 ...
 */
static inline void
AgentRegistry_invalidateLookupCache(const char *context) {
    LookupCacheContext *cptr;
    if ((cptr = AgentRegistry_getContextLookupCache(context)) != NULL) {
...
void
AgentRegistry_clearLookupCache(void) {

    LookupCacheContext *ptr = NULL, *next = NULL;

    ptr = agentRegistry_thecontextcache;
    while (ptr) {
    next = ptr->next;
    ...
    agentRegistry_thecontextcache = NULL; /* !!! */
}
...
void
AgentRegistry_removeSubtree(Subtree *tree)
{
    SubtreeContextCache *ptr;
...
static void AgentRegistry_registerMibDetachNode(Subtree *s);
 ...
 */
void
AgentRegistry_subtreeFree(Subtree *a)
{
  if (a != NULL) {
...
AgentRegistry_subtreeDeepcopy(Subtree *a)
{
  ...
  if (b != NULL) {
    ...
    if (a->variables != NULL) {
      b->variables = (struct Variable_s *)malloc(a->variables_len *
                           a->variables_width);
      if (b->variables != NULL) {
 ...
 */
static inline void
AgentRegistry_subtreeChangeNext(Subtree *ptr, Subtree *thenext)
{
    ptr->next = thenext;
    if (thenext)
 ...
 */
static inline void
AgentRegistry_subtreeChangePrev(Subtree *ptr, Subtree *theprev)
{
    ptr->prev = theprev;
    if (theprev)
 ...
 */
int
AgentRegistry_subtreeCompare(const Subtree *ap, const Subtree *bp)
{
 ...
 */
void
AgentRegistry_subtreeJoin(Subtree *root)
{
    Subtree *s, *tmp, *c, *d;
    ...
    while (root != NULL) {
        ...
        s->end_a      = NULL;
            ...
            for (c = root; c != NULL; c = c->children) {
                AgentRegistry_subtreeChangeNext(c, s->next);
            }
            for (c = s; c != NULL; c = c->children) {
                AgentRegistry_subtreeChangePrev(c, root);
            }
...
AgentRegistry_subtreeSplit(Subtree *current, oid name[], int name_len)
{
    ...
    rc = Api_oidCompare(vp->name, vp->namelen,
        ...
        if (rc2 < 0) {
            ...
            new_sub->variables = (struct Variable_s *)(cp +
    ...
    if (current->variables_len > 0 &&
        ASN01_IS_DELEGATED((u_char) current->variables[0].type)) {
    ...
    for (ptr = current; ptr != NULL; ptr = ptr->children) {
        AgentRegistry_subtreeChangeNext(ptr, new_sub);
    }
    for (ptr = new_sub; ptr != NULL; ptr = ptr->children) {
        AgentRegistry_subtreeChangePrev(ptr, current);
    }
    for (ptr = new_sub->next; ptr != NULL; ptr=ptr->children) {
        AgentRegistry_subtreeChangePrev(ptr, new_sub);
    }
 ...
 */
int
AgentRegistry_subtreeLoad(Subtree *new_sub, const char *context_name)
{
    Subtree *tree1, *tree2;
    Subtree *prev, *next;
    ...
    if (tree2 && Api_oidCompare(new_sub->end_a, new_sub->end_len,
    ...
    if (tree2) {
            AgentRegistry_subtreeChangePrev(new_sub, tree2->prev);
            AgentRegistry_subtreeChangePrev(tree2, new_sub);
    } else {
            AgentRegistry_subtreeChangePrev(new_sub,
                                        AgentRegistry_subtreeFindPrev(new_sub->start_a,
                                                                  new_sub->start_len, NULL, context_name));

        if (new_sub->prev) {
                AgentRegistry_subtreeChangeNext(new_sub->prev, new_sub);
        } else {
        AgentRegistry_subtreeReplaceFirst(new_sub, context_name);
        }

            AgentRegistry_subtreeChangeNext(new_sub, tree2);
    ...
    case  0:
        ...
        new_sub->children = next;
                AgentRegistry_subtreeChangePrev(new_sub, prev->prev);
                AgentRegistry_subtreeChangeNext(new_sub, prev->next);
        } else {
        new_sub->children = next;
                AgentRegistry_subtreeChangePrev(new_sub, next->prev);
                AgentRegistry_subtreeChangeNext(new_sub, next->next);

        for (next = new_sub->next; next != NULL;next = next->children){
                    AgentRegistry_subtreeChangePrev(next, new_sub);
        }

        for (prev = new_sub->prev; prev != NULL;prev = prev->children){
                    AgentRegistry_subtreeChangeNext(prev, new_sub);
        }
        }
        break;
    ...
    case  1:
        /*  New subtree contains the existing one.  */
            {
                Subtree *new2 =
                    AgentRegistry_subtreeSplit(new_sub, tree1->end_a,tree1->end_len);
 ...
 */
void
AgentRegistry_clearSubtree (Subtree *sub) {
              ...
              const char *context_name)
{
    ...
    /* look through everything */
        if (agentRegistry_lookupCacheSize) {
            lookup_cache = AgentRegistry_lookupCacheFind(context_name, name, len, &cmp);
            if (lookup_cache) {
...
#define WTEST_OPTIMIZATION 1
    ...
    for (; myptr != NULL; previous = myptr, myptr = myptr->next) {
        ...
        if (!(ll_off && myptr->oid_off && myptr->oid_off > ll_off) &&
            Api_oidCompareLl(name, len,
                                   myptr->start_a, myptr->start_len,
                                   &ll_off) < 0) {

            if (agentRegistry_lookupCacheSize && previous && cmp) {
                if (lookup_cache) {
                    AgentRegistry_lookupCacheReplace(lookup_cache, myptr, previous);
                } else {
                    AgentRegistry_lookupCacheAdd(context_name, myptr, previous);
                }
            }
            return previous;
        }
              ...
              Subtree *subtree, const char *context_name)
{
    ...
    if (myptr != NULL) {
        ...
        return myptr;
    } else if (subtree != NULL && Api_oidCompare(name, len,
             ...
             const char *context_name)
{
    ...
    if (myptr && myptr->end_a &&
        Api_oidCompare(name, len, myptr->end_a, myptr->end_len)<0) {
        return myptr;
    }
 ...
 */
int
AgentRegistry_registerMib2(const char *moduleName,
                     ...
                     int perform_callback)
{
    ...
    for (i = mibloc[range_subid - 1] + 1; i <= (int)range_ubound; i++) {
        ...
        if (sub2 == NULL) {
                ...
                AgentRegistry_invalidateLookupCache(context);
                return MIB_REGISTRATION_FAILED;
            }
    ...
    if (DefaultStore_getBoolean(DsStorage_APPLICATION_ID,
        ...
        if (agent_mainSession == NULL) {
            AgentRegistry_registerMibDetachNode(subtree);
    }
    }
 ...
 */
static void
AgentRegistry_registerMibReattachNode(Subtree *s)
{
...
AgentRegistry_registerMibReattach(void)
{
    ...
    for (ptr = agentRegistry_contextSubtrees; ptr; ptr = ptr->next) {
        for (s = ptr->first_subtree; s != NULL; s = s->next) {
            AgentRegistry_registerMibReattachNode(s);
            for (t = s->children; t != NULL; t = t->children) {
                AgentRegistry_registerMibReattachNode(t);
            }
        }
    }
}
 ...
 */
static void
AgentRegistry_registerMibDetachNode(Subtree *s)
{
    if (s != NULL) {
...
AgentRegistry_registerMibDetach(void)
{
    ...
    for (ptr = agentRegistry_contextSubtrees; ptr; ptr = ptr->next) {
        for (s = ptr->first_subtree; s != NULL; s = s->next) {
            AgentRegistry_registerMibDetachNode(s);
            for (t = s->children; t != NULL; t = t->children) {
                AgentRegistry_registerMibDetachNode(t);
            }
        }
    }
}
 ...
 */
int
AgentRegistry_registerMibContext(const char *moduleName,
 ...
 */
int
AgentRegistry_registerMibRange(const char *moduleName,
                   const struct Variable_s *var,
                   size_t varsize,
                   size_t numvars,
                   const oid * mibloc,
 ...
 */
int
AgentRegistry_registerMibPriority(const char *moduleName,
                      const struct Variable_s *var,
                      size_t varsize,
                      size_t numvars,
                      const oid * mibloc,
 ...
 */
int
AgentRegistry_registerMib(const char *moduleName,
             const struct Variable_s *var,
             size_t varsize,
             size_t numvars,
             const oid * mibloc,
 ...
 */
void
AgentRegistry_subtreeUnload(Subtree *sub, Subtree *prev, const char *context)
{
    Subtree *ptr;
    ...
    if (sub->children == NULL) {        /* just remove this node completely */
        for (ptr = sub->prev; ptr; ptr = ptr->children) {
            AgentRegistry_subtreeChangeNext(ptr, sub->next);
        }
        for (ptr = sub->next; ptr; ptr = ptr->children) {
            AgentRegistry_subtreeChangePrev(ptr, sub->prev);
        }
    ...
    if (sub->prev == NULL) {
        ...
        for (ptr = sub->prev; ptr; ptr = ptr->children)
            AgentRegistry_subtreeChangeNext(ptr, sub->children);
        for (ptr = sub->next; ptr; ptr = ptr->children)
            AgentRegistry_subtreeChangePrev(ptr, sub->children);
 ...
 */
int
AgentRegistry_unregisterMibContext(oid * name, size_t len, int priority,
                       ...
                       const char *context)
{
    ...
    while(unregistering){
        ...
        for (child = list, prev = NULL; child != NULL;
            ...
            if (Api_oidEquals(child->name_a, child->namelen, name, len) == 0 &&
...
int
AgentRegistry_unregisterMibTableRow(oid * name, size_t len, int priority,
 ...
 */
int
AgentRegistry_unregisterMibRange(oid * name, size_t len, int priority,
                     int range_subid, oid range_ubound)
{
 ...
 */
int
AgentRegistry_unregisterMibPriority(oid * name, size_t len, int priority)
{
 ...
 */
int
AgentRegistry_unregisterMib(oid * name, size_t len)
{
 ...
 */
void
AgentRegistry_unregisterMibsBySession(Types_Session * ss)
{
    Subtree *list, *list2;
    ...
    for (contextptr = AgentRegistry_getTopContextCache(); contextptr != NULL;
        ...
        for (list = contextptr->first_subtree; list != NULL; list = list2) {
            ...
            for (child = list, prev = NULL; child != NULL; child = next_child){
                ...
                if (((!ss || ss->flags & API_FLAGS_SUBSESSION) &&
             child->session == ss) ||
                    (!(!ss || ss->flags & API_FLAGS_SUBSESSION) && child->session &&
 ...
 */
int
AgentRegistry_inAView(oid *name, size_t *namelen, Types_Pdu *pdu, int type)
{
 ...
 */
int
AgentRegistry_acmCheckSubtree(Types_Pdu *pdu, oid *name, size_t namelen)
{                               /* IN - pdu being checked */
    struct ViewParameters_s view_parms;
    view_parms.pdu = pdu;
    view_parms.name = name;
...
int
AgentRegistry_removeTreeEntry (oid *name, size_t len) {
...
void
AgentRegistry_dumpRegistry(void)
{
    struct Variable_s *vp = NULL;
                    ...
                    if (myptr2->label_a && myptr2->label_a[0]) {
                        if (strcmp(myptr2->label_a, "old_api") == 0) {
                            struct Variable_s *vp =
                                (struct Variable_s*)myptr2->reginfo->handler->myvoid;
...
void
AgentRegistry_agentSIGCHLDHandler(int sig)
{
 ...
 */
int
AgentRegistry_registerSignal(int sig, void (*func) (int))
{
 ...
 */
int
AgentRegistry_unregisterSignal(int sig)
{
    signal(sig, SIG_DFL);

...
static void
Trap_freeTrapSession(struct TrapSink *sp)
{
...
int
Trap_removeTrapSession(Types_Session * ss)
{
...
Trap_convertV2pduToV1( Types_Pdu* template_v2pdu )
{
     ...
     */
    if (!vblist ||
        Api_oidCompare(vblist->name,  vblist->nameLength,
                          ...
                          const char * context, int flags)
{
    ...
    if ( trap == -1 ) {
         ...
         */
        if (!trap_vb ||
            Api_oidCompare(trap_vb->name, trap_vb->nameLength,
                             trap_priotTrapOid,  trap_priotTrapOidLen)) {
            Logger_log(LOGGER_PRIORITY_WARNING,
        ...
        if (!Api_oidCompare(vblist->val.objid, ASN01_OID_LENGTH(trap_trapPrefix),
            ...
            var = Client_findVarbindInList( template_v2pdu->variables,
                                        ...
                                        trap_priottrapenterpriseOidLen);
            if (!var &&
                !Api_varlistAddVariable( &(template_v2pdu->variables),
                     trap_priottrapenterpriseOid, trap_priottrapenterpriseOidLen,
                     ASN01_OBJECT_ID,
     ...
     */
    if (template_v1pdu) {
      if (template_v1pdu->trapType == PRIOT_TRAP_AUTHFAIL &&
        trap_enableauthentraps == SNMP_AUTHENTICATED_TRAPS_DISABLED) {
        Api_freePdu(template_v1pdu);
        Api_freePdu(template_v2pdu);
        return 0;
      }
     ...
     */
       ...
       if (v1trapaddress == NULL || res < 0) {
           ...
           *pdu_in_addr_t = System_getMyAddr();
       }
...
*/
int
Trap_handleInformResponse(int op, Types_Session * session,
...
Trap_sendTrapToSess(Types_Session * sess, Types_Pdu *template_pdu)
{
    ...
    if ( template_pdu->command == PRIOT_MSG_INFORM
        ...
        result =
            Api_asyncSend(sess, pdu, &Trap_handleInformResponse, NULL);

    } else {
        if ((sess->version == PRIOT_VERSION_3) &&
    ...
    if (result == 0) {
        Api_sessPerror("snmpd: send_trap", sess);
        Api_freePdu(pdu);
    } else {
...
void
Trap_sendEasyTrap(int trap, int specific)
{
...
void
Trap_priotdParseConfigAuthtrap(const char *token, char *cptr)
{
    int             i;
...
static int      trap_traptype;

static void
Trap_trapOptProc(int argc, char *const *argv, int opt)
{
    switch (opt) {
    case 'C':
        while (*optarg) {
            switch (*optarg++) {
            case 'i':
                trap_traptype = PRIOT_MSG_INFORM;
                break;
            default:
...
void
Trap_priotdParseConfigTrapsess(const char *word, char *cptr)
{
    ...
    ParseArgs_parseArgs(argn, argv, &session, "C:", Trap_trapOptProc,
                        PARSEARGS_NOLOGGING |
                        PARSEARGS_NOZERO);

    transport = Transport_openClient("snmptrap", session.peername);
    if (transport == NULL) {
        ReadConfig_configPerror("snmpd: failed to parse this line.");
        return;
    }
    ...
    if (!ss) {
        ...
        Api_sessPerror("snmpd: Trap_priotdParseConfigTrapsess()", &session);
        return;
    }
    ...
    Trap_addTrapSession(ss, trap_traptype, (trap_traptype == PRIOT_MSG_INFORM), ss->version);
}

...
static Cache*   cacheHandler_head = NULL;
static int      cacheHandler_outstandingValid = 0;
static int      CacheHandler_load( Cache *cache );
...
static void
CacheHandler_free2( Cache *cache );
...
CacheHandler_findByOid(const oid * rootoid, int rootoid_len)
{
    ...
    for (cache = cacheHandler_head; cache; cache = cache->next) {
                     ...
                     const oid * rootoid, int rootoid_len)
{
    ...
    if (rootoid) {
        ...
        cache->next = cacheHandler_head;
        if (cacheHandler_head)
            cacheHandler_head->prev = cache;
        cacheHandler_head = cache;
    }
...
static void
CacheHandler_deref(Cache *cache)
{
    if (--cache->refcnt == 0) {
...
CacheHandler_free(Cache *cache)
{
    ...
    for (pos = cacheHandler_head; pos; pos = pos->next) {
        if (pos == cache) {
            size_t          out_len = 0;
            size_t          buf_len = 0;
            char           *buf = NULL;
...
CacheHandler_remove(Cache *cache)
{
    ...
    if (!cache || !cacheHandler_head)
        return -1;

    if (cache == cacheHandler_head) {
        cacheHandler_head = cacheHandler_head->next;
        if (cacheHandler_head)
            cacheHandler_head->prev = NULL;
        return 0;
    }

    prev = cacheHandler_head;
    cur = cacheHandler_head->next;
...
static void
CacheHandler_timerReload(unsigned int regNo, void *clientargs)
{
...
CacheHandler_timerStart(Cache *cache)
{
    ...
    cache->timer_id = Alarm_register(cache->timeout, ALARM_SA_REPEAT,
                                          CacheHandler_timerReload, cache);
    if(0 == cache->timer_id) {
...
CacheHandler_handlerGet(Cache* cache)
{
    ...
    ret = AgentHandler_createHandler("cache_handler",
...
void CacheHandler_handlerOwnsCache(MibHandler *handler)
{
    ...
    handler->data_clone = (void *(*)(void *))CacheHandler_ref;
    handler->data_free = (void(*)(void*))CacheHandler_deref;
}
...
static char *
CacheHandler_buildCacheName(const char *name)
{
...
CacheHandler_reqinfoInsert(Cache* cache,
                             ...
                             const char *name)
{
    char *cache_name = CacheHandler_buildCacheName(name);
...
CacheHandler_checkAndReload(Cache * cache)
{
    ...
    if (!cache->valid || CacheHandler_checkExpired(cache))
        return CacheHandler_load( cache );
    else {
...
CacheHandler_load( Cache *cache )
{
     ...
     */
    if ((!cacheHandler_outstandingValid) &&
        ...
        cacheHandler_outstandingValid = 1;
    }
    Tools_setMonotonicMarker(&cache->timestampM);
...
CacheHandler_releaseCachedResources(unsigned int regNo, void *clientargs)
{
    ...
    cacheHandler_outstandingValid = 0;

...
{
    return AgentHandler_createHandler("old_api", OldApi_helper);
}
            ...
            if (access) {
                ...
                if (access == NULL) {
                    if (Api_oidEquals(requests->requestvb->name,
                                         ...
                                         save, savelen) != 0) {
                        DEBUG_MSGTL(("old_api", "evil_client: %s\n",
                                    reginfo->handlerName));
 ...
 * don't use this!
 */
static AgentSession *oldApi_currentAgentSession = NULL;

AgentSession *
OldApi_getCurrentAgentSession(void)
{
    return oldApi_currentAgentSession;
}
...
OldApi_setCurrentAgentSession(AgentSession *asp)
{
    oldApi_currentAgentSession = asp;
}

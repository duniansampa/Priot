...
#define MODE_IS_GET(x)        ((x >= 128) && (x != -1)  && (x != PRIOT_MSG_SET))


/* #define MODE_IS_GET(x)        ((x == PRIOT_MSG_GET) || (x == PRIOT_MSG_GETNEXT) || (x == PRIOT_MSG_GETBULK) || (x == PRIOT_MSG_INTERNAL_GET_STASH)) */
...
#define MODE_IS_SET(x)         ((x < 128) || (x == -1) || (x == PRIOT_MSG_SET))
...
void Agent_removeAndFreeAgentPriotSession(AgentSession
                                               *asp);
...
int    Agent_setAllRequestsError(AgentRequestInfo *reqinfo, RequestInfo *requests, int error_value);
...
int    Agent_setRequestError(AgentRequestInfo *reqinfo, RequestInfo *request, int error_value);

/** deprecated, use netsnmp_request_set_error instead */
int    Agent_setModeRequestError(int mode, RequestInfo *request, int error_value);
...
int             Agent_checkPacket( Types_Session *,
                                   Transport_Transport *,
                                   void *,
                                   int );

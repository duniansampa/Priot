...
void Options::process(int argc, char *argv[]){
    ...
    while ((arg = getopt(argc, argv, var_options)) != EOF) {
        ...
        case 'A':
            DefaultStore_setBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::APPEND_LOGFILES, 1);
            break;

        case 'c':
            if (optarg != NULL) {
                DefaultStore_setString(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_STRING::OPTIONALCONFIG, optarg );
            } else {
                this->usage(argv[0]);
            }
            break;
        ...
        case 'C':
            DefaultStore_setBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::DONT_READ_CONFIGS, 1);
            break;

        case 'd':
            DefaultStore_setBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::DUMP_PACKET, ++snmp_dump_packet);
            break;
...
//            m_logSet = true;
        ...
        case 'n':
            if (optarg != NULL) {
                m_appName = optarg;
                DefaultStore_setString(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_STRING::APPTYPE, (char*)m_appName.c_str());
            } else {
                this->usage(argv[0]);
            }
            break;
        ...
        case 'q':
            DefaultStore_setBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::QUICK_PRINT, 1);
            break;

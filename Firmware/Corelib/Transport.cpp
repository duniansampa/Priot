...
static Transport_Tdomain * transport_domainList = NULL;
...
void Transport_initTransport(void)
{
    DefaultStore_registerConfig(ASN01_BOOLEAN,
                               ...
                               DEFAULTSTORE_LIB_BOOLEAN::DONT_LOAD_HOST_FILES);
}
                       ...
                       void **opaque, int *olength)
{
    ...
    dumpPacket = DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                                        DEFAULTSTORE_LIB_BOOLEAN::DUMP_PACKET);
    debugLength = (API_ERR_SUCCESS ==
...
int
Transport_tdomainSupport(const _oid * in_oid,
                        size_t in_len,
                        const _oid ** out_oid, size_t * out_len)
{
    Transport_Tdomain *d = NULL;

    for (d = transport_domainList; d != NULL; d = d->next) {
...
{
    DEBUG_MSGTL(("tdomain", "netsnmp_tdomain_init() called\n"));
...
/* include the configure generated list of constructor calls */
    ...
    Transport_tdomainDump();
}

void
Transport_clearTdomainList(void)
{
    Transport_Tdomain *list = transport_domainList, *next = NULL;
    ...
    transport_domainList = NULL;
}


static void
Transport_tdomainDump(void)
{
    Transport_Tdomain *d;
    int i = 0;
...
{
    Transport_Tdomain **prevNext = &transport_domainList, *d;
...
Transport_findTdomain(const char* spec)
{
    Transport_Tdomain *d;
    for (d = transport_domainList; d != NULL; d = d->next) {
        int i;
...
static int
Transport_isFqdn(const char *thename)
{
    if (!thename)
        return 0;
    while(*thename) {
                               ...
                               const char *default_target)
{
    ...
    if (!DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
        ...
        if (!have_added_handler) {
            ...
            DefaultStore_registerConfig(ASN01_OCTET_STR,
                                       "snmp", "transport",
                                       DEFAULTSTORE_STORAGE::LIBRARY_ID,
                                       DEFAULTSTORE_LIB_STRING::HOSTNAME);
        }
        ...
        prev_hostname = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                                              DEFAULTSTORE_LIB_STRING::HOSTNAME);
        if (prev_hostname)
            prev_hostname = strdup(prev_hostname);

        /* read in the hosts/STRING.conf files */
        config_handlers = ReadConfig_getHandlers("snmp");
        ...
        if (NULL !=
            (newhost = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                                             DEFAULTSTORE_LIB_STRING::HOSTNAME))) {
    ...
    if (str != NULL) {
        ...
        if ((cp = strchr(str, ':')) != NULL) {
            ...
            match = Transport_findTdomain(mystring);
            free(mystring);
        }
    }
    ...
    if (match == NULL) {
        ...
        if (addr && *addr == '/') {
            ...
            if (spec == NULL) {
                ...
                match = Transport_findTdomain("udp");
            } else {
    ...
    for(;;) {
        ...
        addr = str;
        if (spec && *spec)
            match = Transport_findTdomain(*spec++);
        else
            break;
    }
    if (!any_found)
...
Transport_tdomainTransport(const char *str, int local,
              const char *default_domain)
{
    return Transport_tdomainTransportFull("snmp", str, local, default_domain,
                      NULL);
}

...
in_addr_t System_getMyAddr(void)
{
    ...
    for (i = 8;; i += 8) {
        buf = (_s8 *) calloc(i, sizeof(struct ifreq));
        if (buf == NULL) {
            close(sd);
            return 0;
        }
        ...
        if (ioctl(sd, SIOCGIFCONF, (_s8 *) &ifc) < 0) {
    ...
    for (ifrp = ifc.ifc_req; (_s8 *)ifrp < (_s8 *)ifc.ifc_req + ifc.ifc_len; ifrp++) {
        ...
        if (ioctl(sd, SIOCGIFFLAGS, (_s8 *) ifrp) < 0) {
            continue;
        }
        if ((ifrp->ifr_flags & IFF_UP) && (ifrp->ifr_flags & IFF_RUNNING)
            ...
            /*
              ...
              */
            if (ioctl(sd, SIOCGIFADDR, (_s8 *) ifrp) < 0) {
                continue;
            }
            addr =
...
_i32 System_getaddrinfo(const _s8 *name, const _s8 *service,
...
_i32 System_calculateTimeDiff(const struct timeval *now, const struct timeval *then)
{
    ...
    return (_i32)(diff.tv_sec * 100 + diff.tv_usec / 10000);
}
...
_i32 System_mkdirhier(const _s8 *pathname, mode_t mode, int skiplast)
{
    struct stat     sbuf;
    _s8           *ourcopy = strdup(pathname);
    _s8           *entry;
    _s8           *buf = NULL;
    _s8           *st = NULL;
    _i32          res;
    ...
    buf = (_s8 *)malloc(strlen(pathname) + 2);
    if (!buf)
        goto out;

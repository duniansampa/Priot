...
#define SCAPI_QUITFUN(e, l)				            \
        if (e != API_ERR_SUCCESS) {			        \
                rval = API_ERR_SC_GENERAL_FAILURE;	\
                goto l ;				            \
        }
 ...
 *
 * Returns either the length or API_ERR_GENERR for an unknown hashing type.
 */
int Scapi_getProperLength(const _oid * hashtype, u_int hashtype_len)
{
    DEBUG_TRACE;
    /*
...
int Scapi_getProperPrivLength(const _oid * privtype, u_int privtype_len)
{
    int properlength = 0;
 ...
 */
int Scapi_generateKeyedHash(const _oid * authtype, size_t authtypelen,
                       ...
                       u_char * MAC, size_t * maclen)
{
    ...
    if (!authtype || !key || !message || !MAC || !maclen
        ...
        SCAPI_QUITFUN(API_ERR_GENERR, goto_generateKeyedHashQuit);
    }
 ...
 */
int Scapi_hash(const _oid * hashtype, size_t hashtypelen, const u_char * buf,
...
/** initialize the pointer */
    ...
    if (!EVP_DigestInit(cptr, hashfn)) {
        ...
        return API_ERR_SC_NOT_CONFIGURED;
    }
 ...
 */
int Scapi_checkKeyedHash(const _oid * authtype, size_t authtypelen,
                    const u_char * key, u_int keylen,
                    const u_char * message, u_int msglen,
                    const u_char * MAC, u_int maclen)
{
    ...
    if (!authtype || !key || !message || !MAC
        || (keylen <= 0) || (msglen <= 0) || (maclen <= 0)
        || (authtypelen != TOOLS_USM_LENGTH_OID_TRANSFORM)) {
        SCAPI_QUITFUN(API_ERR_GENERR, goto_checkKeyedHashQuit);
    }
 ...
 */
int
Scapi_encrypt(const _oid * privtype, size_t privtypelen,
           ...
           u_char * ciphertext, size_t * ctlen)
{
    ...
    if (!privtype || !key || !iv || !plaintext || !ciphertext || !ctlen
        ...
        SCAPI_QUITFUN(API_ERR_GENERR, sc_encrypt_quit);
    } else if (ptlen > *ctlen) {
        SCAPI_QUITFUN(API_ERR_GENERR, sc_encrypt_quit);
    }
    ...
    if (TOOLS_ISTRANSFORM(privtype, dESPriv)) {
        ...
        if (pad == pad_size)
            pad = 0;
        if (ptlen + pad > *ctlen) {
            SCAPI_QUITFUN(API_ERR_GENERR, sc_encrypt_quit);    /* not enough space */
        }
           ...
           u_char * plaintext, size_t * ptlen)
{
    ...
    if (!privtype || !key || !iv || !plaintext || !ciphertext || !ptlen
        ...
        || (privtypelen != TOOLS_USM_LENGTH_OID_TRANSFORM)) {
        SCAPI_QUITFUN(API_ERR_GENERR, sc_decrypt_quit);
    }

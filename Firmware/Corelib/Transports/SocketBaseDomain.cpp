...
static int SocketBaseDomain_bufferSizeGet(int optname, int local, const char **buftype)
{
    ...
    if (optname == SO_SNDBUF) {
        if (local) {
            size = DefaultStore_getInt(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::SERVERSENDBUF);

        } else {
            size = DefaultStore_getInt(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::CLIENTSENDBUF);
        }
    } else if (optname == SO_RCVBUF) {
        if (local) {
            size = DefaultStore_getInt(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::SERVERRECVBUF);
        } else {
            size = DefaultStore_getInt(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::CLIENTRECVBUF);
        }
    } else {
        size = 0;
    }

...
static struct Vacm_ViewEntry_s *  vacm_viewList = NULL,   *vacm_viewScanPtr = NULL;
static struct Vacm_AccessEntry_s *vacm_accessList = NULL, *vacm_accessScanPtr = NULL;
static struct Vacm_GroupEntry_s * vacm_groupList = NULL,  *vacm_groupScanPtr = NULL;
...
Vacm_save(const char *token, const char *type)
{
    ...
    for (vptr = vacm_viewList; vptr != NULL; vptr = vptr->next) {
        if (vptr->viewStorageType == TC_ST_NONVOLATILE)
            Vacm_saveView(vptr, token, type);
    }

    for (aptr = vacm_accessList; aptr != NULL; aptr = aptr->next) {
    ...
    for (gptr = vacm_groupList; gptr != NULL; gptr = gptr->next) {
...
Vacm_parseConfigView(const char *token, const char *line)
{
    ...
    _oid            *viewSubtree = (_oid *) & view.viewSubtree;
    u_char         *viewMask;
    size_t          len;
    ...
    line =
        ReadConfig_readObjidConst(line, (_oid **) & viewSubtree,
                               &view.viewSubtreeLen);
...
Vacm_viewGet(struct Vacm_ViewEntry_s *head, const char *viewName,
                  _oid * viewSubtree, size_t viewSubtreeLen, int mode)
{
...
Vacm_viewSubtreeCheck(struct Vacm_ViewEntry_s *head, const char *viewName,
                           _oid * viewSubtree, size_t viewSubtreeLen)
{
...
{
    vacm_viewScanPtr = vacm_viewList;
}

struct Vacm_ViewEntry_s *
Vacm_scanViewNext(void)
{
    struct Vacm_ViewEntry_s *returnval = vacm_viewScanPtr;
    if (vacm_viewScanPtr)
        vacm_viewScanPtr = vacm_viewScanPtr->next;
    return returnval;
}
                     ...
                     _oid * viewSubtree, size_t viewSubtreeLen)
{
    ...
    memcpy(vp->viewSubtree + 1, viewSubtree, viewSubtreeLen * sizeof(_oid));
...
Vacm_getGroupEntry(int securityModel, const char *securityName)
{
    ...
    for (vp = vacm_groupList; vp; vp = vp->next) {
...
{
    vacm_groupScanPtr = vacm_groupList;
}

struct Vacm_GroupEntry_s *
Vacm_scanGroupNext(void)
{
    struct Vacm_GroupEntry_s *returnval = vacm_groupScanPtr;
    if (vacm_groupScanPtr)
        vacm_groupScanPtr = vacm_groupScanPtr->next;
    return returnval;
}
...
Vacm_destroyGroupEntry(int securityModel, const char *securityName)
{
    ...
    if (vacm_groupList && vacm_groupList->securityModel == securityModel
        && !strcmp(vacm_groupList->securityName + 1, securityName)) {
        vp = vacm_groupList;
        vacm_groupList = vacm_groupList->next;
    } else {
        for (vp = vacm_groupList; vp; vp = vp->next) {
...
Vacm_destroyAllGroupEntries(void)
{
    ...
    while ((gp = vacm_groupList)) {
        vacm_groupList = gp->next;
        if (gp->reserved)
            free(gp->reserved);
        free(gp);
    }
                    ...
                    int securityModel, int securityLevel)
{
    ...
    for (vp = vacm_accessList; vp; vp = vp->next) {
        if ((securityModel == vp->securityModel
             || vp->securityModel == PRIOT_SEC_MODEL_ANY)
...
{
    vacm_accessScanPtr = vacm_accessList;
}

struct Vacm_AccessEntry_s *
Vacm_scanAccessNext(void)
{
    struct Vacm_AccessEntry_s *returnval = vacm_accessScanPtr;
    if (vacm_accessScanPtr)
        vacm_accessScanPtr = vacm_accessScanPtr->next;
    return returnval;
}
                        ...
                        int securityModel, int securityLevel)
{
    ...
    if (vacm_accessList && vacm_accessList->securityModel == securityModel
        && vacm_accessList->securityLevel == securityLevel
        && !strcmp(vacm_accessList->groupName + 1, groupName)
        && !strcmp(vacm_accessList->contextPrefix + 1, contextPrefix)) {
        vp = vacm_accessList;
        vacm_accessList = vacm_accessList->next;
    } else {
        for (vp = vacm_accessList; vp; vp = vp->next) {
            if (vp->securityModel == securityModel
                && vp->securityLevel == securityLevel
...
Vacm_destroyAllAccessEntries(void)
{
    ...
    while ((ap = vacm_accessList)) {
        vacm_accessList = ap->next;
        if (ap->reserved)
            free(ap->reserved);
        free(ap);
    }
...
{
    if (vacm_accessList == NULL && vacm_groupList == NULL) {
        return 0;
    }
    return 1;
}
...
Vacm_getViewEntry(const char *viewName,
                  _oid * viewSubtree, size_t viewSubtreeLen, int mode)
{
    return Vacm_viewGet( vacm_viewList, viewName, viewSubtree, viewSubtreeLen,
                             mode);
}
...
Vacm_checkSubtree(const char *viewName,
                  _oid * viewSubtree, size_t viewSubtreeLen)
{
    return Vacm_viewSubtreeCheck( vacm_viewList, viewName, viewSubtree,
                                       viewSubtreeLen);
}
...
Vacm_createViewEntry(const char *viewName,
                     _oid * viewSubtree, size_t viewSubtreeLen)
{
    return Vacm_viewCreate( &vacm_viewList, viewName, viewSubtree,
                                viewSubtreeLen);
}
...
Vacm_destroyViewEntry(const char *viewName,
                      _oid * viewSubtree, size_t viewSubtreeLen)
{
    Vacm_viewDestroy( &vacm_viewList, viewName, viewSubtree, viewSubtreeLen);
}

void
Vacm_destroyAllViewEntries(void)
{
    Vacm_viewClear( &vacm_viewList );
}

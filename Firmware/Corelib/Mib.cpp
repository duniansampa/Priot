...
static char * Mib_uptimeString(u_long, char *, size_t);

static struct Parse_Tree_s * Mib_getReallocSymbol(const _oid * objid, size_t objidlen,
...
static void Mib_oidFinishPrinting( const _oid * objid, size_t objidlen,
                                     u_char ** buf, size_t * buf_len,
                                     size_t * out_len,
 ...
 */
static int  Mib_nodeToOid(struct Parse_Tree_s *, _oid *, size_t *);
...
_oid   mib_rFC1213_MIB[] = { 1, 3, 6, 1, 2, 1 };
 ...
 * @return The buffer.
 */
static char * Mib_uptimeString(u_long timeticks, char *buf, size_t buflen)
{
    int centisecs, seconds, minutes, hours, days;

    if (DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::NUMERIC_TIMETICKS)) {
    ...
    if (DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::QUICK_PRINT))
                       ...
                       int allow_realloc, const u_char * cp, size_t line_len)
{
     ...
     */
    if (DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::PRINT_HEX_TEXT)) {
...
{
    int line_len = DefaultStore_getInt(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                                       DEFAULTSTORE_LIB_INTEGER::HEX_OUTPUT_LENGTH);
    if (!line_len)
        line_len=len;
                            ...
                            const char *units)
{
    ...
    if (var->type != ASN01_OCTET_STR) {
        if (!DefaultStore_getBoolean( DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::QUICKE_PRINT)) {
...
#define HEX2DIGIT_NEED_INIT 3
            ...
            while (repeat && cp < ecp) {
                ...
                case 'x':
                    if (HEX2DIGIT_NEED_INIT == hex2digit)
                        hex2digit = DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                                                           DEFAULTSTORE_LIB_BOOLEAN::TWODIGIT_HEX_OUTPUT);
                    /*
    ...
    output_format = DefaultStore_getInt(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::STRING_OUTPUT_FORMAT);
    if (0 == output_format) {
                         ...
                         const char *hint, const char *units)
{
    ...
    Mib_uptimeString(*(u_long *) (var->val.integer), timebuf, sizeof(timebuf));
                       ...
                       const char *hint, const char *units)
{
    ...
    if (enum_string == NULL || DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::PRINT_NUMERIC_ENUM)) {
        if (hint) {
    ...
    } else if (DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::QUICK_PRINT)) {
        if (!Tools_strcat
            (buf, buf_len, out_len, allow_realloc,
                        ...
                        const char *hint, const char *units)
{
    ...
    if (enum_string == NULL ||
        DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::PRINT_NUMERIC_ENUM)) {
        if (hint) {
            if (!(Mib_sprintReallocHintedInteger(buf, buf_len, out_len,
                                                allow_realloc,
                                                *var->val.integer, 'u',
...
static void Mib_handlePrintNumeric(const char *token, char *line)
{
    ...
    if (value && (
        ...
        DefaultStore_setInt(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::OID_OUTPUT_FORMAT, MIB_OID_OUTPUT_NUMERIC);
    }
}
...
char * Mib_outToggleOptions(char *options)
{
    while (*options) {
        switch (*options++) {
        case '0':
           DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::TWODIGIT_HEX_OUTPUT);
            break;
        case 'a':
            DefaultStore_setInt(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::STRING_OUTPUT_FORMAT,
                                                      MIB_STRING_OUTPUT_ASCII);
            break;
        case 'b':
            DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::DONT_BREAKDOWN_OIDS);
            break;
        case 'e':
            DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::PRINT_NUMERIC_ENUM);
            break;
        case 'E':
            DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::ESCAPE_QUOTES);
            break;
        case 'f':
            DefaultStore_setInt(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::OID_OUTPUT_FORMAT,
                                                      MIB_OID_OUTPUT_FULL);
            break;
        case 'n':
            DefaultStore_setInt(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::OID_OUTPUT_FORMAT,
            ...
            break;
        case 'q':
            DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::QUICK_PRINT);
            break;
        case 'Q':
            DefaultStore_setBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,  DEFAULTSTORE_LIB_BOOLEAN::QUICKE_PRINT, 1);
            DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::QUICK_PRINT);
            break;
        case 's':
            DefaultStore_setInt(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::OID_OUTPUT_FORMAT,
        ...
        case 'S':
            ...
            break;
        case 't':
            DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::NUMERIC_TIMETICKS);
            break;
        case 'T':
            DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::PRINT_HEX_TEXT);
            break;
        case 'u':
            DefaultStore_setInt(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::OID_OUTPUT_FORMAT,
                                                      MIB_OID_OUTPUT_UCD);
            break;
        case 'U':
            DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::DONT_PRINT_UNITS);
            break;
        case 'v':
            DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::PRINT_BARE_VALUE);
            break;
        case 'x':
            DefaultStore_setInt(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::STRING_OUTPUT_FORMAT,
            ...
            break;
        case 'X':
            DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::EXTENDED_INDEX);
            break;
        default:
            return options - 1;
        }
...
char * Mib_inOptions(char *optarg, int argc, char *const *argv)
{
    ...
    for (cp = optarg; *cp; cp++) {
        switch (*cp) {
        case 'b':
            DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::REGEX_ACCESS);
            break;
        case 'R':
            DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::RANDOM_ACCESS);
            break;
        case 'r':
            DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::DONT_CHECK_RANGE);
            break;
        case 'h':
            DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::NO_DISPLAY_HINT);
            break;
        case 'u':
            DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::READ_UCD_STYLE_OID);
            break;
        case 's':
            ...
            DefaultStore_setString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                                  DEFAULTSTORE_LIB_STRING::OIDSUFFIX, cp);
            return NULL;
        ...
        case 'S':
            ...
            if (!*(++cp))
                cp = argv[optind++];
            DefaultStore_setString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                                  DEFAULTSTORE_LIB_STRING::OIDPREFIX, cp);
            return NULL;
...
void Mib_registerMibHandlers(void)
{
    ...
    DefaultStore_registerPremib(ASN01_BOOLEAN, "snmp", "showMibErrors", DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::MIB_ERRORS);
    DefaultStore_registerPremib(ASN01_BOOLEAN, "snmp", "commentToEOL",  DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::MIB_COMMENT_TERM);  /* Describes actual behaviour */
    DefaultStore_registerPremib(ASN01_BOOLEAN, "snmp", "strictCommentTerm", DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::MIB_COMMENT_TERM); /* Backward compatibility */
    DefaultStore_registerPremib(ASN01_BOOLEAN, "snmp", "mibAllowUnderline", DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::MIB_PARSE_LABEL);
    DefaultStore_registerPremib(ASN01_INTEGER, "snmp", "mibWarningLevel", DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::MIB_WARNINGS);
    DefaultStore_registerPremib(ASN01_BOOLEAN, "snmp", "mibReplaceWithLatest", DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::MIB_REPLACE);

    DefaultStore_registerPremib(ASN01_BOOLEAN, "snmp", "printNumericEnums", DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::PRINT_NUMERIC_ENUM);
    ReadConfig_registerPrenetMibHandler("snmp", "printNumericOids", Mib_handlePrintNumeric, NULL, "(1|yes|true|0|no|false)");
    DefaultStore_registerPremib(ASN01_BOOLEAN, "snmp", "escapeQuotes", DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::ESCAPE_QUOTES);
    DefaultStore_registerPremib(ASN01_BOOLEAN, "snmp", "dontBreakdownOids", DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::DONT_BREAKDOWN_OIDS);
    DefaultStore_registerPremib(ASN01_BOOLEAN, "snmp", "quickPrinting", DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::QUICK_PRINT);
    DefaultStore_registerPremib(ASN01_BOOLEAN, "snmp", "numericTimeticks", DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::NUMERIC_TIMETICKS);
    DefaultStore_registerPremib(ASN01_INTEGER, "snmp", "oidOutputFormat", DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::OID_OUTPUT_FORMAT);
    DefaultStore_registerPremib(ASN01_INTEGER, "snmp", "suffixPrinting", DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::OID_OUTPUT_FORMAT);
    DefaultStore_registerPremib(ASN01_BOOLEAN, "snmp", "extendedIndex", DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::EXTENDED_INDEX);
    DefaultStore_registerPremib(ASN01_BOOLEAN, "snmp", "printHexText", DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::PRINT_HEX_TEXT);
    DefaultStore_registerPremib(ASN01_BOOLEAN, "snmp", "printValueOnly", DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::PRINT_BARE_VALUE);
    DefaultStore_registerPremib(ASN01_BOOLEAN, "snmp", "dontPrintUnits", DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::DONT_PRINT_UNITS);
    DefaultStore_registerPremib(ASN01_INTEGER, "snmp", "hexOutputLength", DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::HEX_OUTPUT_LENGTH);
}
...
void Mib_setMibDirectory(const char *dir)
{
    ...
    olddir = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_STRING::MIBDIRS);
    if (olddir) {
        if ((*dir == '+') || (*dir == '-')) {
    ...
    DefaultStore_setString(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_STRING::MIBDIRS, newdir);
...
char * Mib_getMibDirectory(void)
{
    ...
    DEBUG_TRACE;
    dir = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_STRING::MIBDIRS);
    if (dir == NULL) {
...
{                               /* number of subid's in "output" */
    ...
    if (*input == '.')
        input++;
    else if (DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::READ_UCD_STYLE_OID)) {
        /*
    ...
    if ((root == NULL) && (parse_treeHead != NULL)) {
        ...
        API_SET_PRIOT_ERROR(API_ERR_NOMIB);
        *out_len = 0;
        return 0;
    }
    name = strdup(input);
                             ...
                             const _oid * objid, size_t objidlen)
{
    ...
    output_format = DefaultStore_getInt(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::OID_OUTPUT_FORMAT);
    if (0 == output_format) {
              ...
              const _oid * objid, size_t objidlen)
{
    ...
    if (Mib_sprintReallocObjid((u_char **) & buf, &buf_len, &out_len, 0,
                             objid, objidlen)) {
        return (int) out_len;
    } else {
        return -1;
 ...
 */
void Mib_printObjid(const _oid * objid, size_t objidlen)
                        ...
                        const Types_VariableList * variable)
{
    ...
    if (!DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::PRINT_BARE_VALUE)) {
        if (DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::QUICKE_PRINT)) {
            if (!Tools_strcat
                (buf, buf_len, out_len, allow_realloc,
                 (const u_char *) " = ")) {
    ...
    if (variable->type == PRIOT_NOSUCHOBJECT) {
        ...
        if (!DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::DONT_PRINT_UNITS)) {
            units = subtree->units;
        }

        if (!DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::NO_DISPLAY_HINT)) {
            hint = subtree->hint;
        }
 ...
 */
void Mib_printVariable(const _oid * objid,
                     ...
                     const Types_VariableList * variable)
{
    ...
    subtree = Mib_getTree(objid, objidlen, subtree);
        if (subtree && !DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::DONT_PRINT_UNITS)) {
            units = subtree->units;
        }
        if (subtree) {
        if(subtree->printomat) {
...
void Mib_printValue(const _oid * objid,
            size_t objidlen, const Types_VariableList * variable)
{
 ...
 *
 * @return API_ERR_SUCCESS or API_ERR_GENERR
 */
int Mib_buildOidSegment(Types_VariableList * var)
{
    ...
    case ASN01_PRIV_IMPLIED_OBJECT_ID:
        var->nameLength = var->valLen / sizeof(_oid);
        if (var->nameLength > (sizeof(var->nameLoc) / sizeof(_oid)))
            var->name = (_oid *) malloc(sizeof(_oid) * (var->nameLength));
        else
            var->name = var->nameLoc;
        if (var->name == NULL)
    ...
    case ASN01_OBJECT_ID:
        var->nameLength = var->valLen / sizeof(_oid) + 1;
        if (var->nameLength > (sizeof(var->nameLoc) / sizeof(_oid)))
            var->name = (_oid *) malloc(sizeof(_oid) * (var->nameLength));
        else
            var->name = var->nameLoc;
    ...
    case ASN01_PRIV_IMPLIED_OCTET_STR:
        ...
        for (i = 0; i < (int) var->valLen; i++)
            var->name[i] = (_oid) var->val.string[i];
        break;
    ...
    case ASN01_OCTET_STR:
        ...
        var->name[0] = (_oid) var->valLen;
        for (i = 0; i < (int) var->valLen; i++)
            var->name[i + 1] = (_oid) var->val.string[i];
        break;
...
int Mib_buildOidNoalloc(_oid * in, size_t in_len, size_t * out_len,
                  _oid * prefix, size_t prefix_len,
                  Types_VariableList * indexes)
{
    ...
    if (prefix) {
        if (in_len < prefix_len)
            return API_ERR_GENERR;
        memcpy(in, prefix, prefix_len * sizeof(_oid));
        *out_len = prefix_len;
    } else {
        *out_len = 0;
    }
    ...
    for (var = indexes; var != NULL; var = var->nextVariable) {
        if (Mib_buildOidSegment(var) != API_ERR_SUCCESS)
            return API_ERR_GENERR;
        if (var->nameLength + *out_len <= in_len) {
            memcpy(&(in[*out_len]), var->name,
                   sizeof(_oid) * var->nameLength);
...
int Mib_buildOid(_oid ** out, size_t * out_len,
          _oid * prefix, size_t prefix_len, Types_VariableList * indexes)
{
    _oid             tmpout[TYPES_MAX_OID_LEN];

    /*
     * xxx-rks: inefficent. try only building segments to find index len:
     *   for (var = indexes; var != NULL; var = var->next_variable) {
     *      if (build_oid_segment(var) != API_ERR_SUCCESS)
     *         return API_ERR_GENERR;
    ...
    if (Mib_buildOidNoalloc(tmpout, sizeof(tmpout), out_len,
                          prefix, prefix_len, indexes) != API_ERR_SUCCESS)
        return API_ERR_GENERR;

    /** xxx-rks: should free previous value? */
    Client_cloneMem((void **) out, (void *) tmpout, *out_len * sizeof(_oid));
    ...
    if (var == NULL || ((*oidLen == 0) && (complete == 0)))
            ...
            for (i = 0; i < 4 && i < *oidLen; ++i) {
                if (oidIndex[i] > 255) {
                    DEBUG_MSGTL(("parse_oid_indexes",
                                ...
                                oidIndex[0]));
                        return API_ERR_GENERR;  /* sub-identifier too large */
                    }
        ...
        case ASN01_PRIV_IMPLIED_OBJECT_ID:
            ...
            if (uitmp > TYPES_MAX_OID_LEN)
                return API_ERR_GENERR;  /* too big and illegal */

            if (uitmp > *oidLen) {
                memcpy(tmpout, oidIndex, sizeof(_oid) * (*oidLen));
                memset(&tmpout[*oidLen], 0x00,
                       sizeof(_oid) * (uitmp - *oidLen));
            ...
            DEBUG_MSGOID(("parse_oid_indexes",
                         var->val.objid, var->valLen / sizeof(_oid)));
...
int Mib_dumpReallocOidToInetAddress(const int addr_type, const _oid * objid, size_t objidlen,
                                u_char ** buf, size_t * buf_len,
                                size_t * out_len, int allow_realloc,
                                char quotechar)
{
...
int Mib_dumpReallocOidToString(const _oid * objid, size_t objidlen,
                           u_char ** buf, size_t * buf_len,
                           size_t * out_len, int allow_realloc,
                           char quotechar)
{
    if (buf) {
        ...
        for (i = 0, alen = 0; i < (int) objidlen; i++) {
            ...
            if (alen == 0) {
                if (DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::ESCAPE_QUOTES)) {
...
void Mib_oidFinishPrinting(const _oid * objid, size_t objidlen,
                    ...
                    struct Parse_IndexList_s *in_dices, size_t * end_of_known)
{
    ...
    int             extended_index =
        DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::EXTENDED_INDEX);
    int             output_format =
        DefaultStore_getInt(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::OID_OUTPUT_FORMAT);
    char            intbuf[64];
    ...
    while (in_dices && (objidlen > 0) &&
           (MIB_OID_OUTPUT_NUMERIC != output_format) &&
           !DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::DONT_BREAKDOWN_OIDS)) {
        size_t          numids;
        ...
        case PARSE_TYPE_TIMETICKS:
            ...
            if (extended_index) {
                Mib_uptimeString( *objid, intbuf, sizeof( intbuf ) );
            } else {
                sprintf(intbuf, "%" "l" "u", *objid);
            }
            if (!*buf_overflow && !Tools_strcat(buf, buf_len, out_len,
...
struct Parse_Tree_s * Mib_getTree(const _oid * objid, size_t objidlen, struct Parse_Tree_s *subtree)
{
    struct Parse_Tree_s    *return_tree = NULL;
 ...
 */
void Mib_printDescription(_oid * objid, size_t objidlen, int width) /* number of subidentifiers */
{
...
int Mib_getModuleNode(const char *fname,
                const char *module, _oid * objid, size_t * objidlen)
{
    int             modid, rc = 0;
    struct Parse_Tree_s    *tp;
    char           *name, *cp;
 ...
 */
static int Mib_nodeToOid(struct Parse_Tree_s *tp, _oid * objid, size_t * objidlen)
{
    int             numids, lenids;
    _oid            *op;
    ...
    *objidlen = (size_t) numids;
    ...
    if (numids < lenids)
        memmove(objid, op, numids * sizeof(_oid));
...
static int Mib_addStringsToOid(struct Parse_Tree_s *tp, char *cp,
                    _oid * objid, size_t * objidlen, size_t maxlen)
{
    _oid             subid;
    ...
    char            doingquote;
    int             len = -1, pos = -1;
    int             check = !DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::DONT_CHECK_RANGE);
    int             do_hint = !DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::NO_DISPLAY_HINT);
 ...
 */
int Mib_getWildNode(const char *name, _oid * objid, size_t * objidlen)
{
...
int Mib_getNode(const char *name, _oid * objid, size_t * objidlen)
{
    const char     *cp;
    char            ch;
    int             res;
    ...
    if (res == 0) {
        API_SET_PRIOT_ERROR(API_ERR_UNKNOWN_OBJID);
    }
 ...
 */
static void     Mib_printParentLabeledOid(FILE *, struct tree *);
...
{
    Mib_uptimeString(timeticks, buf, buflen);
    return buf;
}
...
_oid * Mib_parseOid(const char *argv, _oid * root, size_t * rootlen)
{
    ...
    suffix = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_STRING::OIDSUFFIX);
    prefix = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_STRING::OIDPREFIX);
    ...
    if (DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::RANDOM_ACCESS)
        || strchr(argv, ':')) {
        if (Mib_getNode(argv, root, rootlen)) {
            return root;
        }
    } else if (DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::REGEX_ACCESS)) {
    Mib_clearTreeFlags(parse_treeHead);
 ...
 * @return 0 on Sucess, 1 on failure.
 */
int Mib_oid2chars(char *C, int L, const _oid * O)
{
    char           *c = C;
    const _oid      *o = &O[1];
 ...
 * @return 0 on Sucess, 1 on failure.
 */
int Mib_oid2str(char *S, int L, _oid * O)
{
    int            rc;

...
static char** Service_createWordArrayHelper(const char* cptr, size_t idx, char* tmp, size_t tmplen)
{
    char* item;
    char** res;
    ...
    item = strdup(tmp);
    if (cptr)
        res = Service_createWordArrayHelper(cptr, idx + 1, tmp, tmplen);
    else {
...
static char** Service_createWordArray(const char* cptr)
{
    size_t tmplen = strlen(cptr);
    char* tmp = (char*)malloc(tmplen + 1);
    char** res = Service_createWordArrayHelper(cptr, 0, tmp, tmplen + 1);
    free(tmp);
    return res;
}

static void Service_destroyWordArray(char** arr)
{
    if (arr) {
        char** run = arr;
        while(*run) {
...
static struct Service_LookupDomain_s* service_domains = NULL;

int Service_registerDefaultDomain(const char* application, const char* domain)
{
    struct Service_LookupDomain_s *run = service_domains, *prev = NULL;
    int res = 0;
    ...
    if (run && strcmp(run->application, application) == 0) {
      if (run->domain != NULL) {
          Service_destroyWordArray(run->domain);
      run->domain = NULL;
      res = 1;
      }
    } else {
    ...
    if (prev) {
        ...
        run->next = service_domains;
        service_domains = run;
    }
    }
    if (domain) {
        run->domain = Service_createWordArray(domain);
    } else if (run->userDomain == NULL) {
    if (prev)
        prev->next = run->next;
    else
        service_domains = run->next;
...
{
    while (service_domains) {
    struct Service_LookupDomain_s *tmp = service_domains;
    service_domains = service_domains->next;
    free(tmp->application);
        Service_destroyWordArray(tmp->userDomain);
        Service_destroyWordArray(tmp->domain);
    free(tmp);
    }
}

static void Service_registerUserDomain(const char* token, char* cptr)
{
    struct Service_LookupDomain_s *run = service_domains, *prev = NULL;
...

    {
        ...
        domain = Service_createWordArray(cp);
    }
    ...
    if (run->userDomain != NULL) {
        ReadConfig_configPerror("Default transport already registered for this "
              "application");
            Service_destroyWordArray(domain);
            free(application);
        return;
    }
    } else {
...
static void Service_clearUserDomain(void)
{
    struct Service_LookupDomain_s *run = service_domains, *prev = NULL;

    while (run) {
    if (run->userDomain != NULL) {
            Service_destroyWordArray(run->userDomain);
        run->userDomain = NULL;
    }
    if (run->domain == NULL) {
...
const char* const * Service_lookupDefaultDomains(const char* application)
{
    ...
    res = NULL;
        ...
        struct Service_LookupDomain_s *run = service_domains;
...
static struct Service_LookupTarget_s* service_targets = NULL;
...
int Service_registerDefaultTarget(const char* application, const char* domain,
                const char* target)
{
    struct Service_LookupTarget_s *run = service_targets, *prev = NULL;
    int i = 0, res = 0;
...
{
    while (service_targets) {
    struct Service_LookupTarget_s *tmp = service_targets;
    service_targets = service_targets->next;
    free(tmp->application);
    free(tmp->domain);
    free(tmp->userTarget);
    free(tmp->target);
...
static void Service_registerUserTarget(const char* token, char* cptr)
{
    struct Service_LookupTarget_s *run = service_targets, *prev = NULL;
    size_t len = strlen(cptr) + 1;
    char* application = (char*)malloc(len);
...
static void Service_clearUserTarget(void)
{
    struct Service_LookupTarget_s *run = service_targets, *prev = NULL;
...
{
    int i = 0;
    struct Service_LookupTarget_s *run = service_targets;
    const char *res;
...
{
    ReadConfig_registerConfigHandler("snmp:", "defDomain",
                Service_registerUserDomain,
                Service_clearUserDomain,
                "application domain");
    ReadConfig_registerConfigHandler("snmp:", "defTarget",
                Service_registerUserTarget,
                Service_clearUserTarget,

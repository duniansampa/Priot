...
static u_long   v3_engineBoots = 1;
static unsigned int v3_engineIDType = ENGINEID_TYPE_NETSNMP_RND;
static unsigned char * v3_engineID = NULL;
static size_t   v3_engineIDLength = 0;
static unsigned char * v3_engineIDNic = NULL;
static unsigned int v3_engineIDIsSet = 0;  /* flag if ID set by config */
static unsigned char * v3_oldEngineID = NULL;
static size_t   v3_oldEngineIDLength = 0;
static struct timeval v3_starttime;

static int      V3_getHwAddress(const char *networkDevice, char *addressOut);
...
void V3_secLevelConf(const char *word, char *cptr)
{
    ...
    if ((secLevel = V3_parseSecLevelConf( word, cptr )) >= 0 ) {
        DefaultStore_setInt(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::SECLEVEL, secLevel);
    } else {
    ReadConfig_error("Unknown security level: %s", cptr);
    }
    DEBUG_MSGTL(("snmpv3", "default secLevel set to: %s = %d\n", cptr,
                DefaultStore_getInt(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                   DEFAULTSTORE_LIB_INTEGER::SECLEVEL)));
}
...
int V3_setupEngineID(u_char ** eidp, const char *text)
{
    ...
    u_char         *bufp = NULL;
    size_t          len;
    int             localEngineIDType = v3_engineIDType;
    int             tmpint;
    time_t          tmptime;
     ...
     */
    if (NULL != text) {
        v3_engineIDType = localEngineIDType = ENGINEID_TYPE_TEXT;
    }
    /*
    ...
    case ENGINEID_TYPE_NETSNMP_RND:        /* Net-SNMP specific encoding */
        if (v3_engineID)           /* already setup, keep current value */
            return v3_engineIDLength;
        if (v3_oldEngineID) {
            len = v3_oldEngineIDLength;
        } else {
    ...
    case ENGINEID_TYPE_NETSNMP_RND:
        if (v3_oldEngineID) {
             ...
             */
            memcpy(bufp, v3_oldEngineID, v3_oldEngineIDLength);
        } else {
            /*
    ...
    case ENGINEID_TYPE_MACADDR:
        {
             ...
             */
            if (NULL == v3_engineIDNic) {
          x = V3_getHwAddress(DEFAULT_NIC, (char *)&bufp[5]);
            } else {
          x = V3_getHwAddress((char *)v3_engineIDNic, (char *)&bufp[5]);
            }
            if (0 != x)
                /*
     ...
     */
    if (localsetup) {
        TOOLS_FREE(v3_engineID);
        v3_engineID = bufp;
        v3_engineIDLength = len;
...
{
    TOOLS_FREE(v3_engineID);
    TOOLS_FREE(v3_engineIDNic);
    TOOLS_FREE(v3_oldEngineID);
    v3_engineIDIsSet = 0;
    return 0;
}
...
{
    v3_engineIDType = atoi(cptr);
    /*
     * verify valid type selected
     */
    switch (v3_engineIDType) {
    case ENGINEID_TYPE_IPV4:   /* IPv4 */
    case ENGINEID_TYPE_IPV6:   /* IPv6 */
        /*
         * IPV? is always good
    ...
    default:
        ...
        v3_engineIDType = ENGINEID_TYPE_IPV4;
    }
...
V3_engineIDNicConf(const char *word, char *cptr)
{
         ...
         */
    {
         ...
         */
        TOOLS_FREE(v3_engineIDNic);
        v3_engineIDNic = (u_char *) malloc(strlen(cptr) + 1);
        if (NULL != v3_engineIDNic) {
            strcpy((char *) v3_engineIDNic, cptr);
...
void V3_versionConf(const char *word, char *cptr)
{
    ...
    if ((strcmp(cptr,  "1") == 0) ||
        (strcmp(cptr, "v1") == 0)) {
        DefaultStore_setInt(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::SNMPVERSION,
               DEFAULTSTORE_PRIOT_VERSION_1);       /* bogus value */
        valid = 1;
    }
    if ((strcasecmp(cptr,  "2c") == 0) ||
               (strcasecmp(cptr, "v2c") == 0)) {
        DefaultStore_setInt(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::SNMPVERSION,
               DEFAULTSTORE_PRIOT_VERSION_2c);
        valid = 1;
    }
    ...
    DEBUG_MSGTL(("snmpv3", "set default version to %d\n",
                DefaultStore_getInt(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                   DEFAULTSTORE_LIB_INTEGER::SNMPVERSION)));
}
...
{
    ReadConfig_readOctetString(cptr, &v3_oldEngineID, &v3_oldEngineIDLength);
}
...
{
    ReadConfig_readOctetString(cptr, &v3_engineID, &v3_engineIDLength);
    if (v3_engineIDLength > MAX_ENGINEID_LENGTH) {
    ReadConfig_error(
        "exactEngineID '%s' too long; truncating to %d bytes",
        cptr, MAX_ENGINEID_LENGTH);
        v3_engineID[MAX_ENGINEID_LENGTH - 1] = '\0';
        v3_engineIDLength = MAX_ENGINEID_LENGTH;
    }
    v3_engineIDIsSet = 1;
    v3_engineIDType = ENGINEID_TYPE_EXACT;
}
...
{
    Tools_getMonotonicClock(&v3_starttime);
    ...
    DefaultStore_registerConfig(ASN01_OCTET_STR, "snmp", "defSecurityName",
                   DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_STRING::SECNAME);
    DefaultStore_registerConfig(ASN01_OCTET_STR, "snmp", "defContext",
                   DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_STRING::CONTEXT);
    ...
    ReadConfig_registerConfigHandler("snmp", "defSecurityLevel",
                        ...
                        void *clientarg)
{
     ...
     */
    if (engineIDLen != v3_oldEngineIDLength ||
        v3_oldEngineID == NULL || c_engineID == NULL ||
        memcmp(v3_oldEngineID, c_engineID, engineIDLen) != 0) {
        v3_engineBoots = 1;
    }
...
int V3_store(int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    sprintf(line, "engineBoots %ld", v3_engineBoots);
...
V3_getEngineID(u_char * buf, size_t buflen)
{
     ...
     * Sanity check.
     */
    if (!buf || (buflen < v3_engineIDLength)) {
        return 0;
    }
    if (!v3_engineID) {
        return 0;
    ...
    memcpy(buf, v3_engineID, v3_engineIDLength);
    return v3_engineIDLength;
...
{
    u_char         *newID;
    newID = (u_char *) malloc(v3_engineIDLength);

    if (newID) {
        *length = V3_getEngineID(newID, v3_engineIDLength);
    }
...
V3_localEngineTime(void)
{
    ...
    engineTime = System_calculateSectimeDiff(&now, &v3_starttime) & 0x7fffffffL;
 ...
 */
static int
V3_getHwAddress(const char *networkDevice, /* e.g. "eth0", "eth1" */
             char *addressOut)

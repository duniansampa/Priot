                 ...
                 int reqid, Types_Pdu *pdu, void *magic)
{
    ...
    if (op == API_CALLBACK_OP_RECEIVED_MESSAGE && pdu) {
        if (pdu->command == PRIOT_MSG_REPORT) {
            ...
            char msg_buf[50];
            state->status = CLIENT_STAT_ERROR;
            session->s_snmp_errno = API_ERR_PROTOCOL;
            API_SET_PRIOT_ERROR(API_ERR_PROTOCOL);
        ...
        API_SET_PRIOT_ERROR(API_ERR_TIMEOUT);
    } else if (op == API_CALLBACK_OP_DISCONNECT) {
        state->pdu = NULL;
        state->status = CLIENT_STAT_ERROR;
        session->s_snmp_errno = API_ERR_ABORT;
        API_SET_PRIOT_ERROR(API_ERR_ABORT);
    }
...
Client_clonePduHeader(Types_Pdu *pdu)
{
    ...
    if (Client_cloneMem((void **) &newpdu->enterprise, pdu->enterprise,
                       sizeof(_oid) * pdu->enterpriseLength) ||
...
static
Types_Pdu    *
Client_clonePdu(Types_Pdu *pdu, int drop_err)
{
    Types_Pdu    *newpdu;
...
{
    return Client_clonePdu(pdu, 0);  /* copies all variables */
}
...
Client_fixPdu(Types_Pdu *pdu, int command)
{
    ...
    newpdu = Client_clonePdu(pdu, 1);        /* copies all except errored variable */
    if (!newpdu)
        return NULL;
    if (!newpdu->variables) {
        Api_freePdu(newpdu);
...
Client_setVarObjid(Types_VariableList * vp,
                   const _oid * objid, size_t name_length)
{
    size_t          len = sizeof(_oid) * name_length;
                   ...
                   const void * value, size_t len)
{
    ...
    case ASN01_PRIV_EXCL_RANGE:
        if (largeval) {
            vars->val.objid = (_oid *) malloc(vars->valLen);
        }
        ...
        if (count > 0) {
            ...
            case -1:
                if (errno == EINTR) {
                    ...
                    api_priotErrno = API_ERR_GENERR;    /*MTCRITICAL_RESOURCE */
                    /*
                     ...
                     * ss->s_snmp_errno = API_ERR_GENERR;
                     * ss->s_errno = errno;
                     */
...
Client_queryGetDefaultSession( void ) {
    ...
    if (! client_defQuerySession && ! warning_logged) {
        if (! DefaultStore_getString(DEFAULTSTORE_STORAGE::APPLICATION_ID,
...
Client_stateMachineRun( Client_StateMachineInput *input)
{
    ...
    Assert_requirePtrLRV( input, API_ERR_GENERR );
    Assert_requirePtrLRV( input->steps, API_ERR_GENERR );
    ...
    while (current) {
        ...
        if (API_ERR_SUCCESS == current->result)
            current = current->on_success;
        else if (API_ERR_ABORT == current->result) {
                 ...
                 Client_StateMachineStep *step)
{
    ...
    Assert_requirePtrLRV( input, API_ERR_ABORT );
    Assert_requirePtrLRV( step, API_ERR_ABORT );
                  ...
                  Client_StateMachineStep *step)
{
    ...
    Assert_requirePtrLRV( input, API_ERR_GENERR );
    Assert_requirePtrLRV( step, API_ERR_GENERR );
    Assert_requirePtrLRV( input->input_context, API_ERR_GENERR );
                                    ...
                                    Client_StateMachineStep *step)
{
    ...
    Assert_requirePtrLRV(row_status, API_ERR_GENERR);
                                       ...
                                       Client_StateMachineStep *step)
{
    ...
    Assert_requirePtrLRV(vars, API_ERR_GENERR);
                   ...
                   int row_status_index)
{
    ...
    Assert_requirePtrLRV( sess, API_ERR_GENERR);
    Assert_requirePtrLRV( vars, API_ERR_GENERR);

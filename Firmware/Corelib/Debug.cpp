...
int Debug_enableTokenLogs (const char *token) {
   ...
   if (debug_numTokens == 0 || debug_printEverything) {
       ...
       for(i=0; i < debug_numTokens; i++) {
           if (debug_tokens[i].tokenName &&
               strncmp(debug_tokens[i].tokenName, token,
                       strlen(debug_tokens[i].tokenName)) == 0) {
    ...
    for (i = 0; i < debug_numTokens; i++) {
        ...
        if (debug_tokens[i].tokenName &&
            ...
            if (DEBUG_ACTIVE == debug_tokens[i].enabled)
                return API_ERR_SUCCESS; /* active */
            else
                return API_ERR_GENERR; /* excluded */
        }
    }
    return rc;
}
...
{
    if (Debug_isTokenRegistered(token) == API_ERR_SUCCESS) {
        va_list         debugargs;
...
void Debug_msgOid(const char *token, const _oid * theoid, size_t len)
{
    ...
    if (Mib_sprintReallocObjid(&buf, &buf_len, &out_len, 1, theoid, len)) {
        if (buf != NULL) {
                  ...
                  size_t var_subid, _oid range_ubound)
{
    ...
    if (var_subid == 0) {
        rc = Mib_sprintReallocObjid(&buf, &buf_len, &out_len, 1, theoid,
                                  len);
    } else {
        char            tmpbuf[128];
        ...
        if (rc) {
            ...
            rc = Tools_strcat(&buf, &buf_len, &out_len, 1, (const u_char *)tmpbuf);
            if (rc) {
                for (i = var_subid; i < len; i++) {
                    sprintf(tmpbuf, ".%lu", theoid[i]);
                    if (!Tools_strcat(&buf, &buf_len, &out_len, 1, (const u_char *)tmpbuf)) {
                        break;
                    }
                }
            }
...
{
    ReadConfig_registerPrenetMibHandler("snmp", "doDebugging",
                                    Debug_configTurnOnDebugging, NULL,
                                    "(1|0)");
    ReadConfig_registerPrenetMibHandler("snmp", "debugTokens",

           ...
           u_char * ciphertext, size_t * ctlen)
{
    ...
    if (!privtype || !key || !iv || !plaintext || !ciphertext || !ctlen
        ...
        SCAPI_QUITFUN(ErrorCode_GENERR, sc_encrypt_quit);
    } else if (ptlen > *ctlen) {
        SCAPI_QUITFUN(ErrorCode_GENERR, sc_encrypt_quit);
    }
    ...
    if (TOOLS_ISTRANSFORM(privtype, dESPriv)) {
        ...
        if (pad == pad_size)
            pad = 0;
        if (ptlen + pad > *ctlen) {
            SCAPI_QUITFUN(ErrorCode_GENERR, sc_encrypt_quit);    /* not enough space */
        }
           ...
           u_char * plaintext, size_t * ptlen)
{
    ...
    if (!privtype || !key || !iv || !plaintext || !ciphertext || !ptlen
        ...
        || (privtypelen != TOOLS_USM_LENGTH_OID_TRANSFORM)) {
        SCAPI_QUITFUN(ErrorCode_GENERR, sc_decrypt_quit);
    }

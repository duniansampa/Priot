...
void Parse_mibToggleOptionsUsage(const char *lead, FILE * outf)
{
    fprintf(outf, "%su:  %sallow the use of underlines in MIB symbols\n",
            lead, ((DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::MIB_PARSE_LABEL)) ?
           "dis" : ""));
    fprintf(outf, "%sc:  %sallow the use of \"--\" to terminate comments\n",
            lead, ((DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::MIB_COMMENT_TERM)) ?
           "" : "dis"));

    fprintf(outf, "%sd:  %ssave the DESCRIPTIONs of the MIB objects\n",
            lead, ((DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::SAVE_MIB_DESCRS)) ?
           "do not " : ""));
...
char * Parse_mibToggleOptions(char *options)
{
    if (options) {
        while (*options) {
            switch (*options) {
            case 'u':
                DefaultStore_setBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::MIB_PARSE_LABEL,
                               !DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                                               DEFAULTSTORE_LIB_BOOLEAN::MIB_PARSE_LABEL));
                break;

            case 'c':
                DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::MIB_COMMENT_TERM);
                break;

            case 'e':
                DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::MIB_ERRORS);
                break;

            case 'w':
                DefaultStore_setInt(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::MIB_WARNINGS, 1);
                break;

            case 'W':
                DefaultStore_setInt(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_INTEGER::MIB_WARNINGS, 2);
                break;

            case 'd':
                DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::SAVE_MIB_DESCRS);
                break;

            case 'R':
                DefaultStore_toggleBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::MIB_REPLACE);
                break;
        ...
        for (child2 = tp2->child_list, previous = NULL;
            ...
            if (child1->subid == child2->subid) {
                ...
                if (!strncmp(child1->label, PARSE_ANON, PARSE_ANON_LEN)) {
                    ...
                    if (DefaultStore_getInt(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                           DEFAULTSTORE_LIB_INTEGER::MIB_WARNINGS)) {
        ...
        if (tp) {
            if (!PARSE_LABEL_COMPARE(tp->label, np->label)) {
                ...
                if (DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                       DEFAULTSTORE_LIB_BOOLEAN::MIB_REPLACE)) {
                    /*
                     * Replace from node
                     */
            ...
            } else if (DefaultStore_getInt(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                      DEFAULTSTORE_LIB_INTEGER::MIB_WARNINGS)) {
                Logger_log(LOGGER_PRIORITY_WARNING,
                         "Warning: %s.%ld is both %s and %s (%s)\n",
                ...
                if (type == PARSE_DISPLAYHINT) {
                    ...
                    if (type != PARSE_QUOTESTRING)
                        ...
                        hint = strdup(token);
                           ...
                           DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                                                  DEFAULTSTORE_LIB_BOOLEAN::SAVE_MIB_DESCRS)) {
    ...
    if (type == PARSE_LABEL) {
        ...
        if (tctype == PARSE_LABEL &&
            DefaultStore_getInt(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                   DEFAULTSTORE_LIB_INTEGER::MIB_WARNINGS) > 1) {
...
Parse_isLabelchar(int ich)
{
    if ((isalnum(ich)) || (ich == '-'))
        return 1;
    if (ich == '_' && DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                         DEFAULTSTORE_LIB_BOOLEAN::MIB_PARSE_LABEL)) {
        return 1;
    }
...
Parse_getToken(FILE * fp, char *token, int maxtlen)
{
    ...
    *cp = '\0';
    ...
    case '-':
        ...
        if (ch_next == '-') {
            if (DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                       DEFAULTSTORE_LIB_BOOLEAN::MIB_COMMENT_TERM)) {
                /*
...
static int Parse_parseQuoteString(FILE * fp, char *token, int maxtlen)
{
    ...
    for (ch = Parse_getc(fp); ch != EOF; ch = Parse_getc(fp)) {
        ...
        if (ch == '\n') {
            ...
            *token = '\0';
            if (too_long && DefaultStore_getInt(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                       DEFAULTSTORE_LIB_INTEGER::MIB_WARNINGS) > 1) {
                /*

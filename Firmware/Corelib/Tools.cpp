 ...
 */
u_int Tools_binaryToHex(u_char ** dest, size_t *dest_len, int allow_realloc,
...
u_int Tools_binaryToHex(const u_char * input, size_t len, char **output)
{
    ...
    *output = NULL; /* will alloc new buffer */
    ...
    return Tools_binaryToHex((u_char**)output, &out_len, 1, input, len);
}                               /* end binary_to_hex() */
...
{
    int             res;
    marker_t        now = Tools_atimeNewMarker();
...
{
    return Tools_markerTticks((const_marker_t) tv);
}

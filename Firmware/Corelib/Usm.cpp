...
_oid             usm_noAuthProtocol[10] = { 1, 3, 6, 1, 6, 3, 10, 1, 1, 1 };
_oid             usm_hMACMD5AuthProtocol[10] = { 1, 3, 6, 1, 6, 3, 10, 1, 1, 2 };
_oid             usm_hMACSHA1AuthProtocol[10] = { 1, 3, 6, 1, 6, 3, 10, 1, 1, 3 };
_oid             usm_noPrivProtocol[10] = { 1, 3, 6, 1, 6, 3, 10, 1, 2, 1 };
_oid             usm_dESPrivProtocol[10] = { 1, 3, 6, 1, 6, 3, 10, 1, 2, 2 };
_oid             usm_aESPrivProtocol[10] = { 1, 3, 6, 1, 6, 3, 10, 1, 2, 4 };
/* backwards compat */
_oid *           usm_aES128PrivProtocol = usm_aESPrivProtocol;

static u_int    usm_dummyEtime, usm_dummyEboot;       /* For LCDTIME_ISENGINEKNOWN(). */

/*
 * Set up default snmpv3 parameter value storage.
 */
static const _oid * usm_defaultAuthType = NULL;
static size_t       usm_defaultAuthTypeLen = 0;
static const _oid * usm_defaultPrivType = NULL;
static size_t       usm_defaultPrivTypeLen = 0;

/*
 * Globals.
 */
static u_int    usm_saltInteger;
static u_int    usm_saltInteger64One, usm_saltInteger64Two;
        /*
         * 1/2 of seed for the salt.   Cf. RFC2274, Sect 8.1.1.1.
         */

static struct Usm_User_s * usm_noNameUser = NULL;
/*
 * Local storage (LCD) of the default user list.
 */
static struct Usm_User_s * usm_userList = NULL;
...
{
    USM_MAKE_ENTRY(_oid, auth_protocol, auth_protocol_len,
...
{
    USM_MAKE_ENTRY(_oid, priv_protocol, priv_protocol_len,
             ...
             u_char * priv_salt, size_t priv_salt_length, u_char * msgSalt)
{
    ...
    net_salt_int = htonl(usm_saltInteger);
...
#define PROPER_AES_IV_SIZE 64
    ...
    net_salt_int1 = htonl(usm_saltInteger64One);
    net_salt_int2 = htonl(usm_saltInteger64Two);

    if ((usm_saltInteger64Two += 1) == 0)
        usm_saltInteger64Two += 1;
 ...
 *	API_ERR_USM_AUTHENTICATIONFAILURE
 *	API_ERR_USM_ENCRYPTIONERROR
 *	API_ERR_USM_GENERICERROR
 *	API_ERR_USM_UNKNOWNSECURITYNAME
 *	API_ERR_USM_GENERICERROR
 *	API_ERR_USM_UNSUPPORTEDSECURITYLEVEL
 *
 *
...
{                               /* IN/OUT - Len available, len returned. */
     ...
     */
        ...
        if ((user = Usm_getUser(secEngineID, secEngineIDLen, secName))
            ...
            return API_ERR_USM_UNKNOWNSECURITYNAME;
        }
        ...
        if (user) {
            ...
            theAuthProtocolLength =
                sizeof(usm_noAuthProtocol) / sizeof(_oid);
            theAuthKey = NULL;
            theAuthKeyLength = 0;
            thePrivProtocol = usm_noPrivProtocol;
            thePrivProtocolLength =
                sizeof(usm_noPrivProtocol) / sizeof(_oid);
            thePrivKey = NULL;
            thePrivKeyLength = 0;
        }
    ...
    if (Usm_checkSecLevelVsProtocols(theSecLevel,
        ...
        DEBUG_MSGTL(("usm", "Unsupported Security Level (%d)\n",
                    theSecLevel));
        Usm_freeUsmStateReference(secStateRef);
        return API_ERR_USM_UNSUPPORTEDSECURITYLEVEL;
    }
    ...
    if (Usm_calcOffsets(globalDataLen, theSecLevel, theEngineIDLength,
        ...
        Usm_freeUsmStateReference(secStateRef);
        return API_ERR_USM_GENERICERROR;
    }
    ...
    if (theSecLevel == PRIOT_SEC_LEVEL_AUTHPRIV) {
        ...
        if (Scapi_encrypt(thePrivProtocol, thePrivProtocolLength,
            ...
            Usm_freeUsmStateReference(secStateRef);
            return API_ERR_USM_ENCRYPTIONERROR;
        }
    ...
    if (theSecLevel == PRIOT_SEC_LEVEL_AUTHNOPRIV
        ...
        if (Scapi_generateKeyedHash(theAuthProtocol, theAuthProtocolLength,
            ...
            Usm_freeUsmStateReference(secStateRef);
            return API_ERR_USM_AUTHENTICATIONFAILURE;
        }
                                ...
                                u_int time_uint, int *error)
{
    ...
    if ((myIDLength > USM_MAX_ID_LENGTH) || (myIDLength == 0)) {
        ...
        *error = API_ERR_USM_GENERICERROR;
        return -1;
    }
    ...
    if (secEngineIDLen == myIDLength
        ...
        if (boots_uint == TOOLS_ENGINEBOOT_MAX
            ...
            *error = API_ERR_USM_NOTINTIMEWINDOW;
            return -1;
        }

        *error = API_ERR_SUCCESS;
        return 0;
    }
 ...
 *	API_ERR_SUCCESS			On success.
 *	API_ERR_USM_AUTHENTICATIONFAILURE
 *	API_ERR_USM_DECRYPTIONERROR
 *	API_ERR_USM_GENERICERROR
 *	API_ERR_USM_PARSEERROR
 *	API_ERR_USM_UNKNOWNENGINEID
 *	API_ERR_USM_PARSEERROR
 *	API_ERR_USM_UNKNOWNSECURITYNAME
 *	API_ERR_USM_UNSUPPORTEDSECURITYLEVEL
 *
...
{                               /* IN     - v3 Message flags.              */
    ...
    if ((rc = Usm_parseSecurityParameters(secParams, remaining,
        ...
        if (rc == -2) {
            ...
            return API_ERR_USM_DECRYPTIONERROR;
        }
        Api_incrementStatistic(API_STAT_SNMPINASNPARSEERRS);
        return API_ERR_USM_PARSEERROR;
    }
    ...
    if ((sess && (sess->isAuthoritative == API_SESS_AUTHORITATIVE ||
        ...
        if (LCDTIME_ISENGINEKNOWN(secEngineID, *secEngineIDLen) == FALSE) {
            ...
            return API_ERR_USM_UNKNOWNENGINEID;
        }
    } else {
...
Usm_sessionInit(Types_Session *in_session, Types_Session *session)
{
    ...
    if ((in_session->securityAuthKeyLen <= 0) &&
        ((cp = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                     DEFAULTSTORE_LIB_STRING::AUTHMASTERKEY)))) {
        ...
        if (!Tools_hexToBinary1(&tmpp, &buflen,
            ...
            return PRIOT_ERR_GENERR;
               ...
               ((cp = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                                            DEFAULTSTORE_LIB_STRING::AUTHPASSPHRASE)) ||
                (cp = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                                            DEFAULTSTORE_LIB_STRING::PASSPHRASE)))) {
        session->securityAuthKeyLen = USM_AUTH_KU_LEN;
        if (Keytools_generateKu(session->securityAuthProto,
                        session->securityAuthProtoLen,
                        (u_char *) cp, strlen(cp),
                        session->securityAuthKey,
                        &session->securityAuthKeyLen) != API_ERR_SUCCESS) {
            Api_setDetail
    ...
    if ((in_session->securityPrivKeyLen <= 0) &&
        ((cp = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                     DEFAULTSTORE_LIB_STRING::PRIVMASTERKEY)))) {
        ...
        if (!Tools_hexToBinary1(&tmpp, &buflen,
            ...
            return PRIOT_ERR_GENERR;
               ...
               ((cp = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                                            DEFAULTSTORE_LIB_STRING::PRIVPASSPHRASE)) ||
                (cp = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                                            DEFAULTSTORE_LIB_STRING::PASSPHRASE)))) {
        ...
        if (Keytools_generateKu(session->securityAuthProto,
                        ...
                        &session->securityPrivKeyLen) != API_ERR_SUCCESS) {
            Api_setDetail
    ...
    if (user->authKey == NULL) {
        if (session->securityAuthLocalKey != NULL
            ...
            if (Keytools_generateKul(user->authProtocol, user->authProtocolLen,
                             ...
                             &user->authKeyLen) != API_ERR_SUCCESS) {
                Usm_freeUser(user);
                return API_ERR_GENERR;
            }
        } else if ((cp = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                                               DEFAULTSTORE_LIB_STRING::AUTHLOCALIZEDKEY))) {
    ...
    if (user->privKey == NULL) {
        if (session->securityPrivLocalKey != NULL
            ...
            if (Keytools_generateKul(user->authProtocol, user->authProtocolLen,
                             ...
                             &user->privKeyLen) != API_ERR_SUCCESS) {
                Usm_freeUser(user);
                return API_ERR_GENERR;
            }
        } else if ((cp = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                                               DEFAULTSTORE_LIB_STRING::PRIVLOCALIZEDKEY))) {
...
static int
Usm_buildProbePdu(Types_Pdu **pdu)
{
    struct Usm_User_s *user;
    ...
    *pdu = Client_pduCreate(PRIOT_MSG_GET);
    ...
    if (user == NULL) {
        ...
        user->authProtocolLen = sizeof(usm_noAuthProtocol) / sizeof(_oid);
        user->authProtocol =
            Api_duplicateObjid(usm_noAuthProtocol, user->authProtocolLen);
        user->privProtocolLen = sizeof(usm_noPrivProtocol) / sizeof(_oid);
        user->privProtocol =
...
int Usm_discoverEngineid(void *slpv, Types_Session *session) {
    ...
    case CLIENT_STAT_SUCCESS:
        session->s_snmp_errno = API_ERR_INVALID_MSG; /* XX?? */
    ...
    case CLIENT_STAT_ERROR:   /* this is what we expected -> Report == STAT_ERROR */
        session->s_snmp_errno = API_ERR_UNKNOWN_ENG_ID;
        break;
    case CLIENT_STAT_TIMEOUT:
        session->s_snmp_errno = API_ERR_TIMEOUT;
        break;
    default:
        DEBUG_MSGTL(("snmp_sess_open",
    ...
    session->s_snmp_errno = API_ERR_SUCCESS;
...
Usm_initUsm(void)
{
    ...
    ReadConfig_registerConfigHandler("snmp", "defAuthType", Usm_v3AuthtypeConf,
                            NULL, "MD5|SHA");
    ReadConfig_registerConfigHandler("snmp", "defPrivType", Usm_v3PrivtypeConf,
                            NULL,
                            "DES|AES"
                           );
    ...
    type = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_STRING::APPTYPE);
...
Usm_initUsmPostConfig(int majorid, int minorid, void *serverarg,
                     void *clientarg)
{
    size_t          salt_integer_len = sizeof(usm_saltInteger);

    if (Scapi_random((u_char *) & usm_saltInteger, &salt_integer_len) !=
        API_ERR_SUCCESS) {
        DEBUG_MSGTL(("usm", "Scapi_random() failed: using time() as salt.\n"));
        usm_saltInteger = (u_int) time(NULL);
    }

    salt_integer_len = sizeof (usm_saltInteger64One);
    if (Scapi_random((u_char *) & usm_saltInteger64One, &salt_integer_len) !=
        API_ERR_SUCCESS) {
        DEBUG_MSGTL(("usm", "Scapi_random() failed: using time() as aes1 salt.\n"));
        usm_saltInteger64One = (u_int) time(NULL);
    }
    salt_integer_len = sizeof (usm_saltInteger64One);
    if (Scapi_random((u_char *) & usm_saltInteger64Two, &salt_integer_len) !=
        API_ERR_SUCCESS) {
        DEBUG_MSGTL(("usm", "Scapi_random() failed: using time() as aes2 salt.\n"));
        usm_saltInteger64Two = (u_int) time(NULL);
    }

    usm_noNameUser = Usm_createInitialUser("", usm_hMACMD5AuthProtocol,
                                         TOOLS_USM_LENGTH_OID_TRANSFORM,
                                         usm_dESPrivProtocol,
                                         TOOLS_USM_LENGTH_OID_TRANSFORM);

    if ( usm_noNameUser ) {
        TOOLS_FREE(usm_noNameUser->engineID);
        usm_noNameUser->engineIDLen = 0;
    }
...
Usm_deinitUsmPostConfig(int majorid, int minorid, void *serverarg,
               void *clientarg)
{
    if (Usm_freeUser(usm_noNameUser) != NULL) {
...
{
    struct Usm_User_s *tmp = usm_userList, *next = NULL;
    ...
    if ((level == PRIOT_SEC_LEVEL_AUTHPRIV
        ...
        (Api_oidEquals
         (user->authProtocol, user->authProtocolLen, usm_noAuthProtocol,
          sizeof(usm_noAuthProtocol) / sizeof(_oid)) == 0)) {
        DEBUG_MSGTL(("usm", "Level: %d\n", level));
        DEBUG_MSGTL(("usm", "User (%s) Auth Protocol: ", user->name));
        DEBUG_MSGOID(("usm", user->authProtocol, user->authProtocolLen));
        DEBUG_MSG(("usm", ", User Priv Protocol: "));
    ...
    if (level == PRIOT_SEC_LEVEL_AUTHPRIV
        ...
        (Api_oidEquals
         (privProtocol, privProtocolLen, usm_noPrivProtocol,
          sizeof(usm_noPrivProtocol) / sizeof(_oid)) == 0)) {
        DEBUG_MSGTL(("usm", "Level: %d\n", level));
...
Usm_getUser(u_char * engineID, size_t engineIDLen, char *name)
{
    ...
    return Usm_getUserFromList(engineID, engineIDLen, name, usm_userList,
                                  1);
}
...
Usm_addUser(struct Usm_User_s *user)
{
    ...
    uptr = Usm_addUserToList(user, usm_userList);
    if (uptr != NULL)
        usm_userList = uptr;
    return uptr;
}
...
{
    return Usm_removeUserFromList(user, &usm_userList);
}
...
{
    /*
     * copy the authProtocol _oid row pointer
     */
    ...
    /*
     * copy the privProtocol _oid row pointer
     */
...
Usm_createUser(void)
{
    ...
    newUser->authProtocolLen = sizeof(usm_noAuthProtocol) / sizeof(_oid);
    ...
    if ((newUser->privProtocol =
        ...
        NULL)
        return Usm_freeUser(newUser);
    newUser->privProtocolLen = sizeof(usm_noPrivProtocol) / sizeof(_oid);
...
Usm_createInitialUser(const char *name,
                        const _oid * authProtocol, size_t authProtocolLen,
                        const _oid * privProtocol, size_t privProtocolLen)
{
    ...
    if ((newUser->cloneFrom = (_oid *) malloc(sizeof(_oid) * 2)) == NULL)
        return Usm_freeUser(newUser);
...
Usm_storeUsers(int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    if (appname == NULL) {
        appname = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
...
{
    Usm_saveUsersFromList(usm_userList, token, type);
}
...
Usm_setPassword(const char *token, char *line)
{
    ...
    if (strncmp(cp, USM_WILDCARDSTRING, strlen(USM_WILDCARDSTRING)) == 0) {
        ...
        for (user = usm_userList; user != NULL; user = user->next) {
...
Usm_v3AuthtypeConf(const char *word, char *cptr)
{
    if (strcasecmp(cptr, "MD5") == 0)
        usm_defaultAuthType = usm_hMACMD5AuthProtocol;
    else
        if (strcasecmp(cptr, "SHA") == 0)
        usm_defaultAuthType = usm_hMACSHA1AuthProtocol;
    else
        ReadConfig_configPerror("Unknown authentication type");
    usm_defaultAuthTypeLen = TOOLS_USM_LENGTH_OID_TRANSFORM;
...
{
    if (usm_defaultAuthType == NULL) {
        usm_defaultAuthType = API_DEFAULT_AUTH_PROTO;
        usm_defaultAuthTypeLen = API_DEFAULT_AUTH_PROTOLEN;
    }
    if (len)
        *len = usm_defaultAuthTypeLen;
    return usm_defaultAuthType;
}
...
Usm_v3PrivtypeConf(const char *word, char *cptr)
{
    ...
    if (strcasecmp(cptr, "DES") == 0) {
        testcase = 1;
        usm_defaultPrivType = usm_dESPrivProtocol;
    }

    /* XXX AES: assumes _oid length == des _oid length */
    if (strcasecmp(cptr, "AES128") == 0 ||
        strcasecmp(cptr, "AES") == 0) {
        testcase = 1;
        usm_defaultPrivType = usm_aES128PrivProtocol;
    }
    if (testcase == 0)
        ReadConfig_configPerror("Unknown privacy type");
    usm_defaultPrivTypeLen = API_DEFAULT_PRIV_PROTOLEN;
...
{
    if (usm_defaultPrivType == NULL) {
        usm_defaultPrivType = usm_dESPrivProtocol;

        usm_defaultPrivTypeLen = TOOLS_USM_LENGTH_OID_TRANSFORM;
    }
    if (len)
        *len = usm_defaultPrivTypeLen;
    return usm_defaultPrivType;
}

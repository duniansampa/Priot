...
u_char * Asn01_parseObjid(u_char * data,
                size_t * datalength,
                u_char * type, _oid * objid, size_t * objidlength)
{
    ...
    *datalength -= (int) asn_length + (bufp - data);
    ...
    while (length > 0 && (*objidlength)-- > 0) {
        ...
        do {                    /* shift and add in low order 7 bits */
            subidentifier =
                (subidentifier << 7) + (*(u_char *) bufp & ~ASN01_BIT8);
            length--;
...
u_char * Asn01_parseUnsignedInt64(u_char * data,
                         size_t * datalength,
                         u_char * type,
                         struct Asn01_Counter64_s * cp, size_t countersize)
{
    /*
     * ASN.1 integer ::= 0x02 asnlength byte {byte}*
     */
    ...
    *type = *bufp++;
     ...
     */
    if ((*type == ASN01_OPAQUE) &&
        (asn_length <= ASN01_OPAQUE_COUNTER64_MX_BER_LEN) &&
        (*bufp == ASN01_OPAQUE_TAG1) &&
...
u_char * Asn01_buildUnsignedInt64(u_char * data,
                         size_t * datalength,
                         u_char type,
                         const struct Asn01_Counter64_s * cp, size_t countersize)
{
    /*
     * ASN.1 integer ::= 0x02 asnlength byte {byte}*
     */
                       ...
                       size_t countersize)
{
    ...
    *type = *bufp++;
    ...
    if ((*type == ASN01_OPAQUE) &&
        (asn_length <= ASN01_OPAQUE_COUNTER64_MX_BER_LEN) &&
        (*bufp == ASN01_OPAQUE_TAG1) && (*(bufp + 1) == ASN01_OPAQUE_I64)) {
        /*
                ...
                u_char * type, float *floatp, size_t floatsize)
{
    ...
    *type = *bufp++;
     ...
     */
    if ((*type == ASN01_OPAQUE) &&
        (asn_length == ASN01_OPAQUE_FLOAT_BER_LEN) &&
        (*bufp == ASN01_OPAQUE_TAG1) && (*(bufp + 1) == ASN01_OPAQUE_FLOAT)) {
                 ...
                 u_char * type, double *doublep, size_t doublesize)
{
    ...
    *type = *bufp++;
     ...
     */
    if ((*type == ASN01_OPAQUE) &&
        (asn_length == ASN01_OPAQUE_DOUBLE_BER_LEN) &&
        (*bufp == ASN01_OPAQUE_TAG1) && (*(bufp + 1) == ASN01_OPAQUE_DOUBLE)) {
...
int Asn01_reallocRbuildObjid(u_char ** pkt, size_t * pkt_len,
                         size_t * offset, int r,
                         u_char type,
                         const _oid * objid, size_t objidlength)
{
    /*
     * ASN.1 objid ::= 0x06 asnlength subidentifier {subidentifier}*
     * subidentifier ::= {leadingbyte}* lastbyte

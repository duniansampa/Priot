 ...
 *
 * Returns:
 *	API_ERR_SUCCESS		Success -- when a record for engineID is found.
                   ...
                   u_int * engine_time, u_int authenticated)
{
    ...
    if (!engine_time || !engineboot) {
        TOOLS_QUITFUN(API_ERR_GENERR, goto_getEnginetime);
    }
    ...
    *engine_time = *engineboot = 0;
    ...
    if (timediff > (int) (TOOLS_ENGINETIME_MAX - *engine_time)) {
        *engine_time = (timediff - (TOOLS_ENGINETIME_MAX - *engine_time));

        /*
         * FIX -- move this check up... should not change anything
         * * if engineboot is already locked.  ???
         */
        if (*engineboot < TOOLS_ENGINEBOOT_MAX) {
            *engineboot += 1;
        }
                  ...
                  u_int * last_engine_time, u_int authenticated)
{
    ...
    if (!engine_time || !engineboot || !last_engine_time) {
        TOOLS_QUITFUN(API_ERR_GENERR, goto_getEnginetimeEx);
    }
...
{
    int             rval = API_ERR_SUCCESS, iindex;
    LcdTime_EnginetimePTR e = NULL;
    ...
    if (!(e = LcdTime_searchEnginetimeList(engineID, engineID_len))) {
        if ((iindex = LcdTime_hashEngineID(engineID, engineID_len)) < 0) {
           TOOLS_QUITFUN(API_ERR_GENERR, goto_setEnginetime);
        }
...
LcdTime_EnginetimePTR LcdTime_searchEnginetimeList(const u_char * engineID, u_int engineID_len)
{
     ...
     */
    if (!engineID || (engineID_len <= 0)) {
        TOOLS_QUITFUN(API_ERR_GENERR, goto_searchEnginetimeListQuit);
    }
...
{
    int             rval = API_ERR_GENERR;
    size_t          buf_len = TOOLS_MAXBUF;
    u_int           additive = 0;
    u_char         *bufp, buf[TOOLS_MAXBUF];
    void           *context = NULL;
     ...
     */
    if (!engineID || (engineID_len <= 0)) {
        TOOLS_QUITFUN(API_ERR_GENERR, goto_hashEngineID);
    }
    ...
    rval = Scapi_hash(usm_hMACMD5AuthProtocol,
                   sizeof(usm_hMACMD5AuthProtocol) / sizeof(_oid),
                   engineID, engineID_len, buf, &buf_len);
    if (rval == API_ERR_SC_NOT_CONFIGURED) {
        /* fall back to sha1 */
        rval = Scapi_hash(usm_hMACSHA1AuthProtocol,
                   sizeof(usm_hMACSHA1AuthProtocol) / sizeof(_oid),
                   engineID, engineID_len, buf, &buf_len);
    }

...
void Logger_initLogger(void)
{
    ...
    DefaultStore_registerPremib(DATATYPE_BOOLEAN, "priot", "logTimestamp",
                                DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::LOG_TIMESTAMP);
        ...
        if (logh) {
            ...
            Logger_enableFilelog(logh,
                                   DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                                                          DEFAULTSTORE_LIB_BOOLEAN::APPEND_LOGFILES));
        }
        break;
        /*
         * Log to syslog
...
int Logger_logHandlerStdouterr( Logger_LogHandler * logh, int pri, const char *str)
{
    ...
    if ( DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                               DEFAULTSTORE_LIB_BOOLEAN::LOG_TIMESTAMP) && newline) {
...
int Logger_logHandlerSyslog(Logger_LogHandler * logh, int pri, const char *str)
{
    ...
    if (!(logh->imagic)) {
        ...
        if (!ident)
            ident = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_STRING::APPTYPE);
...
int Logger_logHandlerFile(Logger_LogHandler * logh, int pri, const char *str)
{
     ...
     */
    if (DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID, DEFAULTSTORE_LIB_BOOLEAN::LOG_TIMESTAMP) && logh->imagic) {
        Logger_sprintfStamp(NULL, sbuf);
    } else {
        strcpy(sbuf, "");
    }

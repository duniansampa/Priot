 ...
 * @return API_ERR_SUCCESS on success, API_ERR_GENERR if data is
...
OidStash_addData(OidStash_Node **root,
                           const _oid * lookup, size_t lookup_len, void *mydata)
{
...
OidStash_stashGetNode(OidStash_Node *root,
                           const _oid * lookup, size_t lookup_len)
{
    OidStash_Node *curnode, *tmpp, *loopp;
    unsigned int    i;
...
OidStash_storeAll(int majorID, int minorID,
    ...
    _oid oidbase[TYPES_MAX_OID_LEN];
...
OidStash_store(OidStash_Node *root,
    ...
    OidStash_Node *tmpp;
    char *cp;
    char *appname = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,

 ...
 *
 * Returns:
 *	API_ERR_SUCCESS			Success.
 *	API_ERR_GENERR			All errors.
 *
 *
 ...
 */
int Keytools_generateKu(const _oid * hashtype, u_int hashtype_len, const u_char * P, size_t pplen, u_char * Ku, size_t * kulen){
    int     rval = API_ERR_SUCCESS,
    ...
    if (!hashtype || !P || !Ku || !kulen || (*kulen <= 0)
        ...
        TOOLS_QUITFUN(API_ERR_GENERR, generate_Ku_quit);
    }
 ...
 */
int Keytools_generateKul(const _oid * hashtype, u_int hashtype_len, const u_char * engineID, size_t engineID_len, const u_char * Ku, size_t ku_len, u_char * Kul, size_t * kul_len){
    ...
    if (!hashtype || !engineID || !Ku || !Kul || !kul_len
        ...
        || (hashtype_len != TOOLS_USM_LENGTH_OID_TRANSFORM)) {
        TOOLS_QUITFUN(API_ERR_GENERR, generate_kul_quit);
    }


    iproperlength = Scapi_getProperLength(hashtype, hashtype_len);
    if (iproperlength == API_ERR_GENERR)
        TOOLS_QUITFUN(API_ERR_GENERR, generate_kul_quit);
    ...
    TOOLS_QUITFUN(rval, generate_kul_quit);
 ...
 */
int Keytools_encodeKeychange(const _oid * hashtype, u_int hashtype_len, u_char * oldkey, size_t oldkey_len, u_char * newkey, size_t newkey_len, u_char * kcstring, size_t * kcstring_len)
{
        int             rval = API_ERR_SUCCESS;
        int             iproperlength;
        size_t          properlength;
        ...
        if (!hashtype || !oldkey || !newkey || !kcstring || !kcstring_len
            ...
            || (hashtype_len != TOOLS_USM_LENGTH_OID_TRANSFORM)) {
            TOOLS_QUITFUN(API_ERR_GENERR, encode_keychange_quit);
        }
        ...
        TOOLS_QUITFUN(rval, encode_keychange_quit);
 ...
 * XXX:  if the newkey is not long enough, it should be freed and remalloced
 */
int Keytools_decodeKeychange(const _oid * hashtype, u_int hashtype_len, u_char * oldkey, size_t oldkey_len,
    ...
    if (!hashtype || !oldkey || !kcstring || !newkey || !newkey_len
        ...
        || (hashtype_len != TOOLS_USM_LENGTH_OID_TRANSFORM)) {
        TOOLS_QUITFUN(API_ERR_GENERR, decode_keychange_quit);
    }
    ...
    *newkey_len = properlength;
    ...
    if (tmpbuf) {
        ...
        rval = Scapi_hash(hashtype, hashtype_len, tmpbuf, properlength * 2,
                       tmp_buf, &tmp_buf_len);
        TOOLS_QUITFUN(rval, decode_keychange_quit);

                                            ...
                                            int oldStorage)
{
     ...
     */
    if ((TC_RS_DESTROY == newValue) &&
        ((PRIOT_STORAGE_PERMANENT == oldStorage) ||
         (PRIOT_STORAGE_READONLY == oldStorage)))
        return PRIOT_ERR_WRONGVALUE;
...
char Tc_checkStorageTransition(int oldValue, int newValue)
{
    ...
    case PRIOT_STORAGE_NONVOLATILE:
        if (newValue == PRIOT_STORAGE_PERMANENT ||
            newValue == PRIOT_STORAGE_READONLY)
            return PRIOT_ERR_INCONSISTENTVALUE;
    }

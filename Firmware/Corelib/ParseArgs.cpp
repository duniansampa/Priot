    ...
    while ((arg = getopt(argc, argv, Opts)) != EOF) {
        ...
        case 'd':
            DefaultStore_setBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                   DEFAULTSTORE_LIB_BOOLEAN::DUMP_PACKET, 1);
            break;

        case 'v':
            session->version = -1;
            if (!strcmp(optarg, "1")) {
                session->version = PRIOT_VERSION_1;
            }
            if (!strcasecmp(optarg, "2c")) {
                session->version = PRIOT_VERSION_2c;
            }
...
#define SNMPV3_CMD_OPTIONS
    ...
    if (session->version == API_DEFAULT_VERSION) {
         ...
         */
        session->version = DefaultStore_getInt(DEFAULTSTORE_STORAGE::LIBRARY_ID,
        ...
        if (!session->version) {
            ...
            if (session->version == DEFAULTSTORE_PRIOT_VERSION_1)  /* bogus value.  version 1 actually = 0 */
                session->version = PRIOT_VERSION_1;
        }
    }
    ...
    if (session->version == PRIOT_VERSION_1 ||
    session->version == PRIOT_VERSION_2c)
    {
        if (Cpsz == NULL) {
            Cpsz = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                     DEFAULTSTORE_LIB_STRING::COMMUNITY);
        if (Cpsz == NULL) {
                if (DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                                           DEFAULTSTORE_LIB_BOOLEAN::IGNORE_NO_COMMUNITY)){
                    DEBUG_MSGTL(("snmp_parse_args",
                                "ignoring that the community string is not present\n"));
                    session->community = NULL;
                    session->communityLen = 0;
                } else {
            ...
            session->communityLen = strlen(Cpsz);
        }
    }

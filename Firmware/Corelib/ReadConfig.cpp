 ...
 *
 * DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID, NETSNMP_DS_LIB_APPTYPE);
 *
                 ...
                 int when)
{
    ...
    if (type == NULL || *type == '\0') {
        type = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
...
ReadConfig_withTypeWhen(const char *filename, const char *type, int when)
{
    ...
    if (ctmp)
        ...
        DEBUG_MSGTL(("read_config",
                    ...
                    type, filename));
    return API_ERR_GENERR;     /* No config files read */
}
    ...
    const char *token, char *cptr, int when)
{
    ...
    if (lptr != NULL) {
        if (when == EITHER_CONFIG || lptr->config_time == when) {
           ...
           !DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                       DEFAULTSTORE_LIB_BOOLEAN::NO_TOKEN_WARNINGS)) {
...
int ReadConfig_configWhen(char *line, int when)
{
    ...
    if (lptr == NULL && DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                      DEFAULTSTORE_LIB_BOOLEAN::NO_TOKEN_WARNINGS)) {
...
int ReadConfig_config(char *line)
{
    ...
    if (DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                   DEFAULTSTORE_LIB_BOOLEAN::HAVE_READ_CONFIG)) {
 ...
 *
 * Returns API_ERR_SUCCESS if the file is processed successfully.
 * Returns API_ERR_GENERR  if it cannot.
 *    Note that individual config token errors do not trigger API_ERR_GENERR
            ...
            ReadConfig_error("No handlers regestered for type %s.",
                ...
                if ( strcasecmp( token, "include" )==0) {
                    ...
                    if (ReadConfig_readConfig(fname, line_handler, when) !=
                        API_ERR_SUCCESS && when != PREMIB_CONFIG)
                    ...
                    if (( ret != API_ERR_SUCCESS ) && (when != PREMIB_CONFIG))
...
/*
 * Return API_ERR_SUCCESS if any config files are processed
 * Return API_ERR_GENERR if _no_ config files are processed
...
ReadConfig_optional(const char *optional_config, int when)
{
    ...
    int              ret = API_ERR_GENERR;
    char *type = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                       DEFAULTSTORE_LIB_STRING::APPTYPE);
    ...
    while (cp) {
        ...
        if (stat(cp, &statbuf)) {
            ...
            if ( ReadConfig_withTypeWhen(cp, type, when) == API_ERR_SUCCESS )
                ret = API_ERR_SUCCESS;
        }
        cp = strtok_r(NULL, ",", &st);
    }
...
{
    char *optional_config = DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                           DEFAULTSTORE_LIB_STRING::OPTIONALCONFIG);
    ...
    DefaultStore_setBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
               DEFAULTSTORE_LIB_BOOLEAN::HAVE_READ_CONFIG, 1);
...
ReadConfig_readPremibConfigs(void)
{
    ...
    DefaultStore_setBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
               DEFAULTSTORE_LIB_BOOLEAN::HAVE_READ_PREMIB_CONFIG, 1);
...
ReadConfig_setConfigurationDirectory(const char *dir)
{
    DefaultStore_setString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
              DEFAULTSTORE_LIB_STRING::CONFIGURATION_DIR, dir);
}
...
ReadConfig_getConfigurationDirectory(void)
{
    ...
    if (NULL == DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                      DEFAULTSTORE_LIB_STRING::CONFIGURATION_DIR)) {
    ...
    return (DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                  DEFAULTSTORE_LIB_STRING::CONFIGURATION_DIR));
}
...
{
    DefaultStore_setString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
              DEFAULTSTORE_LIB_STRING::PERSISTENT_DIR, dir);
}
...
{
    if (NULL == DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                      DEFAULTSTORE_LIB_STRING::PERSISTENT_DIR)) {
...
{
    DefaultStore_setString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
              DEFAULTSTORE_LIB_STRING::TEMP_FILE_PATTERN, pattern);
}
...
{
    if (NULL == DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                      DEFAULTSTORE_LIB_STRING::TEMP_FILE_PATTERN)) {
            ...
            for (j = 0; j <= MAX_PERSISTENT_BACKUPS; j++) {
                ...
                if (stat(configfile, &statbuf) != 0) {
                    ...
                    DEBUG_MSGTL(("read_config_files",
                                ...
                                configfile));
                    if (ReadConfig_readConfig(configfile, ctmp->start, when) == API_ERR_SUCCESS)
                        ret = API_ERR_SUCCESS;
                }
            }
        }
...
int ReadConfig_filesOfType(int when, struct ReadConfig_ConfigFiles_s *ctmp)
{
    ...
    if (DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                               DEFAULTSTORE_LIB_BOOLEAN::DONT_PERSIST_STATE)
        || DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                                  DEFAULTSTORE_LIB_BOOLEAN::DISABLE_CONFIG_LOAD)
...
int ReadConfig_files(int when) {
     ...
     */
    for (; ctmp != NULL; ctmp = ctmp->next) {
        if ( ReadConfig_filesOfType(when, ctmp) == API_ERR_SUCCESS )
            ret = API_ERR_SUCCESS;
    }
...
ReadConfig_store(const char *type, const char *line)
{
    ...
    if (DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                               DEFAULTSTORE_LIB_BOOLEAN::DONT_PERSIST_STATE)
     || DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                               DEFAULTSTORE_LIB_BOOLEAN::DISABLE_PERSISTENT_LOAD)) return;
...
{
    ReadConfig_store(DefaultStore_getString(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                        DEFAULTSTORE_LIB_STRING::APPTYPE), line);
}
...
ReadConfig_savePersistent(const char *type)
{
    ...
    if (DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                               DEFAULTSTORE_LIB_BOOLEAN::DONT_PERSIST_STATE)
     || DefaultStore_getBoolean(DEFAULTSTORE_STORAGE::LIBRARY_ID,
                               DEFAULTSTORE_LIB_BOOLEAN::DISABLE_PERSISTENT_SAVE)) return;
 ...
 */
char * ReadConfig_saveObjid(char *saveto, _oid * objid, size_t len)
{
    int             i;
 ...
 */
char * ReadConfig_readObjid(char *readfrom, _oid ** objid, size_t * len)
{
    return TOOLS_REMOVE_CONST(char *,
...
const char  *
ReadConfig_readObjidConst(const char *readfrom, _oid ** objid, size_t * len)
{
    ...
    if (*objid == NULL) {
        *len = 0;
        if ((*objid = (_oid *) malloc(TYPES_MAX_OID_LEN * sizeof(_oid))) == NULL)
            return NULL;
                        ...
                        char *dataptr, size_t * len)
{
    ...
    case ASN01_OBJECT_ID:
        readfrom =
            ReadConfig_readObjid(readfrom, (_oid **) & dataptr, len);
        *len *= sizeof(_oid);
        return readfrom;
